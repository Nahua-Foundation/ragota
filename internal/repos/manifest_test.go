...
func TestSectionsAreIndependent(t *testing.T) {
	t.Run("ignore only", func(t *testing.T) {
		m, err := LoadManifest(write(t, ".ragota.yaml", "ignore:\n  - \"**/generated/**\"\n"))
		if err != nil {
			t.Fatal(err)
		}
		if len(m.Services) != 0 {
	...
	t.Run("services only", func(t *testing.T) {
		m, err := LoadManifest(write(t, ".ragota.yaml", "services:\n  - name: orders\n    root: services/orders\n"))
		if err != nil {
			t.Fatal(err)
		}
		if len(m.Ignore) != 0 {
		...
		want := []ManifestService{{Name: "orders", Root: "services/orders"}}
...
func TestYmlExtensionIsAccepted(t *testing.T) {
	m, err := LoadManifest(write(t, ".ragota.yml", "ignore: [\"docs/**\"]\n"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestMalformedManifestIsAnError(t *testing.T) {
	_, err := LoadManifest(write(t, ".ragota.yaml", "ignore:\n  - \"**/x/**\"\n   bad indentation: [\n"))
	if err == nil {
...
func TestNormalization(t *testing.T) {
	m, err := LoadManifest(write(t, ".ragota.yaml", `
services:
  - name: "  orders  "
    root: "/services/orders/"
  - name: ""
