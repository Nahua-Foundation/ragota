...
// pipeline (service.go) followed by a graph expansion around each hit — the
...
type ContextItem struct {
	Hit     *indexing.Hit        `json:"hit"`
	Unit    *storage.ASTUnit     `json:"unit,omitempty"`
...
func (s *Service) BuildContext(ctx context.Context, query string, repos []string, mode string, limit, hops int, intent string) (*ContextResult, error) {
	...
	resolved, _, err := s.promote.ResolveIntent(&indexing.SearchQuery{Query: query, Intent: intent})
	if err != nil {
...
func (s *Service) searchForContext(ctx context.Context, query string, repos []string, mode string, limit int, intent string) (*indexing.SearchResult, error) {
	return s.Search(ctx, &indexing.SearchQuery{
		Query:  query,
		Repos:  repos,
