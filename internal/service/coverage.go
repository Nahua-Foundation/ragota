...
// *indexing.IndexResult or by the indexer itself, whichever the producing
...
type CoverageReporter interface {
	// ContractCoverage returns, keyed by contract kind (storage.ContractKindHTTP
...
func (a *coverageAccumulator) add(counts map[string]storage.CoverageCounts) {
	a.reported = true
	if a.kinds == nil {
		a.kinds = make(map[string]storage.CoverageCounts, len(counts))
	}
...
// indexer reported.
func (a *coverageAccumulator) summary(repoID string, at time.Time) *storage.RepoCoverage {
	if !a.reported {
		return nil
	}
	kinds := make(map[string]storage.CoverageCounts, len(a.kinds))
	for kind, c := range a.kinds {
		kinds[kind] = c
	}
	return &storage.RepoCoverage{RepoID: repoID, UpdatedAt: at.Unix(), Kinds: kinds}
}
...
func (s *Service) storeCoverage(ctx context.Context, repoID string, acc *coverageAccumulator, complete bool) {
	...
	if err := s.storage.StoreRepoCoverage(ctx, summary); err != nil {
...
// is an error (storage.ErrNotFound); a known repo without a summary is not —
...
func (s *Service) RepoCoverage(ctx context.Context, repoID string) (*CoverageReport, error) {
	...
	cov, err := s.storage.GetRepoCoverage(ctx, repoID)
...
func coverageKind(kind string, counts storage.CoverageCounts) CoverageKind {
	return CoverageKind{
		Kind:       kind,
