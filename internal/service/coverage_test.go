...
type coverageIndexer struct {
	mockIndexer
	counts map[string]storage.CoverageCounts
}

func (c *coverageIndexer) ContractCoverage() map[string]storage.CoverageCounts { return c.counts }
...
func TestDoIndexPersistsCoverage(t *testing.T) {
	...
	idx := &coverageIndexer{
		mockIndexer: mockIndexer{name: "ast", indexType: indexing.IndexTypeAST},
		counts: map[string]storage.CoverageCounts{
			storage.ContractKindHTTP: {Candidates: 3000, Edges: 104},
			storage.ContractKindDB:   {Candidates: 12, Edges: 12},
		},
	}
	st := &mockStorage{}
	svc := newLocalTestService(t, st, map[indexing.IndexType]indexing.Indexer{indexing.IndexTypeAST: idx})
	...
	if got := byKind[storage.ContractKindHTTP]; got.Candidates != 3000 || got.Edges != 104 {
		t.Errorf("http coverage = %+v, want 104 of 3000", got)
	}
	if got := byKind[storage.ContractKindDB].Ratio; got != 1 {
		t.Errorf("db ratio = %v, want 1 (every candidate produced an edge)", got)
	}
	if got := byKind[storage.ContractKindMessaging].Ratio; got != 1 {
...
func TestRepoCoverageUnreported(t *testing.T) {
	ctx := context.Background()
	st := &mockStorage{repo: &repos.Repo{ID: "r1", Name: "t", IndexedAt: 42}}
...
func TestCoverageAccumulatorSumsBatches(t *testing.T) {
	...
	idx := &coverageIndexer{counts: map[string]storage.CoverageCounts{
		storage.ContractKindHTTP: {Candidates: 10, Edges: 4},
	}}
	acc.collect(idx, &indexing.IndexResult{})
	acc.collect(idx, &indexing.IndexResult{})
	...
	if got := summary.Kinds[storage.ContractKindHTTP]; got.Candidates != 20 || got.Edges != 8 {
...
func TestCoveragePartialPassKeepsPreviousSummary(t *testing.T) {
	...
	idx := &coverageIndexer{
		mockIndexer: mockIndexer{name: "ast", indexType: indexing.IndexTypeAST},
		counts:      map[string]storage.CoverageCounts{storage.ContractKindHTTP: {Candidates: 40, Edges: 40}},
	}
	st := &mockStorage{}
	svc := newLocalTestService(t, st, map[indexing.IndexType]indexing.Indexer{indexing.IndexTypeAST: idx})
	repo := &repos.Repo{ID: "r1", Name: "t", Source: repos.SourceTypeLocal, Path: dir}
	...
	st.mu.Unlock()
	idx.counts = map[string]storage.CoverageCounts{storage.ContractKindHTTP: {}}
	...
	for _, k := range report.Kinds {
		if k.Kind == storage.ContractKindHTTP && k.Candidates != 40 {
