...
func (s *Service) ApplyCommits(ctx context.Context, repoID string, commits []CommitEvent) (*CommitAck, error) {
	...
	if s.distributed() {
		...
		job, jerr := s.storage.EnqueueCommitJob(ctx, repoID, payload)
		if jerr != nil {
		...
		if perr := s.storage.SetRepoPendingCommit(ctx, repoID, lastSHA); perr != nil {
...
func (s *Service) runApplyCommits(ctx context.Context, repo *repos.Repo, commits []CommitEvent) error {
	...
	if len(unapplied) > 0 {
		...
		if uerr := s.storage.UpdateRepoStatus(statusCtx, repo.ID, repos.StatusError, msg, repo.IndexedAt); uerr != nil {
			slog.Error("update repo status to error failed", "repo_id", repo.ID, "err", uerr)
		}
	...
	lastSHA := commits[len(commits)-1].SHA
	if uerr := s.storage.UpdateRepoLastCommit(statusCtx, repo.ID, lastSHA); uerr != nil {
...
func (s *Service) doApplyCommits(ctx context.Context, repo *repos.Repo, commits []CommitEvent) ([]string, error) {
	start := time.Now()
	defer func() {
		obs.RecordDuration("ragota_apply_commits_seconds", time.Since(start).Seconds())
	}()
...
// a whole.
func (s *Service) applyPlan(ctx context.Context, repo *repos.Repo, plan map[string]commitPlanEntry, origin string) ([]string, error) {
	...
	for path, entry := range plan {
		...
		toIndex = append(toIndex, &indexing.FileToIndex{
			Path:     path,
			Hash:     hash,
			Language: lang,
			Content:  content,
		})
		processed = append(processed, &storage.File{
			RepoID:   repo.ID,
			Path:     path,
			Hash:     hash,
			Language: lang,
