...
func activeRepoList(active, inactive []string) []*repos.Repo {
	out := make([]*repos.Repo, 0, len(active)+len(inactive))
	for _, id := range active {
		out = append(out, &repos.Repo{ID: id, Name: id, Active: true})
	}
	for _, id := range inactive {
		out = append(out, &repos.Repo{ID: id, Name: id})
	}
	return out
}
...
func TestSearchDefaultsToActiveRepos(t *testing.T) {
	...
	if _, err := svc.Search(context.Background(), &indexing.SearchQuery{Query: "add"}, "keyword"); err != nil {
...
func TestSearchExplicitReposReachInactive(t *testing.T) {
	...
	q := &indexing.SearchQuery{Query: "add", Repos: []string{"r2"}}
...
func TestSearchWithNothingActive(t *testing.T) {
	...
	res, err := svc.Search(context.Background(), &indexing.SearchQuery{Query: "add"}, "keyword")
	if err != nil {
		t.Fatalf("Search() error = %v", err)
	}
...
type symbolStorage struct {
	*mockStorage
	queries []storage.QueryOpts
	units   []*storage.ASTUnit
}

func (s *symbolStorage) GetASTUnits(_ context.Context, opts storage.QueryOpts) ([]*storage.ASTUnit, error) {
...
func symbolService(active, inactive []string) (*Service, *symbolStorage) {
	st := &symbolStorage{
		...
		units:       []*storage.ASTUnit{{ID: "u1", RepoID: "r1", Name: "Handle"}},
	}
...
func theQuery(t *testing.T, st *symbolStorage) storage.QueryOpts {
	t.Helper()
	if len(st.queries) != 1 {
...
func TestSymbolsDefaultToActiveRepos(t *testing.T) {
	...
	if _, err := svc.Symbols(context.Background(), storage.QueryOpts{Name: "Handle"}); err != nil {
...
func TestSymbolsExplicitRepoReachesInactive(t *testing.T) {
	...
	units, err := svc.Symbols(context.Background(), storage.QueryOpts{RepoID: "r2", Name: "Handle"})
	if err != nil {
		t.Fatalf("Symbols() error = %v", err)
	}
	if len(units) != 1 {
	...
	if _, err := svc.Symbols(context.Background(), storage.QueryOpts{RepoID: "r2", Name: "Handle"}); err != nil {
...
func TestSymbolsWithNothingActive(t *testing.T) {
	...
	units, err := svc.Symbols(context.Background(), storage.QueryOpts{Name: "Handle"})
	if err != nil {
		t.Fatalf("Symbols() error = %v", err)
	}
	if len(units) != 0 {
