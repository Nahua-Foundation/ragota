...
func contractUseKinds(query string, callers bool) []string {
	var kinds []string
	if callers {
		kinds = append(kinds, storage.EdgeRPCCall, storage.EdgeHTTPCall)
	}
	if consumeVerbRe.MatchString(query) {
		kinds = append(kinds, storage.EdgeConsumes)
	}
	if produceVerbRe.MatchString(query) {
		kinds = append(kinds, storage.EdgeProduces)
	}
	return kinds
}
...
// the graph.
func (p *Promoter) PromoteContractUses(ctx context.Context, q *indexing.SearchQuery, intent string,
	...
	promoted := make([]*indexing.Hit, 0, len(edges))
