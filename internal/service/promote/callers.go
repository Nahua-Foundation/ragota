...
var structuralEdgeKinds = map[string]bool{
	storage.EdgeImport:        true,
	storage.EdgeImplementsRPC: true,
	storage.EdgeRuntimeCall:   true,
}
...
var callerKindRank = map[string]int{
	storage.EdgeCall:        0,
	storage.EdgeRPCCall:     1,
	storage.EdgeHTTPCall:    2,
	storage.EdgeHandledBy:   3, // src is the route unit: the registration site
	storage.EdgeProduces:    4,
	storage.EdgeConsumes:    5,
	storage.EdgeWritesTo:    6,
	storage.EdgeReadsFrom:   7,
	storage.EdgeRPCRequest:  8,
	storage.EdgeRPCResponse: 9,
	storage.EdgeKafkaFlow:   10,
}

// callerKindVerb phrases a hit's Reason for each edge kind.
var callerKindVerb = map[string]string{
	storage.EdgeCall:        "calls",
	storage.EdgeRPCCall:     "rpc call to",
	storage.EdgeHTTPCall:    "http call to",
	storage.EdgeHandledBy:   "routes to",
	storage.EdgeProduces:    "publishes to",
	storage.EdgeConsumes:    "consumes",
	storage.EdgeWritesTo:    "writes",
	storage.EdgeReadsFrom:   "reads",
	storage.EdgeRPCRequest:  "uses message",
	storage.EdgeRPCResponse: "uses message",
	storage.EdgeKafkaFlow:   "feeds",
}
...
func (p *Promoter) PromoteCallers(ctx context.Context, q *indexing.SearchQuery, callee string,
	hits []*indexing.Hit, meta map[string]interface{}) []*indexing.Hit {

	if p.storage == nil || p.graph == nil {
		return hits
	}
...
// repeated.
func prependPromoted(promoted, hits []*indexing.Hit) []*indexing.Hit {
	top := float32(1.0)
	...
	merged := make([]*indexing.Hit, 0, len(promoted)+len(hits))
...
func (p *Promoter) calleeUnits(ctx context.Context, q *indexing.SearchQuery, callee string, hits []*indexing.Hit) []*calleeGroup {
	var out []*calleeGroup
	byKey := map[string]*calleeGroup{}
	add := func(u *storage.ASTUnit) {
		if u == nil || u.ID == "" {
			return
		}
	...
	for _, tok := range identifierTokens(callee) {
		opts := storage.QueryOpts{Name: tok, Limit: calleeNameLookup}
		if len(q.Repos) == 1 {
...
func rankGateKind(kind string) bool {
	return kind != storage.KindConfigKey && kind != storage.KindSummary
}
...
// the others.
func (p *Promoter) callerHits(ctx context.Context, q *indexing.SearchQuery, callees []*calleeGroup) []*indexing.Hit {
	filters := indexing.ParseFilters(q.Filter)

	perCallee := make([][]*storage.Edge, len(callees))
	var srcIDs []string
...
// which.
func (p *Promoter) incomingEdges(ctx context.Context, g *calleeGroup) []*storage.Edge {
	...
	for _, id := range g.ids {
		...
		found, err := p.storage.GetEdges(ctx, storage.QueryOpts{
	...
	if len(edges) < callerEdgeLimit {
		unresolved, uerr := p.storage.GetEdges(ctx, storage.QueryOpts{
...
func sortCallerEdges(edges []*storage.Edge) {
...
func sortEdgesBy(edges []*storage.Edge, rank func(kind string) int) {
...
func resolvedEdge(e *storage.Edge) bool {
...
func callerHit(e *storage.Edge, src *storage.ASTUnit, callee *storage.ASTUnit) *indexing.Hit {
...
func callerSnippet(e *storage.Edge, src, callee *storage.ASTUnit, verb string) string {
...
func (p *Promoter) unitsByID(ctx context.Context, ids []string) map[string]*storage.ASTUnit {
	out := map[string]*storage.ASTUnit{}
	if len(ids) == 0 {
		return out
	}
	units, err := p.storage.GetASTUnitsByIDs(ctx, dedupStrings(ids))
	if err != nil {
...
func overlapsAny(h *indexing.Hit, list []*indexing.Hit) bool {
	for _, p := range list {
		if p.Overlaps(h) {
			return true
		}
