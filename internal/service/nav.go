...
func (s *Service) GetASTUnits(ctx context.Context, opts storage.QueryOpts) ([]*storage.ASTUnit, error) {
	if s.storage == nil {
		return nil, fmt.Errorf("storage not available")
	}
	return s.storage.GetASTUnits(ctx, opts)
}

// GetEdges returns edges matching the query options.
func (s *Service) GetEdges(ctx context.Context, opts storage.QueryOpts) ([]*storage.Edge, error) {
	if s.storage == nil {
		return nil, fmt.Errorf("storage not available")
	}
	return s.storage.GetEdges(ctx, opts)
}
...
func (s *Service) Symbols(ctx context.Context, opts storage.QueryOpts) ([]*storage.ASTUnit, error) {
...
// in a large file.
func (s *Service) Definition(ctx context.Context, repoID, filePath string, line int) (*storage.ASTUnit, error) {
	if line <= 0 {
		return nil, nil
	}
	units, err := s.GetASTUnits(ctx, storage.QueryOpts{
...
func (s *Service) References(ctx context.Context, repoID, filePath string, line, limit int) ([]*storage.Edge, error) {
	...
	references, err := s.GetEdges(ctx, storage.QueryOpts{DstID: unit.ID, Limit: limit})
	if err != nil {
		return nil, err
	}

	if room := limit - len(references); room > 0 {
		unresolved, err := s.GetEdges(ctx, storage.QueryOpts{
