...
func contractUseFixture(t *testing.T) *sqlite.SQLite {
	...
	for _, e := range []*storage.Edge{
		...
		{RepoID: "r1", SrcID: units["CatalogApi"].ID, Kind: storage.EdgeProduces,
			DstName:  "topic:ProductPriceChangedIntegrationEvent",
			FilePath: "src/Catalog.API/Apis/CatalogApi.cs", Line: 356, Confidence: 0.9},
		{RepoID: "r1", SrcID: units["loadgen"].ID, Kind: storage.EdgeHTTPCall,
			DstName: "http:GET /api/catalogue/products", FilePath: "load-gen/robot-shop.py", Line: 56, Confidence: 0.8},
		{RepoID: "r1", SrcID: units["cartTest"].ID, Kind: storage.EdgeRPCCall,
...
func firstPaths(hits []*indexing.Hit, n int) []string {
	var out []string
	for i, h := range hits {
		if i >= n {
			break
...
func TestSearchLeavesUndescribedContractsAlone(t *testing.T) {
	...
	tests := []struct {
		...
		// "Create" is one word every service has; the question names no service.
