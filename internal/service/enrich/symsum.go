...
// stands without it.
func (e *Enricher) SummarizeSymbols(ctx context.Context, repo *repos.Repo) error {
	...
	for start := 0; start < len(units); start += symbolBatchSize {
		...
		for i, text := range summaries {
			...
			meta.Summary = text
			u.Meta = storage.EncodeUnitMeta(meta)
			if serr := e.storage.StoreASTUnit(ctx, u); serr != nil {
...
func (e *Enricher) boundarySymbols(ctx context.Context, repoID string) ([]*storage.ASTUnit, error) {
	edges, err := e.storage.GetEdges(ctx, storage.QueryOpts{RepoID: repoID, Kinds: boundaryEdgeKinds})
	if err != nil {
	...
	units, err := e.storage.GetASTUnitsByIDs(ctx, dedupStrings(ids))
	if err != nil {
	...
	for _, u := range units {
		...
		case storage.DecodeUnitMeta(u.Meta).Summary != "": // already summarized
		default:
			out = append(out, u)
		}
	}
...
func (e *Enricher) summarizeBatch(ctx context.Context, repo *repos.Repo, batch []*storage.ASTUnit) ([]string, error) {
...
func (e *Enricher) symbolBody(repo *repos.Repo, u *storage.ASTUnit) string {
...
func (e *Enricher) reindexAnnotated(ctx context.Context, repo *repos.Repo, byFile map[string][]*storage.ASTUnit) error {
	vecIdx, ok := e.indexers[indexing.IndexTypeVector]
	if !ok || e.storage.VectorStore() == nil {
		return nil // nothing to carry them into
	}

	files := make([]*indexing.FileToIndex, 0, len(byFile))
	...
	for path, units := range byFile {
		...
		for _, u := range units {
			meta := storage.DecodeUnitMeta(u.Meta)
			annotations[indexing.AnnotationKey(path, u.StartLine)] = meta.Summary
		}
	}
	if len(files) == 0 {
		return nil
