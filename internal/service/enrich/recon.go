...
func (e *Enricher) ReconRepo(ctx context.Context, repo *repos.Repo) error {
	existing, err := e.storage.GetASTUnits(ctx, storage.QueryOpts{RepoID: repo.ID, Kind: KindRecon})
	if err != nil {
		return err
	}
	if len(existing) > 0 {
	...
	unit := &storage.ASTUnit{
		...
		Doc:       raw,
		Meta:      storage.EncodeUnitMeta(&storage.UnitMeta{DetectedBy: "llm"}),
		StartLine: 1,
		EndLine:   1,
	}
...
func (e *Enricher) ReconHints(ctx context.Context, repo *repos.Repo) []svcdetect.Service {
	units, err := e.storage.GetASTUnits(ctx, storage.QueryOpts{RepoID: repo.ID, Kind: KindRecon})
	...
	var hints []svcdetect.Service
	for _, sv := range res.Services {
		if sv.Name == "" {
			continue
		}
		hints = append(hints, svcdetect.Service{
			Name:       sv.Name,
			Root:       sv.Root,
