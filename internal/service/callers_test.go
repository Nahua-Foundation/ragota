...
//
//	shipping/service.go        ShipOrder (the callee, lines 40-80)
...
func callersFixture(t *testing.T) (*sqlite.SQLite, *storage.ASTUnit) {
	t.Helper()
	...
	callee := &storage.ASTUnit{
		RepoID: "r1", FilePath: "shipping/service.go", Language: "go",
	...
	for _, u := range []*storage.ASTUnit{callee, caller, testCaller} {
	...
	edges := []*storage.Edge{
		{RepoID: "r1", SrcID: caller.ID, DstID: callee.ID, Kind: storage.EdgeCall,
			DstName: "ShipOrder", FilePath: "checkout/main.go", Line: 25, Confidence: 0.9},
		{RepoID: "r1", SrcID: testCaller.ID, DstID: callee.ID, Kind: storage.EdgeCall,
		...
		{RepoID: "r1", SrcID: caller.ID, DstID: callee.ID, Kind: storage.EdgeImport,
...
func TestSearchExplicitCallersIntentOnBareSymbol(t *testing.T) {
	...
	srch := &stubSearcher{byQuery: map[string][]*indexing.Hit{}} // retrieval finds nothing
	svc := serviceWithSearcher(srch, st)
	defer svc.Close(context.Background())
...
func TestSearchIntentNoneLeavesResultsAlone(t *testing.T) {
	...
	srch := &stubSearcher{byQuery: map[string][]*indexing.Hit{
		"what calls the shipping service ShipOrder rpc": {{
			RepoID: "r1", FilePath: "shipping/service.go", Path: "shipping/service.go",
	...
	if len(res.Hits) != 1 || res.Hits[0].FilePath != "shipping/service.go" {
...
func TestCalleeGroupsUnitsSharingAQualifiedName(t *testing.T) {
	...
	var copies []*storage.ASTUnit
	for i := 0; i < 12; i++ {
		u := &storage.ASTUnit{
			RepoID: "r1", FilePath: fmt.Sprintf("src/svc%d/genproto/demo.pb.go", i),
			Language: "go", Kind: storage.KindRPCMethod, Name: "ShipOrder",
	...
	if err := st.StoreEdge(ctx, &storage.Edge{
		RepoID: "r1", SrcID: caller.ID, DstID: copies[len(copies)-1].ID, Kind: storage.EdgeRPCCall,
