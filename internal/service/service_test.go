...
// mockStorage is a test double for storage.Storage.
type mockStorage struct {
	mu          sync.Mutex
	initFunc    func(context.Context) error
	closeFunc   func() error
	filesByRepo map[string][]*storage.File
	getFileErr  error
	...
	claimCalls    int
	storedRepos   []*repos.Repo
	storedFiles   []*storage.File
	lastCommit    string
...
// shutdown behaviour.
func (m *mockStorage) StoreFile(ctx context.Context, f *storage.File) error {
...
func (m *mockStorage) GetFile(_ context.Context, repoID, path string) (*storage.File, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	if m.getFileErr != nil {
...
func (m *mockStorage) BatchStoreFiles(ctx context.Context, files []*storage.File) error {
	for _, f := range files {
...
func (m *mockStorage) GetFilesByRepo(ctx context.Context, repoID string) ([]*storage.File, error) {
...
func (m *mockStorage) StoreASTUnit(context.Context, *storage.ASTUnit) error { return nil }
func (m *mockStorage) StoreRepo(_ context.Context, r *repos.Repo) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	cp := *r
...
func (m *mockStorage) GetRepo(context.Context, string) (*repos.Repo, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	if m.getRepoErr != nil {
	...
	return &repos.Repo{ID: "repo-1", Name: "test", Source: repos.SourceTypeLocal}, nil
}
func (m *mockStorage) ListRepos(context.Context) ([]*repos.Repo, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	return m.repoList, m.listReposErr
}
func (m *mockStorage) ListActiveRepos(context.Context) ([]*repos.Repo, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	out := make([]*repos.Repo, 0, len(m.repoList))
...
func (m *mockStorage) UpdateRepoStatus(_ context.Context, _ string, st repos.Status, lastErr string, _ int64) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *mockStorage) GetASTUnits(context.Context, storage.QueryOpts) ([]*storage.ASTUnit, error) {
	return nil, nil
}
...
func (m *mockStorage) StoreEdge(context.Context, *storage.Edge) error     { return nil }
func (m *mockStorage) GetEdges(context.Context, storage.QueryOpts) ([]*storage.Edge, error) {
	return nil, nil
}
...
func (m *mockStorage) VectorStore() storage.VectorStorage                         { return nil }
...
func (m *mockStorage) GetASTUnitByID(context.Context, string) (*storage.ASTUnit, error) {
	return nil, storage.ErrNotFound
}
...
func (m *mockStorage) GetASTUnitsByIDs(context.Context, []string) ([]*storage.ASTUnit, error) {
	return nil, nil
}
func (m *mockStorage) BatchStoreASTUnits(context.Context, []*storage.ASTUnit) error { return nil }
func (m *mockStorage) BatchStoreEdges(context.Context, []*storage.Edge) error       { return nil }
...
func (m *mockStorage) StoreRepoCoverage(_ context.Context, c *storage.RepoCoverage) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	cp := *c
	m.coverage = &cp
...
func (m *mockStorage) GetRepoCoverage(_ context.Context, repoID string) (*storage.RepoCoverage, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *mockStorage) EnqueueIndexJob(_ context.Context, repoID string, force bool) (*storage.IndexJob, error) {
	return &storage.IndexJob{
		ID: "job-1", RepoID: repoID, Kind: storage.JobKindIndex, Force: force,
		Status: storage.JobStatusPending, CreatedAt: 7,
	}, nil
}
func (m *mockStorage) EnqueueCommitJob(_ context.Context, repoID, payload string) (*storage.IndexJob, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	...
	return &storage.IndexJob{
		ID: "commit-job-1", RepoID: repoID, Kind: storage.JobKindCommits,
		Status: storage.JobStatusPending, CreatedAt: 8,
	}, nil
}
func (m *mockStorage) GetIndexJob(_ context.Context, jobID string) (*storage.IndexJob, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	if m.job != nil {
		cp := *m.job
	...
	return &storage.IndexJob{ID: jobID, Kind: storage.JobKindIndex, Status: storage.JobStatusPending}, nil
}
func (m *mockStorage) ListIndexJobs(_ context.Context, repoID string, limit int) ([]*storage.IndexJob, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *mockStorage) ClaimNextIndexJob(context.Context, string) (*storage.IndexJob, error) {
	return nil, storage.ErrNotFound
}
...
// mockIndexer is a test double for indexing.Indexer.
...
func (m *mockIndexer) Type() indexing.IndexType { return m.indexType }
func (m *mockIndexer) Init(ctx context.Context, config map[string]interface{}) error {
	m.initCalled = true
	return nil
}
func (m *mockIndexer) Index(_ context.Context, req *indexing.IndexRequest) (*indexing.IndexResult, error) {
	if m.indexFunc != nil {
...
func (m *mockIndexer) Stats(context.Context) (*indexing.IndexerStats, error) {
	return &indexing.IndexerStats{}, nil
}
...
type mockSource struct {
	name    repos.SourceType
	addFunc func(context.Context, *repos.AddRequest) (*repos.Repo, error)
}

func (m *mockSource) Name() string                                                  { return string(m.name) }
func (m *mockSource) Type() repos.SourceType                                        { return m.name }
func (m *mockSource) Init(ctx context.Context, config map[string]interface{}) error { return nil }
func (m *mockSource) Add(ctx context.Context, req *repos.AddRequest) (*repos.Repo, error) {
	if m.addFunc != nil {
		return m.addFunc(ctx, req)
	}
	return &repos.Repo{ID: "test-repo", Name: req.Name, Source: m.name}, nil
}
func (m *mockSource) Remove(context.Context, string) error      { return nil }
func (m *mockSource) Update(context.Context, *repos.Repo) error { return nil }
func (m *mockSource) GetFiles(context.Context, *repos.Repo, []string) ([]*repos.RepoFile, error) {
	return nil, nil
}
func (m *mockSource) Clean(context.Context, *repos.Repo) error { return nil }
...
func TestNew(t *testing.T) {
	...
	t.Run("closes all components", func(t *testing.T) {
		...
		stor := &mockStorage{}
		idx := &mockIndexer{name: "test", indexType: indexing.IndexTypeCustom}
		src := &mockSource{name: repos.SourceTypeLocal}

		svc := New(&config.Config{}, stor,
			map[indexing.IndexType]indexing.Indexer{indexing.IndexTypeCustom: idx},
			map[repos.SourceType]repos.RepoSource{repos.SourceTypeLocal: src}, nil)
...
func TestAddRepo(t *testing.T) {
	...
	stor := &mockStorage{getRepoErr: storage.ErrNotFound}
	src := &mockSource{
		name: repos.SourceTypeLocal,
		addFunc: func(ctx context.Context, req *repos.AddRequest) (*repos.Repo, error) {
			return &repos.Repo{ID: "repo-1", Name: req.Name, Source: repos.SourceTypeLocal}, nil
		},
	}
	...
	repo, err := svc.AddRepo(ctx, repos.SourceTypeLocal, &repos.AddRequest{
	...
	t.Run("invalid source type", func(t *testing.T) {
		_, err := svc.AddRepo(ctx, "nonexistent", &repos.AddRequest{
...
func TestIndexRepo(t *testing.T) {
	...
	src := &mockSource{
		name: repos.SourceTypeLocal,
		addFunc: func(ctx context.Context, req *repos.AddRequest) (*repos.Repo, error) {
			return &repos.Repo{ID: "repo-1", Name: req.Name, Path: "/path/to/repo"}, nil
		},
	}
...
// TestServiceWithSQLite tests service with real SQLite storage.
func TestServiceWithSQLite(t *testing.T) {
	...
	stor := &mockStorage{filesByRepo: map[string][]*storage.File{}}
