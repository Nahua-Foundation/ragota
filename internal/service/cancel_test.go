...
func TestCancelledIndexLeavesTheRepoIdleNotFailed(t *testing.T) {
	...
	repo := &repos.Repo{ID: "r1", Name: "r1", Path: dir, Source: repos.SourceTypeLocal}
	...
	svc.sources = map[repos.SourceType]repos.RepoSource{repos.SourceTypeLocal: local}
	...
	if got.Status == repos.StatusError {
