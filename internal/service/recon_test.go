...
func TestReconPass(t *testing.T) {
	...
	// no build manifest, so heuristic detection cannot see it as a service.
	dir := t.TempDir()
	...
	repo := &repos.Repo{ID: "web-1", Name: "web", Source: repos.SourceTypeLocal, Path: dir}
	...
	recons, err := st.GetASTUnits(ctx, storage.QueryOpts{RepoID: "web-1", Kind: enrich.KindRecon})
	if err != nil {
		t.Fatal(err)
	}
	if len(recons) != 1 {
	...
	if meta := storage.DecodeUnitMeta(ru.Meta); meta.DetectedBy != "llm" {
		t.Errorf("recon unit DetectedBy = %q, want llm", meta.DetectedBy)
	}

	// The LLM hint became a service unit alongside the detected root service.
	assertServices := func() {
		t.Helper()
		units, err := st.GetASTUnits(ctx, storage.QueryOpts{RepoID: "web-1", Kind: storage.KindService})
		if err != nil {
			t.Fatal(err)
		}
		...
		payMeta := storage.DecodeUnitMeta(pay.Meta)
		...
		if meta := storage.DecodeUnitMeta(web.Meta); meta.DetectedBy == "llm" {
...
func TestReconFailureDoesNotFailIndex(t *testing.T) {
	...
	repo := &repos.Repo{ID: "web-2", Name: "web", Source: repos.SourceTypeLocal, Path: dir}
	if err := svc.doIndex(ctx, repo, false); err != nil {
		t.Fatalf("doIndex must not fail on recon errors, got %v", err)
	}
	recons, err := st.GetASTUnits(ctx, storage.QueryOpts{RepoID: "web-2", Kind: enrich.KindRecon})
	if err != nil {
		t.Fatal(err)
	}
	if len(recons) != 0 {
	...
	units, err := st.GetASTUnits(ctx, storage.QueryOpts{RepoID: "web-2", Kind: storage.KindService})
	if err != nil {
		t.Fatal(err)
	}
