...
// Open открывает (или создаёт) индекс по указанному пути.
//
// Параметры k1 и b игнорируются: Bleve v2 использует встроенный BM25Okapi
// с фиксированными параметрами (k1=1.2, b=0.75). Кастомизация требует
// подмены similarity-функции на уровне Bleve index, что не поддерживается
// публичным API. Параметры сохранены в сигнатуре для обратной совместимости.
...
func (b *bleveIndex) DeleteByPath(ctx context.Context, path string) error {
	...
	// Paginated delete: large files may have >10K chunks.
...
func (b *bleveIndex) Clear(ctx context.Context) error {
	...
	// Paginated delete: Bleve limits results to 100K per query.
	const batchSize = 10000
	for {
