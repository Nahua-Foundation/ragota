...
func TestUnitFilters(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			d:        SQLiteDialect{},
			opts:     storage.QueryOpts{NameOrQualified: "ShipOrder"},
		...
		},
		{
			...
			d:        PostgresDialect{},
			opts:     storage.QueryOpts{Language: "go", QualifiedSuffix: "Foo"},
		...
		},
		{
			...
			d:        SQLiteDialect{},
			opts:     storage.QueryOpts{Repos: []string{"r1", "r2"}, Name: "Foo"},
		...
		},
		{
			...
			d:        PostgresDialect{},
			opts:     storage.QueryOpts{RepoID: "r1", Repos: []string{"r1", "r2"}, Kind: "function"},
		...
		},
		{
			...
			d:        SQLiteDialect{},
			opts:     storage.QueryOpts{Repos: nil, Name: "Foo"},
			want:     " AND LOWER(name) = LOWER(?)",
			wantArgs: []any{"Foo"},
		},
		{
			...
			d:        SQLiteDialect{},
			tier:     TierSubstring,
			opts:     storage.QueryOpts{Repos: []string{"r1"}, Name: "Foo"},
		...
		},
		{
			...
			d:        SQLiteDialect{},
			opts:     storage.QueryOpts{FilePath: "a.go", Line: 900},
		...
		},
		{
			...
			d:        PostgresDialect{},
			opts:     storage.QueryOpts{Line: 12},
		...
		},
		{
			...
			d:        SQLiteDialect{},
			opts:     storage.QueryOpts{Line: 0, Name: "Foo"},
			want:     " AND LOWER(name) = LOWER(?)",
			wantArgs: []any{"Foo"},
		},
	}
...
func TestEdgeFilters(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			d:        SQLiteDialect{},
			opts:     storage.QueryOpts{SrcID: "not-a-number"},
			want:     " AND src_id = ?",
		...
		},
		{
			...
			d:        PostgresDialect{},
			opts:     storage.QueryOpts{Unresolved: true},
			want:     " AND dst_id = 0",
			wantArgs: nil,
		},
		{
			...
			d:        PostgresDialect{},
			opts:     storage.QueryOpts{Repos: []string{"r1"}, Kind: "call"},
			want:     " AND kind = $1",
			wantArgs: []any{"call"},
		},
	}
...
func TestFullQueryShape(t *testing.T) {
	opts := storage.QueryOpts{RepoID: "r1", Kinds: []string{"function", "method"}, QualifiedSuffix: "Foo", Limit: 3}
	...
	got, args = UnitQuery(PostgresDialect{}, storage.QueryOpts{Name: "Ship", Limit: 5},
	...
	got, args = UnitQuery(PostgresDialect{}, storage.QueryOpts{NameOrQualified: "Ship", Limit: 5},
...
func TestHandWrittenStatement(t *testing.T) {
	opts := storage.QueryOpts{RepoID: "r1", Name: "Charge", Limit: 2}
...
func TestQueryUnitsPlans(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			opts:  storage.QueryOpts{Name: "Ship", Limit: 4},
			first: 1,
		...
		},
		{
			...
			opts:  storage.QueryOpts{Name: "Ship", Fallback: true},
			first: 0,
			want:  []UnitTier{TierExact},
		},
		{
			...
			opts:  storage.QueryOpts{RepoID: "r1", Limit: 4, Fallback: true},
			first: 0,
			want:  []UnitTier{TierExact},
		},
		{
			...
			opts:  storage.QueryOpts{NameOrQualified: "Ship", Limit: 2, Fallback: true},
			first: 2,
			want:  []UnitTier{TierHandWritten},
		},
		{
			name:  "short page tops up",
			opts:  storage.QueryOpts{NameOrQualified: "Ship", Limit: 4, Fallback: true},
			first: 2,
			want:  []UnitTier{TierHandWritten, TierRemainder},
		},
	}
	rows := func(ids ...string) []*storage.ASTUnit {
		out := make([]*storage.ASTUnit, 0, len(ids))
		for _, id := range ids {
			out = append(out, &storage.ASTUnit{ID: id, Name: "Ship"})
		}
		return out
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := QueryUnits(d, tt.opts, intOrZero,
				func(query string, _ []any) ([]*storage.ASTUnit, error) {
