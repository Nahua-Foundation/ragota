...
func TestRepoCoverageRoundTrip(t *testing.T) {
	...
	if _, err := s.GetRepoCoverage(ctx, "repo-1"); !errors.Is(err, storage.ErrNotFound) {
	...
	first := &storage.RepoCoverage{
		...
		Kinds: map[string]storage.CoverageCounts{
			...
			storage.ContractKindMessaging: {Candidates: 8, Edges: 8},
		},
	}
	...
	if c := got.Kinds[storage.ContractKindHTTP]; c.Candidates != 3000 || c.Edges != 104 {
	...
	second := &storage.RepoCoverage{
		RepoID:    "repo-1",
		UpdatedAt: 200,
		Kinds:     map[string]storage.CoverageCounts{storage.ContractKindHTTP: {Candidates: 42, Edges: 42}},
	}
	...
	if len(got.Kinds) != 1 || got.Kinds[storage.ContractKindHTTP].Edges != 42 || got.UpdatedAt != 200 {
