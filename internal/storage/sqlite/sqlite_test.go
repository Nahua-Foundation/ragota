...
func TestGetEdgesFilters(t *testing.T) {
	...
	callEdges, err := s.GetEdges(context.Background(), storage.QueryOpts{RepoID: repoID, Kind: "call"})
	if err != nil {
	...
	nameEdges, err := s.GetEdges(context.Background(), storage.QueryOpts{RepoID: repoID, Name: "bar"})
	if err != nil {
	...
	limitEdges, err := s.GetEdges(context.Background(), storage.QueryOpts{RepoID: repoID, Limit: 1})
	if err != nil {
...
func TestClaimRepoExpiredClaimIsTakenOver(t *testing.T) {
	...
	r := &repos.Repo{ID: "repo-1", Name: "test", Source: "local", Path: "/tmp/test", Status: repos.StatusIdle, CreatedAt: 1000}
...
func TestResetStuckRepos(t *testing.T) {
	...
	for _, id := range []string{"repo-1", "repo-2"} {
		r := &repos.Repo{ID: id, Name: id, Source: "local", Path: "/tmp/" + id, Status: repos.StatusIdle, CreatedAt: 1000}
		if err := s.StoreRepo(ctx, r); err != nil {
			t.Fatalf("store repo: %v", err)
		}
	}
	...
	if got.LastError != storage.RepoResetMessage {
		t.Errorf("last_error after reset = %q, want %q", got.LastError, storage.RepoResetMessage)
	}
}
...
func TestIndexJobQueue(t *testing.T) {
	...
	if err := s.HeartbeatIndexJob(ctx, claimed.ID, "worker-2"); !errors.Is(err, storage.ErrNotFound) {
		t.Errorf("heartbeat from a foreign worker = %v, want ErrNotFound", err)
	}
	if err := s.CompleteIndexJob(ctx, claimed.ID, "worker-2", "boom"); !errors.Is(err, storage.ErrNotFound) {
	...
	if done.Status != storage.JobStatusDone || done.Error != "" {
...
func TestReleaseIndexJob(t *testing.T) {
	...
	if back.Status != storage.JobStatusPending || back.ClaimedBy != "" {
...
func TestCommitJobQueue(t *testing.T) {
	...
	if first.Kind != storage.JobKindCommits {
		t.Errorf("kind = %q, want %q", first.Kind, storage.JobKindCommits)
	}
...
func TestListIndexJobs(t *testing.T) {
	...
	if jobs[0].Kind != storage.JobKindCommits {
		t.Errorf("first listed job kind = %q, want the newest entry (%q)", jobs[0].Kind, storage.JobKindCommits)
	}
...
func TestDeleteEdgesByKindAndDst(t *testing.T) {
	...
	src := &storage.ASTUnit{RepoID: repoID, FilePath: "a.go", Language: "go", Kind: "function", Name: "src"}
	...
	rest, err := s.GetEdges(context.Background(), storage.QueryOpts{RepoID: repoID})
	if err != nil {
...
func TestDeleteFilesByPathsChunks(t *testing.T) {
	...
	for i := range files {
		...
		files[i] = &storage.File{RepoID: "r1", Path: paths[i], Language: "go", Hash: "h", Indexed: true}
	}
...
func TestDeleteByFilesChunksBeyondParameterLimit(t *testing.T) {
	...
	for i := 0; i < nFiles; i++ {
		...
		u := &storage.ASTUnit{RepoID: "repo-1", FilePath: path, Language: "go", Kind: "function", Name: "F"}
		if err := s.StoreASTUnit(ctx, u); err != nil {
			t.Fatalf("store unit: %v", err)
		}
		if err := s.StoreEdge(ctx, &storage.Edge{RepoID: "repo-1", SrcID: u.ID, Kind: storage.EdgeCall,
	...
	units, err := s.GetASTUnits(ctx, storage.QueryOpts{RepoID: "repo-1"})
	if err != nil {
	...
	edges, err := s.GetEdges(ctx, storage.QueryOpts{RepoID: "repo-1"})
	if err != nil {
		t.Fatalf("get edges: %v", err)
	}
	if len(edges) != 0 {
...
func TestDeleteByFilesEmptyIsNoOp(t *testing.T) {
	...
	u := &storage.ASTUnit{RepoID: "repo-1", FilePath: "a.go", Language: "go", Kind: "function", Name: "A"}
	if err := s.StoreASTUnit(ctx, u); err != nil {
		t.Fatalf("store unit: %v", err)
	}
	if err := s.StoreEdge(ctx, &storage.Edge{RepoID: "repo-1", SrcID: u.ID, Kind: storage.EdgeCall,
