...
func jobFromRow(r pgdb.GetIndexJobRow) *storage.IndexJob {
...
// queued cheap one.
func (p *Postgres) EnqueueIndexJob(ctx context.Context, repoID string, force bool) (*storage.IndexJob, error) {
	row, err := p.q.InsertIndexJob(ctx, pgdb.InsertIndexJobParams{
		RepoID:    repoID,
		Force:     force,
	...
	return jobFromRow(pgdb.GetIndexJobRow(row)), nil
}
...
func (p *Postgres) EnqueueCommitJob(ctx context.Context, repoID, payload string) (*storage.IndexJob, error) {
	row, err := p.q.InsertCommitJob(ctx, pgdb.InsertCommitJobParams{
		RepoID:    repoID,
		CreatedAt: time.Now().Unix(),
		Payload:   payload,
	})
...
func (p *Postgres) GetIndexJob(ctx context.Context, jobID string) (*storage.IndexJob, error) {
...
func (p *Postgres) ListIndexJobs(ctx context.Context, repoID string, limit int) ([]*storage.IndexJob, error) {
	if limit <= 0 {
		limit = defaultJobListLimit
	}
	rows, err := p.q.ListIndexJobs(ctx, pgdb.ListIndexJobsParams{RepoID: repoID, Limit: int32(limit)})
	if err != nil {
		return nil, fmt.Errorf("list index jobs: %w", err)
	}
	jobs := make([]*storage.IndexJob, 0, len(rows))
	for _, r := range rows {
		jobs = append(jobs, jobFromRow(pgdb.GetIndexJobRow(r)))
	}
	return jobs, nil
}
...
func (p *Postgres) HasPendingCommitJobBefore(ctx context.Context, repoID, jobID string) (bool, error) {
	ok, err := p.q.HasPendingCommitJobBefore(ctx, pgdb.HasPendingCommitJobBeforeParams{
...
func (p *Postgres) ClaimNextIndexJob(ctx context.Context, workerID string) (*storage.IndexJob, error) {
	row, err := p.q.ClaimNextIndexJob(ctx, pgdb.ClaimNextIndexJobParams{
		ClaimedBy: workerID,
	...
	job := jobFromRow(pgdb.GetIndexJobRow{
...
func (p *Postgres) HeartbeatIndexJob(ctx context.Context, jobID, workerID string) error {
	n, err := p.q.HeartbeatIndexJob(ctx, pgdb.HeartbeatIndexJobParams{
...
func (p *Postgres) CompleteIndexJob(ctx context.Context, jobID, workerID string, jobErr string) error {
	...
	n, err := p.q.CompleteIndexJob(ctx, pgdb.CompleteIndexJobParams{
		Status:    status,
		Error:     jobErr,
		ID:        intOrZero(jobID),
		ClaimedBy: workerID,
...
func (p *Postgres) ReleaseIndexJob(ctx context.Context, jobID, workerID string) error {
	n, err := p.q.ReleaseIndexJob(ctx, pgdb.ReleaseIndexJobParams{
		ID:        intOrZero(jobID),
		ClaimedBy: workerID,
	})
	if err != nil {
