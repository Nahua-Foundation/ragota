...
func (p *Postgres) StoreRepoCoverage(ctx context.Context, c *storage.RepoCoverage) error {
	if c == nil || c.RepoID == "" {
...
func (p *Postgres) GetRepoCoverage(ctx context.Context, repoID string) (*storage.RepoCoverage, error) {
	...
	for rows.Next() {
		...
		cov.Kinds[kind] = storage.CoverageCounts{Candidates: int(candidates), Edges: int(edges)}
