...
const (
	...
	// refused because the repository was already indexing.
...
type Indexer interface {
	...
	IgnorePatternsFor(repo *repos.Repo) []string
	...
	ApplyLocalChanges(ctx context.Context, repoID string, files []service.CommitFile) error
}
...
func (w *Watcher) Add(repo *repos.Repo) error {
...
func (w *Watcher) changeSet(ctx context.Context, t *tree, b *batch) (changes []service.CommitFile, changed, deleted int) {
	for dir := range b.dirs {
		...
		for _, p := range paths {
			changes = append(changes, service.CommitFile{Path: p, Status: "D"})
			deleted++
		}
	}
	...
	for rel := range b.files {
		...
		if _, err := os.Lstat(abs); err != nil {
			changes = append(changes, service.CommitFile{Path: rel, Status: "D"})
			deleted++
			continue
		}
		...
		changes = append(changes, service.CommitFile{Path: rel, Status: "M"})
		changed++
	}
