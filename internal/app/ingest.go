	...
	go func() {
		...
		if source, ok := s.sources[repo.Source]; ok {
			if err := source.Update(s.baseCtx, repo); err != nil {
				...
				uerr := s.store.UpdateRepoStatus(statusCtx, repo.ID, domain.StatusError, fmt.Sprintf("update source: %v", err), repo.IndexedAt)
				cancel()
				if uerr != nil {
...
func (s *Service) FindRepoByHints(ctx context.Context, hints []string) (*domain.Repo, error) {
	all, err := s.store.ListRepos(ctx)
	if err != nil {
		return nil, err
	}

	byURL := make(map[string]*domain.Repo, len(all))
	for _, r := range all {
		if r.URL != "" {
...
func (s *Service) IngestRuntimeServiceGraph(ctx context.Context, edges []RuntimeServiceEdge) (*RuntimeIngestResult, error) {
	svcUnits, err := s.store.GetASTUnits(ctx, domain.QueryOpts{Kind: store.KindService})
	if err != nil {
		return nil, err
	}
	...
	if err := s.store.DeleteEdgesByKind(ctx, "", store.EdgeRuntimeCall); err != nil {
		return nil, err
	}
	stored := 0
	for _, e := range matched {
		...
		edge := &domain.Edge{
			...
			Kind:       store.EdgeRuntimeCall,
			DstName:    "runtime:" + e.Server,
			FilePath:   client.FilePath,
			Confidence: 1.0,
			Meta:       fmt.Sprintf(`{"calls":%d}`, e.Calls),
		}
		if err := s.store.StoreEdge(ctx, edge); err != nil {
			return nil, err
		}
		stored++
	}
