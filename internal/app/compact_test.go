...
func newCompactable(typ index.IndexType) *compactableIndexer {
...
// internal/index/bm25 (Compact) for what the layout does to a score.
func TestDoIndexCompacts(t *testing.T) {
	...
	keyword := newCompactable(index.IndexTypeBM25)
	plain := &mockIndexer{name: "vector", indexType: index.IndexTypeVector}
	svc := newLocalTestService(t, &mockStorage{}, map[index.IndexType]index.Indexer{
		index.IndexTypeBM25:   keyword,
		index.IndexTypeVector: plain,
	})
...
func TestDoIndexSkipsCompactionWhenNothingWasIndexed(t *testing.T) {
	...
	repo := &domain.Repo{ID: "r1", Name: "t", Source: domain.SourceTypeLocal, Path: t.TempDir()}
...
func TestCompactIndexesSurvivesAFailure(t *testing.T) {
	broken := newCompactable(index.IndexTypeBM25)
	broken.err = errors.New("force merge already in progress")
	other := newCompactable(index.IndexTypeAST)
	svc := newTestService(&mockStorage{}, map[index.IndexType]index.Indexer{
		index.IndexTypeBM25: broken,
		index.IndexTypeAST:  other,
	})
