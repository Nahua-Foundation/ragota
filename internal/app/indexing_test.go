...
// a real repository.
func newLocalTestService(t *testing.T, st store.Storage, indexers map[index.IndexType]index.Indexer) *Service {
	t.Helper()
	src := local.New()
...
func TestScanFileClassifiesUnreadablePaths(t *testing.T) {
	...
	repo := &domain.Repo{ID: "r1", Path: dir}
	for _, path := range paths {
		t.Run(path, func(t *testing.T) {
			got := svc.scanFile(repo, &domain.RepoFile{Path: path, Language: "go"}, nil)
...
func TestScanFileUnknownPathIsNotAFailure(t *testing.T) {
	...
	svc := newTestService(&mockStorage{}, nil)
	known := map[string]*store.File{"other.go": {Path: "other.go", Indexed: true}}
	got := svc.scanFile(&domain.Repo{ID: "r1", Path: dir}, &domain.RepoFile{Path: "good.go", Language: "go"}, known)
...
func TestScanFileSkipsUnchangedIndexedFile(t *testing.T) {
	...
	svc := newTestService(&mockStorage{}, nil)
	f := &domain.RepoFile{Path: "good.go", Language: "go"}
	repo := &domain.Repo{ID: "r1", Path: dir}

	indexed := map[string]*store.File{"good.go": {Path: "good.go", Hash: hash, Indexed: true}}
	if got := svc.scanFile(repo, f, indexed); !got.skipped {
		t.Errorf("scanFile() = %+v, want the unchanged file skipped", got)
	}
	stale := map[string]*store.File{"good.go": {Path: "good.go", Hash: "other", Indexed: true}}
	if got := svc.scanFile(repo, f, stale); got.skipped {
		t.Error("scanFile() skipped a file whose stored hash differs")
	}
	unfinished := map[string]*store.File{"good.go": {Path: "good.go", Hash: hash}}
