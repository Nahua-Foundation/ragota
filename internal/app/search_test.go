...
type stubSearcher struct {
	mu      sync.Mutex
	hits    []*index.Hit
	got     *index.SearchQuery
	byQuery map[string][]*index.Hit
}

func (s *stubSearcher) Search(ctx context.Context, q *index.SearchQuery) (*index.SearchResult, error) {
	s.mu.Lock()
	s.got = q
	s.mu.Unlock()
	hits := s.hits
...
func serviceWithSearcher(srch index.Searcher, st store.Storage) *Service {
	if st == nil {
		st = &mockStorage{}
	}
	return New(nil, st, nil, nil, search.New(map[index.IndexType]index.Searcher{
		index.IndexTypeBM25:   srch,
		index.IndexTypeVector: srch,
	}, nil))
}

func hitsForFiles(paths ...string) []*index.Hit {
	hits := make([]*index.Hit, len(paths))
	for i, p := range paths {
		hits[i] = &index.Hit{RepoID: "r1", FilePath: p, Path: p, Line: 1, EndLine: 10, Reason: "keyword"}
	}
	return hits
}
...
func TestSearchValidatesLimit(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			q := &index.SearchQuery{Query: "add", Limit: tt.limit}
...
func TestSearchRejectsUnknownMode(t *testing.T) {
	...
	for _, mode := range []string{"sematic", "SEMANTIC", "fuzzy", "bm25"} {
		_, err := svc.Search(context.Background(), &index.SearchQuery{Query: "add"}, mode)
	...
	for _, mode := range []string{"", SearchModeSemantic, SearchModeKeyword, SearchModeHybrid} {
		if _, err := svc.Search(context.Background(), &index.SearchQuery{Query: "add"}, mode); err != nil {
...
func TestSearchRejectsEmptyQuery(t *testing.T) {
	...
	for _, query := range []string{"", "   "} {
		_, err := svc.Search(context.Background(), &index.SearchQuery{Query: query}, "keyword")
		if !errors.Is(err, ErrBadRequest) {
	...
	if _, err := svc.Search(context.Background(), &index.SearchQuery{Vector: []float32{0.1}}, "semantic"); err != nil {
...
func (u *unitStorage) GetASTUnits(context.Context, domain.QueryOpts) ([]*domain.ASTUnit, error) {
	return u.units, nil
}
...
func TestBuildContextFillsLimitWithDistinctUnits(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		hits = append(hits,
			&index.Hit{RepoID: "r1", FilePath: path, Line: 1, EndLine: 20},
			&index.Hit{RepoID: "r1", FilePath: path, Line: 21, EndLine: 40},
		)
	}
	svc := serviceWithSearcher(&stubSearcher{hits: hits}, nil)
	defer svc.Close(context.Background())
...
func TestSearchRejectsUnknownIntent(t *testing.T) {
	...
	_, err := svc.Search(context.Background(),
		&index.SearchQuery{Query: "anything", Intent: "sideways"}, SearchModeKeyword)
	if !errors.Is(err, ErrBadRequest) {
