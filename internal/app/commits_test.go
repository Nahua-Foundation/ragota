...
func commitTestService(t *testing.T, st *mockStorage, ignore []string) (*Service, *domain.Repo) {
	t.Helper()
	dir := t.TempDir()
	...
	svc.sources = map[domain.SourceType]repos.RepoSource{
		domain.SourceTypeLocal: &mockSource{name: domain.SourceTypeLocal},
	}
	return svc, &domain.Repo{ID: "repo-1", Name: "test", Source: domain.SourceTypeLocal, Path: dir}
}
...
func TestRunApplyCommitsDoesNotAdvanceCursorOnPartialBatch(t *testing.T) {
	...
	if len(statuses) != 1 || statuses[0].Status != domain.StatusError {
...
func TestRunApplyCommitsAdvancesCursorOnCleanBatch(t *testing.T) {
	...
	if len(statuses) != 1 || statuses[0].Status != domain.StatusIdle {
...
func TestApplyCommitsRecordsPendingCommit(t *testing.T) {
	st := &mockStorage{repo: &domain.Repo{ID: "repo-1", Name: "test", Source: domain.SourceTypeLocal, Path: t.TempDir()}}
	svc := newTestService(st, map[index.IndexType]index.Indexer{})
	svc.sources = map[domain.SourceType]repos.RepoSource{
		domain.SourceTypeLocal: &mockSource{name: domain.SourceTypeLocal},
	}
...
func TestApplyCommitsContinuingInFlightBatchIsBusyNotGap(t *testing.T) {
	st := &mockStorage{repo: &domain.Repo{
		ID: "repo-1", Name: "test", Source: domain.SourceTypeLocal, Path: t.TempDir(),
		Status: domain.StatusIndexing, LastCommit: "sha-1", PendingCommit: "sha-2",
	}}
...
func distributedCommitService(t *testing.T, st *mockStorage) *Service {
	...
	dir := t.TempDir()
	if st.repo == nil {
		st.repo = &domain.Repo{ID: "repo-1", Name: "test", Source: domain.SourceTypeLocal, Path: dir}
...
func TestApplyCommitsDistributedChecksGapBeforeQueueing(t *testing.T) {
	st := &mockStorage{repo: &domain.Repo{
		ID: "repo-1", Name: "test", Source: domain.SourceTypeLocal, LastCommit: "sha-1",
	}}
	svc := distributedCommitService(t, st)
...
func TestRunQueuedJobAppliesCommitBatch(t *testing.T) {
	...
	job := &domain.IndexJob{ID: "job-7", RepoID: "repo-1", Kind: domain.JobKindCommits,
		Status: domain.JobStatusRunning, Payload: payload}
...
func TestRunQueuedJobWaitsForPrecedingBatch(t *testing.T) {
	st := &mockStorage{
		repo:             &domain.Repo{ID: "repo-1", Name: "test", Source: domain.SourceTypeLocal, LastCommit: "sha-1"},
	...
	svc.runQueuedJob(context.Background(), &domain.IndexJob{
		ID: "job-8", RepoID: "repo-1", Kind: domain.JobKindCommits,
		Status: domain.JobStatusRunning, Payload: payload,
	}, "worker-1", time.Hour)
...
func TestRunQueuedJobFailsWhenTheChainIsBroken(t *testing.T) {
	...
	svc.runQueuedJob(context.Background(), &domain.IndexJob{
		ID: "job-9", RepoID: "repo-1", Kind: domain.JobKindCommits,
		Status: domain.JobStatusRunning, Payload: payload,
	}, "worker-1", time.Hour)
...
func TestRunQueuedJobSkipsAlreadyAppliedBatch(t *testing.T) {
	st := &mockStorage{repo: &domain.Repo{
		ID: "repo-1", Name: "test", Source: domain.SourceTypeLocal, LastCommit: "sha-2",
	}}
	svc := distributedCommitService(t, st)
	...
	svc.runQueuedJob(context.Background(), &domain.IndexJob{
		ID: "job-10", RepoID: "repo-1", Kind: domain.JobKindCommits,
		Status: domain.JobStatusRunning, Payload: payload,
	}, "worker-1", time.Hour)
...
func TestRunQueuedJobRejectsUndecodablePayload(t *testing.T) {
	...
	svc.runQueuedJob(context.Background(), &domain.IndexJob{
		ID: "job-11", RepoID: "repo-1", Kind: domain.JobKindCommits,
		Status: domain.JobStatusRunning, Payload: "{not json",
	}, "worker-1", time.Hour)
...
func TestCommitJobOrder(t *testing.T) {
	...
	cases := []struct {
		...
		{"no cursor yet", &domain.Repo{}, commitBatchNext},
		{"continues the cursor", &domain.Repo{LastCommit: "sha-1"}, commitBatchNext},
		{"cursor already past it", &domain.Repo{LastCommit: "sha-2"}, commitBatchApplied},
		{"predecessor missing", &domain.Repo{LastCommit: "sha-0"}, commitBatchTooEarly},
	}
