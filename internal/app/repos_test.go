...
func TestResolveRepoRef(t *testing.T) {
	...
	// An unknown reference is store.ErrNotFound, so callers can decorate it
	// (the CLI appends how to see what is registered) without string-matching.
	if _, err := resolveRepoRef(list, "gamma"); !errors.Is(err, store.ErrNotFound) {
		t.Errorf("unknown repo error = %v, want errors.Is(..., store.ErrNotFound)", err)
	}
}
