...
func newTestService(st store.Storage, indexers map[index.IndexType]index.Indexer) *Service {
...
func TestIndexFileSetSkipsFailedFiles(t *testing.T) {
	st := &mockStorage{}
	idx := &mockIndexer{
		name: "vector", indexType: index.IndexTypeVector,
		indexFunc: func(req *index.IndexRequest) (*index.IndexResult, error) {
	...
	svc := newTestService(st, map[index.IndexType]index.Indexer{index.IndexTypeVector: idx})
	...
	failed, err := svc.indexFileSet(context.Background(), &domain.Repo{ID: "r1"}, toIndex, processed, false, nil, nil)
	if err != nil {
...
func TestIndexFileSetUnattributedFailureFailsBatch(t *testing.T) {
	...
	idx := &mockIndexer{
		...
		indexFunc: func(*index.IndexRequest) (*index.IndexResult, error) {
			return &index.IndexResult{FilesFailed: 1, Errors: []string{"batch upsert rejected"}}, nil
		},
	}
	svc := newTestService(st, map[index.IndexType]index.Indexer{index.IndexTypeVector: idx})

	toIndex := []*index.FileToIndex{{Path: "a.go"}, {Path: "b.go"}}
	processed := []*store.File{{RepoID: "r1", Path: "a.go"}, {RepoID: "r1", Path: "b.go"}}
...
func TestIndexFileSetSuccessStoresEverything(t *testing.T) {
	st := &mockStorage{}
	idx := &mockIndexer{name: "ast", indexType: index.IndexTypeAST}
	svc := newTestService(st, map[index.IndexType]index.Indexer{index.IndexTypeAST: idx})
...
func TestAttributeFailures(t *testing.T) {
	batch := []*index.FileToIndex{{Path: "a/b.go"}, {Path: "c.go"}}
...
func TestAddRepoPreservesExistingState(t *testing.T) {
	existing := &domain.Repo{
		ID: "repo-1", Name: "test", Source: domain.SourceTypeLocal,
		Status: domain.StatusIndexing, LastCommit: "sha-1", IndexedAt: 111, CreatedAt: 5,
	}
	st := &mockStorage{repo: existing}
	src := &mockSource{
		name: domain.SourceTypeLocal,
		addFunc: func(context.Context, *domain.AddRequest) (*domain.Repo, error) {
			return &domain.Repo{ID: "repo-1", Name: "test", Source: domain.SourceTypeLocal, Path: "/tmp/a"}, nil
		},
	}
	svc := newTestService(st, nil)
	svc.sources = map[domain.SourceType]repos.RepoSource{domain.SourceTypeLocal: src}

	got, err := svc.AddRepo(context.Background(), domain.SourceTypeLocal, &domain.AddRequest{Name: "test", Path: "/tmp/a"})
	if err != nil {
...
func TestAddRepoNewRepoStartsIdle(t *testing.T) {
	st := &mockStorage{getRepoErr: store.ErrNotFound}
...
func TestStartIndexDistributedReportsJob(t *testing.T) {
	...
	if ack.JobID != "job-1" || ack.JobStatus != domain.JobStatusPending {
...
func TestStartIndexSingleInstanceReportsIndexing(t *testing.T) {
	st := &mockStorage{}
	svc := newTestService(st, map[index.IndexType]index.Indexer{})
...
func TestFindRepoByHintsRejectsPartialMatches(t *testing.T) {
	all := []*domain.Repo{{ID: "r1", Name: "gateway", URL: "https://github.com/acme/gateway.git"}}
	svc := newTestService(&mockStorage{repoList: all}, nil)

	for _, hint := range []string{"gate", "gateway-v2", "https://github.com/other/gateway-v2.git"} {
		if _, err := svc.FindRepoByHints(context.Background(), []string{hint}); !errors.Is(err, store.ErrNotFound) {
...
func TestRepoJobsCapsTheLimit(t *testing.T) {
	st := &mockStorage{jobList: []*domain.IndexJob{{ID: "1", RepoID: "repo-1"}}}
	svc := newTestService(st, nil)
...
func TestRepoJobsUnknownRepoIsNotFound(t *testing.T) {
	...
	if _, err := svc.RepoJobs(context.Background(), "gone", 0); !errors.Is(err, store.ErrNotFound) {
...
func TestRepoJobRejectsForeignJob(t *testing.T) {
	st := &mockStorage{job: &domain.IndexJob{ID: "5", RepoID: "repo-2", Kind: domain.JobKindIndex}}
	svc := newTestService(st, nil)

	if _, err := svc.RepoJob(context.Background(), "repo-1", "5"); !errors.Is(err, store.ErrNotFound) {
...
func TestIndexFileSetRunsIndexersConcurrently(t *testing.T) {
	...
	indexers := make(map[index.IndexType]index.Indexer, n)
	for _, typ := range []index.IndexType{index.IndexTypeAST, index.IndexTypeBM25, index.IndexTypeVector} {
	...
	toIndex := []*index.FileToIndex{{Path: "a.go"}}
	processed := []*store.File{{RepoID: "r1", Path: "a.go"}}

	done := make(chan error, 1)
	go func() {
		_, err := svc.indexFileSet(context.Background(), &domain.Repo{ID: "r1"}, toIndex, processed, false, nil, nil)
		done <- err
	}()
...
func TestIndexFileSetGivesEachIndexerItsOwnFiles(t *testing.T) {
	svc := newTestService(&mockStorage{}, map[index.IndexType]index.Indexer{
		index.IndexTypeAST:    &mockIndexer{name: "ast", indexType: index.IndexTypeAST},
		index.IndexTypeBM25:   &mockIndexer{name: "bm25", indexType: index.IndexTypeBM25},
		index.IndexTypeVector: &mockIndexer{name: "vector", indexType: index.IndexTypeVector},
	})

	files := []*index.FileToIndex{{Path: "a.go", Language: "go"}}
	runs := withFiles(svc.indexerRuns(), files)
	if len(runs) != 3 {
		t.Fatalf("runs = %d, want 3", len(runs))
	}
	if want := []index.IndexType{index.IndexTypeAST, index.IndexTypeBM25, index.IndexTypeVector}; runs[0].typ != want[0] || runs[1].typ != want[1] || runs[2].typ != want[2] {
		t.Errorf("run order = %v %v %v, want %v", runs[0].typ, runs[1].typ, runs[2].typ, want)
	}
	seen := map[*index.FileToIndex]bool{}
	for _, r := range runs {
...
func TestIndexerTimesLog(t *testing.T) {
	spent := indexerTimes{}
	spent.add(index.IndexTypeBM25, 63*time.Second)
	spent.add(index.IndexTypeAST, 20*time.Second)
	spent.add(index.IndexTypeAST, 21*time.Second)
	...
	indexerTimes(nil).add(index.IndexTypeAST, time.Second)
}
