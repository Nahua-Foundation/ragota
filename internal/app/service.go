// Package app provides the core application service layer.
package app
...
// Service is the core application app.
...
type Service struct {
	...
	// is a *progress.Bus rather than an interface so that the nil case stays a
...
func New(
	...
	// Distributed indexing worker. Started here (rather than from bootstrap.Build)
...
func (s *Service) Search(ctx context.Context, q *index.SearchQuery, mode string) (*index.SearchResult, error) {
	if q == nil {
	...
	if none {
		return &index.SearchResult{
			Hits: []*index.Hit{}, Query: q.Query, Mode: mode,
	...
	case SearchModeSemantic:
		hits, meta, err = s.searchSvc.CandidatesFrom(ctx, index.IndexTypeVector, exec)
	case SearchModeKeyword:
		hits, meta, err = s.searchSvc.CandidatesFrom(ctx, index.IndexTypeBM25, exec)
...
func (s *Service) Stats(ctx context.Context) (map[string]*index.IndexerStats, error) {
	stats := make(map[string]*index.IndexerStats)
...
// The enrichment setters are delegated so that wiring (bootstrap.Build) keeps
...
func (s *Service) SetStatusBus(b *progress.Bus) { s.bus = b }
