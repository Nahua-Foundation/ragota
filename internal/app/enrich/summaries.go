...
func (e *Enricher) SummarizeRepo(ctx context.Context, repo *domain.Repo, files []*index.FileToIndex) error {
	if e.generator == nil || !e.fileSummaries || len(files) == 0 {
		return nil
	}

	svcs, _ := repos.DetectServices(repo.Path, repo.Name)

	count := 0
	var errs []error
	var summaryDocs []*index.FileToIndex
	for _, f := range files {
		...
		unit := &domain.ASTUnit{
			...
			Meta:      store.EncodeUnitMeta(&store.UnitMeta{Scope: "file"}),
			StartLine: 1,
			EndLine:   1,
			Hash:      f.Hash,
		}
...
func summaryDoc(scope, name, text string) *index.FileToIndex {
	content := "summary: " + scope + " " + name + "\n\n" + text + "\n"
	return &index.FileToIndex{
...
// is configured.
func (e *Enricher) indexSummaries(ctx context.Context, repo *domain.Repo, docs []*index.FileToIndex) error {
	if len(docs) == 0 {
		return nil
	}
...
// summarizeServices rolls file summaries up into one summary per app.
// It returns the generated service summaries as synthetic documents for the
// vector index.
func (e *Enricher) summarizeServices(ctx context.Context, repo *domain.Repo, svcs []repos.Service) ([]*index.FileToIndex, error) {
	fileSums, err := e.store.GetASTUnits(ctx, domain.QueryOpts{RepoID: repo.ID, Kind: store.KindSummary})
	if err != nil {
		return nil, err
	}
	...
	for _, u := range fileSums {
		...
		svcName := repos.ServiceFor(svcs, u.FilePath)
		if svcName == "" {
			svcName = repo.Name
		}
	...
	for svcName, sums := range byService {
		...
		if derr := e.store.DeleteASTUnitsByFile(ctx, repo.ID, pseudoPath); derr != nil {
		...
		unit := &domain.ASTUnit{
			...
			Doc:       strings.TrimSpace(text),
			Meta:      store.EncodeUnitMeta(&store.UnitMeta{Scope: "service"}),
			StartLine: 1,
			EndLine:   1,
		}
		if serr := e.store.StoreASTUnit(ctx, unit); serr != nil {
