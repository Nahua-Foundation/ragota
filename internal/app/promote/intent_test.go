...
func TestResolveIntent(t *testing.T) {
	...
	if _, _, err := p.ResolveIntent(&index.SearchQuery{Query: "x", Intent: "sideways"}); err == nil {
...
func TestResolveIntentConfigOff(t *testing.T) {
	...
	kind, _, err := p.ResolveIntent(&index.SearchQuery{Query: "who calls ParseFilters"})
	if err != nil || kind != "" {
		t.Fatalf("off/auto = (%q, %v), want no intent", kind, err)
	}
	// ...but an explicit request is still honoured.
	kind, callee, err := p.ResolveIntent(&index.SearchQuery{Query: "who calls ParseFilters", Intent: IntentCallers})
	if err != nil || kind != IntentCallers || callee != "ParseFilters" {
