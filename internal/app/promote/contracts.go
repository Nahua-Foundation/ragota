...
var contractEdgeRank = map[string]int{
	store.EdgeHandledBy:     0, // route -> handler: the server side
	store.EdgeImplementsRPC: 0,
	store.EdgeProduces:      1,
	store.EdgeWritesTo:      1,
	store.EdgeConsumes:      2,
	store.EdgeReadsFrom:     2,
}
...
var contractClientEdges = map[string]bool{
	store.EdgeHTTPCall: true,
	store.EdgeRPCCall:  true,
}

// contractEdgeVerb phrases a hit's Reason per edge kind.
var contractEdgeVerb = map[string]string{
	store.EdgeHandledBy:     "handles",
	store.EdgeImplementsRPC: "implements",
...
// callers answer.
func (p *Promoter) PromoteRPCImplementations(ctx context.Context, q *index.SearchQuery,
	hits []*index.Hit, meta map[string]interface{}) []*index.Hit {

	if p.store == nil || !rpcMentionRe.MatchString(q.Query) {
		return hits
	}
	...
	for _, repoID := range repos {
		edges, err := p.store.GetEdges(ctx, domain.QueryOpts{
			RepoID: repoID, Kind: store.EdgeImplementsRPC, Limit: rpcImplScanLimit,
		})
		if err != nil {
	...
	promoted := make([]*index.Hit, 0, len(matched))
	seen := map[string]bool{}
	for _, e := range matched {
...
func (p *Promoter) PromoteContracts(ctx context.Context, q *index.SearchQuery,
	hits []*index.Hit, meta map[string]interface{}) []*index.Hit {
...
func (p *Promoter) contractHits(ctx context.Context, q *index.SearchQuery, ref contractRef) []*index.Hit {
	units := p.unitsByKey(ctx, q, ref.key)

	var out []*index.Hit
	seen := map[string]bool{}
	push := func(h *index.Hit) {
	...
	edges, err := p.store.GetEdges(ctx, domain.QueryOpts{Name: ref.key, Limit: contractEdgeLimit})
	if err != nil {
	...
	for _, e := range edges {
		switch {
		case e == nil || e.Kind == store.EdgeCall:
...
func (p *Promoter) unitsByKey(ctx context.Context, q *index.SearchQuery, key string) []*domain.ASTUnit {
	opts := domain.QueryOpts{Qualified: key, Limit: 5}
	if len(q.Repos) == 1 {
	...
	if len(units) == 0 {
		...
		if units, err = p.store.GetASTUnits(ctx, opts); err != nil {
			return nil
		}
	}
	out := units[:0]
...
// serve it.
func (p *Promoter) handlersOf(ctx context.Context, route *domain.ASTUnit) []*index.Hit {
	edges, err := p.store.GetEdges(ctx, domain.QueryOpts{
		SrcID: route.ID, Kind: store.EdgeHandledBy, Limit: 10,
	})
...
func sortContractEdges(edges []*domain.Edge) {
...
func unitHit(u *domain.ASTUnit, reason string) *index.Hit {
	return &index.Hit{
		RepoID:   u.RepoID,
		FilePath: u.FilePath,
		Path:     u.FilePath,
...
// contract.
func contractEdgeHit(e *domain.Edge, src *domain.ASTUnit, key, verb string) *index.Hit {
	hit := &index.Hit{
		RepoID:   e.RepoID,
		FilePath: e.FilePath,
		Path:     e.FilePath,
...
// already knows.
func unitSnippet(u *domain.ASTUnit, reason string) string {
	var b strings.Builder
	writeUnitHead(&b, u)
	if reason != "" {
		b.WriteByte('\n')
...
func writeUnitHead(b *strings.Builder, u *domain.ASTUnit) {
	if u.Kind != "" {
