...
func contractFixture(t *testing.T) *sqlite.SQLite {
	...
	for _, u := range []*domain.ASTUnit{route, handler, table, writer, producer} {
	...
	for _, e := range []*domain.Edge{
		{RepoID: "r1", SrcID: route.ID, DstID: handler.ID, Kind: store.EdgeHandledBy,
			DstName: "CancelOrder", FilePath: "api/routes.go", Line: 12, Confidence: 0.95},
		{RepoID: "r1", SrcID: writer.ID, Kind: store.EdgeWritesTo,
			DstName: "db:orders", FilePath: "store/orders.go", Line: 42, Confidence: 0.9},
		{RepoID: "r1", SrcID: handler.ID, Kind: store.EdgeReadsFrom,
			DstName: "db:orders", FilePath: "api/orders.go", Line: 95, Confidence: 0.9},
		{RepoID: "r1", SrcID: producer.ID, Kind: store.EdgeProduces,
...
func TestSearchPromotesRouteHandler(t *testing.T) {
	...
	srch := &stubSearcher{hits: []*index.Hit{{
...
func TestSearchPromotesRPCImplementation(t *testing.T) {
	...
	for _, u := range []*domain.ASTUnit{impl, stub, testImpl} {
		if err := st.StoreASTUnit(ctx, u); err != nil {
			t.Fatal(err)
		}
	}
	for _, e := range []*domain.Edge{
		{RepoID: "r1", SrcID: impl.ID, Kind: store.EdgeImplementsRPC,
		...
		{RepoID: "r1", SrcID: stub.ID, Kind: store.EdgeImplementsRPC, DstName: "grpc:/ShipOrder",
			FilePath: "src/checkoutservice/genproto/demo_grpc.pb.go", Line: 524, Confidence: 0.6},
		{RepoID: "r1", SrcID: testImpl.ID, Kind: store.EdgeImplementsRPC, DstName: "grpc:ShippingService/ShipOrder",
