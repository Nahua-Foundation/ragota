...
type refStorage struct {
	*mockStorage
	unit       *domain.ASTUnit
	resolved   []*domain.Edge
	unresolved []*domain.Edge
	// limits are the Limit of every GetEdges call, resolved lookup first.
	limits []int
}

func (r *refStorage) GetASTUnits(context.Context, domain.QueryOpts) ([]*domain.ASTUnit, error) {
	if r.unit == nil {
		return nil, nil
	}
	return []*domain.ASTUnit{r.unit}, nil
}

func (r *refStorage) GetEdges(_ context.Context, opts domain.QueryOpts) ([]*domain.Edge, error) {
...
func edges(prefix string, n int) []*domain.Edge {
	out := make([]*domain.Edge, n)
	for i := range out {
		out[i] = &domain.Edge{ID: prefix + strconv.Itoa(i), RepoID: "r1", Kind: "call", DstName: "Handle"}
	}
	return out
}
...
func refService(resolved, unresolved int) (*Service, *refStorage) {
	st := &refStorage{
		...
		unit:        &domain.ASTUnit{ID: "u1", RepoID: "r1", Name: "Handle", StartLine: 10, EndLine: 20},
