...
func TestIngestRuntimeServiceGraphNamesAndDiagnostics(t *testing.T) {
	...
	for _, name := range []string{"checkoutservice", "shippingservice"} {
		if err := st.StoreASTUnit(ctx, &domain.ASTUnit{
			RepoID: "r1", FilePath: "src/" + name, Kind: store.KindService,
	...
	edges, err := st.GetEdges(ctx, domain.QueryOpts{Kind: store.EdgeRuntimeCall})
