...
func DiscoverAndRegister(ctx context.Context, svc *service.Service, root string, bus *status.Bus) ([]*repos.Repo, error) {
	...
	registered := make([]*repos.Repo, 0, len(paths))
	for _, path := range paths {
		repo, err := svc.AddRepo(ctx, repos.SourceTypeLocal, &repos.AddRequest{Path: path})
		if err != nil {
...
func IndexAll(ctx context.Context, svc *service.Service, list []*repos.Repo) {
