...
// TreeSitterServer — MCP-сервер для AST/tree-sitter индекса.
// Tools:
...
//   - ts.stats()
//
// Все операции чтения идут через astindex.Indexer (ast_units + edges),
// таблица symbols (legacy treesitter) не используется.
type TreeSitterServer struct {
	cfg *config.Config
	idx *astindex.Indexer
	st  *store.SQLite
	bus *state.Bus
}

// NewTreeSitterServer создаёт сервер.
func NewTreeSitterServer(cfg *config.Config, idx *astindex.Indexer, st *store.SQLite, bus *state.Bus) *TreeSitterServer {
...
func (s *TreeSitterServer) handleReindex(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	if s.idx == nil {
		return mcp.NewToolResultError("tree-sitter index is not initialized"), nil
	}
...
func (s *TreeSitterServer) handleStats(ctx context.Context, _ mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	return jsonResult(map[string]any{"units": st.Units, "edges": st.Edges})
}
