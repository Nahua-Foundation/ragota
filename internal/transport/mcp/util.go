...
// parseCSVParam парсит comma-separated строку в []string.
...
// wrapToolHandler — общая обёртка для MCP tool-хендлеров.
// Считает вызовы и ошибки в state.Bus, конвертирует ошибки в MCP error result.
func wrapToolHandler(subsystem string, bus *state.Bus, fn func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error)) server.ToolHandlerFunc {
	return func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if bus != nil {
			bus.IncMCPCall(subsystem, name, false)
		}
		return res, nil
	}
}
