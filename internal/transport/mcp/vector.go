...
func NewVectorServer(cfg *config.Config, idx *vector.Vector, qd *qdrant.Client, bus *state.Bus) *VectorServer {
	return &VectorServer{cfg: cfg, idx: idx, qd: qd, bus: bus}
}

// SetBM25 подключает поисковый индекс (используется в search_keyword/search_hybrid).
func (s *VectorServer) SetBM25(b vector.WriteSink) { s.bm25 = b }
...
func (s *VectorServer) handleSearchHybrid(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	vecRet := &hybrid.VectorHybridAdapter{V: s.idx}
