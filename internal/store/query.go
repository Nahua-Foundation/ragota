// SQL query-building helpers shared by the SQL backends (sqlite, postgres):
// the WHERE-filter logic for AST unit and edge queries lives here so the
// backends cannot silently drift apart. Only placeholder syntax differs between
// them, expressed via Dialect.
package store
...
func UnitFilters(b *Builder, opts domain.QueryOpts, tier UnitTier) {
	if opts.RepoID != "" {
...
func unitTerms(b *Builder, opts domain.QueryOpts, tier UnitTier) {
	if opts.Name != "" {
...
// against.
func unitTermConds(b *Builder, opts domain.QueryOpts, tier UnitTier) []string {
	start := len(b.conds)
	unitTerms(b, opts, tier)
...
func buildPenaltyPath() string {
	markers := append(domain.GeneratedPathMarkers(), domain.TestPathMarkers()...)
...
func unitOrderBy(b *Builder, opts domain.QueryOpts, tier UnitTier) string {
...
func UnitQuery(d Dialect, opts domain.QueryOpts, tier UnitTier, limit int, exclude []any) (string, []any) {
	b := NewBuilder(d)
...
type UnitScanner func(query string, args []any) ([]*domain.ASTUnit, error)
...
func QueryUnits(d Dialect, opts domain.QueryOpts, idConv func(string) any, scan UnitScanner) ([]*domain.ASTUnit, error) {
...
func hasUnitTerm(opts domain.QueryOpts) bool {
...
func EdgeFilters(b *Builder, opts domain.QueryOpts, idConv func(string) any) {
	if opts.RepoID != "" {
		b.Eq("repo_id", opts.RepoID)
	}
	if opts.FilePath != "" {
