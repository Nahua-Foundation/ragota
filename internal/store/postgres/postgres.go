// Package postgres implements store.Storage on PostgreSQL. It is the
// primary relational backend for ragota: static queries are compiled by sqlc
// into this package and executed over native pgx (pgxpool); dynamic filtered
// queries (QueryOpts) are built with the shared query builder in store.
// Schema and semantics mirror the SQLite backend, which remains a lightweight
// embedded/dev option.
package postgres
...
// Postgres implements store.Storage.
...
var _ store.Storage = (*Postgres)(nil)

type Postgres struct {
	pool        *pgxpool.Pool
	q           *Queries
	vectorStore store.VectorStorage
}
...
func Open(cfg *Config) (*Postgres, error) {
	...
	p := &Postgres{pool: pool, q: New(pool)}
...
func (p *Postgres) SetVectorStore(vs store.VectorStorage) { p.vectorStore = vs }
...
func (p *Postgres) VectorStore() store.VectorStorage { return p.vectorStore }

// intOrZero is store.IntOrZero under its local name.
...
func (p *Postgres) StoreFile(ctx context.Context, f *store.File) error {
	return p.q.StoreFile(ctx, StoreFileParams{
		RepoID:   f.RepoID,
		Path:     f.Path,
		Language: f.Language,
		Hash:     f.Hash,
...
func (p *Postgres) BatchStoreFiles(ctx context.Context, files []*store.File) error {
	if len(files) == 0 {
		return nil
	}
	batch := &pgx.Batch{}
...
func (p *Postgres) GetFile(ctx context.Context, repoID, path string) (*store.File, error) {
	r, err := p.q.GetFile(ctx, GetFileParams{RepoID: repoID, Path: path})
	if err != nil {
...
func (p *Postgres) GetFilesByRepo(ctx context.Context, repoID string) ([]*store.File, error) {
...
func (p *Postgres) DeleteFile(ctx context.Context, repoID, path string) error {
	return p.q.DeleteFile(ctx, DeleteFileParams{RepoID: repoID, Path: path})
}
...
func (p *Postgres) StoreASTUnit(ctx context.Context, u *domain.ASTUnit) error {
...
func (p *Postgres) BatchStoreASTUnits(ctx context.Context, units []*domain.ASTUnit) error {
	if len(units) == 0 {
		return nil
	}
	tx, err := p.pool.Begin(ctx)
	...
	params := make([]BatchInsertASTUnitsParams, len(units))
	for i, u := range units {
		params[i] = BatchInsertASTUnitsParams(unitInsertParams(u))
	}
	var batchErr error
...
// SQLite backend: both go through store.QueryUnits, and only running the statement is
// this backend's own.
func (p *Postgres) GetASTUnits(ctx context.Context, opts domain.QueryOpts) ([]*domain.ASTUnit, error) {
	return store.QueryUnits(store.PostgresDialect{}, opts,
...
func (p *Postgres) GetASTUnitByID(ctx context.Context, id string) (*domain.ASTUnit, error) {
...
func (p *Postgres) GetASTUnitsByIDs(ctx context.Context, ids []string) ([]*domain.ASTUnit, error) {
	if len(ids) == 0 {
		return nil, nil
	}
...
func (p *Postgres) DeleteASTUnitsByFile(ctx context.Context, repoID, filePath string) error {
	...
	q := p.q.WithTx(tx)
	if _, err := q.UnresolveEdgesByDstFile(ctx, UnresolveEdgesByDstFileParams{
		RepoID: repoID, FilePath: filePath,
	}); err != nil {
		return fmt.Errorf("unresolve edges into file: %w", err)
	}
	if err := q.DeleteASTUnitsByFile(ctx, DeleteASTUnitsByFileParams{
		RepoID: repoID, FilePath: filePath,
	}); err != nil {
		return fmt.Errorf("delete ast units by file: %w", err)
	}
...
func (p *Postgres) DeleteASTUnitsByFiles(ctx context.Context, repoID string, paths []string) error {
	...
	q := p.q.WithTx(tx)
	if _, err := q.UnresolveEdgesByDstFiles(ctx, UnresolveEdgesByDstFilesParams{
		RepoID: repoID, Paths: paths,
	}); err != nil {
		return fmt.Errorf("unresolve edges into files: %w", err)
	}
	if err := q.DeleteASTUnitsByFiles(ctx, DeleteASTUnitsByFilesParams{
		RepoID: repoID, Paths: paths,
	}); err != nil {
		return fmt.Errorf("delete ast units by files: %w", err)
	}
...
func (p *Postgres) DeleteASTUnitsByKind(ctx context.Context, repoID, kind string) error {
	return p.q.DeleteASTUnitsByKind(ctx, DeleteASTUnitsByKindParams{RepoID: repoID, Kind: kind})
}
...
func (p *Postgres) StoreEdge(ctx context.Context, e *domain.Edge) error {
...
func (p *Postgres) BatchStoreEdges(ctx context.Context, edges []*domain.Edge) error {
	if len(edges) == 0 {
		return nil
	}
	tx, err := p.pool.Begin(ctx)
	...
	params := make([]BatchInsertEdgesParams, len(edges))
	for i, e := range edges {
		params[i] = BatchInsertEdgesParams(edgeInsertParams(e))
	}
	var batchErr error
...
func (p *Postgres) GetEdges(ctx context.Context, opts domain.QueryOpts) ([]*domain.Edge, error) {
	b := store.NewBuilder(store.PostgresDialect{})
	...
	where, args := b.Where()
	query := "SELECT " + store.EdgeColumns + " FROM edges WHERE TRUE" + where + store.EdgeOrder + limit
...
func (p *Postgres) DeleteEdgesByKindAndDst(ctx context.Context, kind, dstName string) error {
	return p.q.DeleteEdgesByKindAndDst(ctx, DeleteEdgesByKindAndDstParams{Kind: kind, DstName: dstName})
}
...
func (p *Postgres) DeleteEdgesByFile(ctx context.Context, repoID, filePath string) error {
	return p.q.DeleteEdgesByFile(ctx, DeleteEdgesByFileParams{RepoID: repoID, FilePath: filePath})
}
...
func (p *Postgres) DeleteEdgesByFiles(ctx context.Context, repoID string, paths []string) error {
	...
	return p.q.DeleteEdgesByFiles(ctx, DeleteEdgesByFilesParams{RepoID: repoID, Paths: paths})
}
...
func (p *Postgres) DeleteEdgesByKind(ctx context.Context, repoID, kind string) error {
	...
	return p.q.DeleteEdgesByKind(ctx, DeleteEdgesByKindParams{RepoID: repoID, Kind: kind})
}
...
func (p *Postgres) UpdateEdgeResolution(ctx context.Context, edgeID, dstID, dstRepoID string, confidence float32) error {
	n, err := p.q.UpdateEdgeResolution(ctx, UpdateEdgeResolutionParams{
...
func (p *Postgres) BatchUpdateEdgeResolutions(ctx context.Context, res []store.EdgeResolution) ([]store.EdgeResolutionFailure, error) {
...
func (p *Postgres) resolutionBatch(ctx context.Context, offset int, chunk []store.EdgeResolution) ([]store.EdgeResolutionFailure, error) {
	arg := BatchUpdateEdgeResolutionsParams{
...
func (p *Postgres) resolutionRows(ctx context.Context, offset int, chunk []store.EdgeResolution) []store.EdgeResolutionFailure {
	var failures []store.EdgeResolutionFailure
	for i, r := range chunk {
...
func (p *Postgres) UpdateEdgeDstName(ctx context.Context, edgeID, dstName string) error {
	n, err := p.q.UpdateEdgeDstName(ctx, UpdateEdgeDstNameParams{DstName: dstName, ID: intOrZero(edgeID)})
	if err != nil {
		return err
	}
	if n == 0 {
...
func (p *Postgres) StoreRepo(ctx context.Context, r *domain.Repo) error {
	return p.q.StoreRepo(ctx, StoreRepoParams{
		ID:         r.ID,
		Name:       r.Name,
...
func (p *Postgres) GetRepo(ctx context.Context, id string) (*domain.Repo, error) {
...
func (p *Postgres) ListRepos(ctx context.Context) ([]*domain.Repo, error) {
...
func (p *Postgres) ListActiveRepos(ctx context.Context) ([]*domain.Repo, error) {
...
func (p *Postgres) ClaimRepoForIndexing(ctx context.Context, id, owner string, ttlSeconds int64) (bool, error) {
	...
	now := time.Now().Unix()
	n, err := p.q.ClaimRepoForIndexing(ctx, ClaimRepoForIndexingParams{
...
func (p *Postgres) ResetStuckRepos(ctx context.Context, force bool) (int, error) {
	...
	var err error
	if force {
		n, err = p.q.ResetIndexingRepos(ctx, store.RepoResetMessage)
	} else {
		n, err = p.q.ResetExpiredIndexingRepos(ctx, ResetExpiredIndexingReposParams{
...
func (p *Postgres) SetRepoPendingCommit(ctx context.Context, id, sha string) error {
	n, err := p.q.SetRepoPendingCommit(ctx, SetRepoPendingCommitParams{PendingCommit: sha, ID: id})
	if err != nil {
...
func (p *Postgres) UpdateRepoStatus(ctx context.Context, id string, status domain.Status, lastError string, indexedAt int64) error {
	n, err := p.q.UpdateRepoStatus(ctx, UpdateRepoStatusParams{
...
func (p *Postgres) UpdateRepoLastCommit(ctx context.Context, id, sha string) error {
	n, err := p.q.UpdateRepoLastCommit(ctx, UpdateRepoLastCommitParams{LastCommit: sha, ID: id})
	if err != nil {
		return err
	}
	if n == 0 {
...
// Compile-time check.
var (
	_ store.Storage               = (*Postgres)(nil)
	_ store.EdgeResolutionBatcher = (*Postgres)(nil)
)
