...
//
//	RAGOTA_TEST_POSTGRES_DSN=postgres://postgres:postgres@localhost:5432/ragota_test go test ./internal/store/postgres/
...
func TestPostgresBatchAndClaim(t *testing.T) {
	...
	meta := store.EncodeUnitMeta(&store.UnitMeta{Root: "svc", DetectedBy: "go.mod"})
	...
	edges := []*domain.Edge{
		{RepoID: repoID, SrcID: units[0].ID, Kind: store.EdgeCall, DstName: "Bar", FilePath: "a.go", Line: 1},
		{RepoID: repoID, SrcID: units[1].ID, Kind: store.EdgeCall, DstName: "Baz", FilePath: "a.go", Line: 2},
		{RepoID: repoID, SrcID: units[2].ID, Kind: store.EdgeImport, DstName: "Bar", FilePath: "a.go", Line: 3},
	}
	...
	if err := st.DeleteEdgesByKindAndDst(ctx, store.EdgeCall, "Bar"); err != nil {
		t.Fatalf("delete edges by kind and dst: %v", err)
	}
	rest, err := st.GetEdges(ctx, domain.QueryOpts{RepoID: repoID})
	if err != nil || len(rest) != 2 {
		t.Fatalf("edges after delete: %v %v", rest, err)
	}
	for _, e := range rest {
		if e.Kind == store.EdgeCall && e.DstName == "Bar" {
			t.Fatalf("edge should have been deleted: %+v", e)
		}
	}

	// ClaimRepoForIndexing: first claim wins, second returns false, missing -> ErrNotFound.
	repo := &domain.Repo{ID: repoID, Name: "t", Source: domain.SourceTypeLocal, Path: "/tmp/x", Status: domain.StatusIdle, CreatedAt: 1}
	...
	if err != nil || r.Status != domain.StatusIndexing {
	...
	if _, err := st.ClaimRepoForIndexing(ctx, repoID+"-missing", "owner-a", 3600); !errors.Is(err, store.ErrNotFound) {
	...
	r, err = st.GetRepo(ctx, repoID)
	if err != nil || r.Status != domain.StatusIdle {
	...
	if err := st.UpdateRepoStatus(ctx, repoID, domain.StatusIdle, "", 7); err != nil {
		t.Fatal(err)
	}
...
func TestPostgresCommitJobQueue(t *testing.T) {
	...
	if first.ID == second.ID || first.Kind != domain.JobKindCommits {
...
func TestPostgresJobQueue(t *testing.T) {
	...
	if _, err := st.ClaimNextIndexJob(ctx, "w1"); !errors.Is(err, store.ErrNotFound) {
	...
	if j1.ID == "" || j1.Status != domain.JobStatusPending || j1.CreatedAt == 0 {
	...
	if c1.ID != j1.ID || c1.Status != domain.JobStatusRunning || c1.ClaimedBy != "w1" ||
	...
	if err := st.HeartbeatIndexJob(ctx, j3.ID, "w1"); !errors.Is(err, store.ErrNotFound) {
		t.Fatalf("heartbeat pending: err = %v, want ErrNotFound", err)
	}
	// Bookkeeping is scoped to the claiming worker.
	if err := st.HeartbeatIndexJob(ctx, c1.ID, "someone-else"); !errors.Is(err, store.ErrNotFound) {
		t.Fatalf("heartbeat from a foreign worker: err = %v, want ErrNotFound", err)
	}
	if err := st.CompleteIndexJob(ctx, c1.ID, "someone-else", "boom"); !errors.Is(err, store.ErrNotFound) {
	...
	if status != domain.JobStatusDone || jobErr != "" {
	...
	if status != domain.JobStatusError || jobErr != "boom" {
		t.Fatalf("job 2 after complete: status=%s error=%q", status, jobErr)
	}
	if err := st.CompleteIndexJob(ctx, c1.ID, "w1", ""); !errors.Is(err, store.ErrNotFound) {
	...
	if re.ID != j3.ID || re.ClaimedBy != "w2" || re.Status != domain.JobStatusRunning {
