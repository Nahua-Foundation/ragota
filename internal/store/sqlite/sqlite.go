...
// SQLite implements store.Storage with SQLite backend.
...
func (s *SQLite) SetVectorStore(vs store.VectorStorage) {
	s.vectorStore = vs
}
...
func (s *SQLite) VectorStore() store.VectorStorage {
	return s.vectorStore
}

// Compile-time check that *SQLite implements store.Storage and, since the
// linker only reaches the batched path through a type assertion, the optional
// EdgeResolutionBatcher as well.
var (
	_ store.Storage               = (*SQLite)(nil)
	_ store.EdgeResolutionBatcher = (*SQLite)(nil)
)
