...
func scanJobRow(sc scanner, withPayload bool) (*domain.IndexJob, error) {
...
// queued cheap one.
func (s *SQLite) EnqueueIndexJob(ctx context.Context, repoID string, force bool) (*domain.IndexJob, error) {
	job, err := scanJobRow(s.db.QueryRowContext(ctx,
		...
		repoID, domain.JobKindIndex, force, domain.JobStatusPending, time.Now().Unix(),
	), false)
	if err != nil {
...
func (s *SQLite) EnqueueCommitJob(ctx context.Context, repoID, payload string) (*domain.IndexJob, error) {
	row, err := s.q.EnqueueCommitJob(ctx, EnqueueCommitJobParams{
		RepoID:    repoID,
...
func (s *SQLite) GetIndexJob(ctx context.Context, jobID string) (*domain.IndexJob, error) {
	row, err := s.q.GetIndexJob(ctx, intOrZero(jobID))
	if err != nil {
		return nil, notFound(err, "get index job")
	}
	return jobFromRow(row), nil
}

// ListIndexJobs returns the repo's jobs, newest first.
func (s *SQLite) ListIndexJobs(ctx context.Context, repoID string, limit int) ([]*domain.IndexJob, error) {
	if limit <= 0 {
		limit = defaultJobListLimit
	}
	rows, err := s.q.ListIndexJobs(ctx, ListIndexJobsParams{RepoID: repoID, Limit: int64(limit)})
	if err != nil {
...
func (s *SQLite) HasPendingCommitJobBefore(ctx context.Context, repoID, jobID string) (bool, error) {
	ok, err := s.q.HasPendingCommitJobBefore(ctx, HasPendingCommitJobBeforeParams{RepoID: repoID, ID: intOrZero(jobID)})
	if err != nil {
...
func (s *SQLite) ClaimNextIndexJob(ctx context.Context, workerID string) (*domain.IndexJob, error) {
	now := time.Now().Unix()
	job, err := scanJobRow(s.db.QueryRowContext(ctx,
		...
		domain.JobStatusRunning, workerID, now, now, domain.JobStatusPending,
	), true)
...
func (s *SQLite) HeartbeatIndexJob(ctx context.Context, jobID, workerID string) error {
	n, err := s.q.HeartbeatIndexJob(ctx, HeartbeatIndexJobParams{
		HeartbeatAt: time.Now().Unix(), ID: intOrZero(jobID), ClaimedBy: workerID,
	})
	if err != nil {
...
func (s *SQLite) CompleteIndexJob(ctx context.Context, jobID, workerID string, jobErr string) error {
	...
	n, err := s.q.CompleteIndexJob(ctx, CompleteIndexJobParams{
		Status: status, Error: jobErr, ID: intOrZero(jobID), ClaimedBy: workerID,
	})
	if err != nil {
...
func (s *SQLite) ReleaseIndexJob(ctx context.Context, jobID, workerID string) error {
	res, err := s.db.ExecContext(ctx,
		...
		domain.JobStatusPending, intOrZero(jobID), domain.JobStatusRunning, workerID,
	)
	if err != nil {
...
func (s *SQLite) RequeueStaleIndexJobs(ctx context.Context, olderThanSec int64) (int, error) {
	...
	if _, err := s.db.ExecContext(ctx,
		...
		domain.JobStatusError, domain.JobStatusRunning, cutoff, domain.JobStatusRunning, cutoff,
	); err != nil {
	...
	res, err := s.db.ExecContext(ctx,
		...
		domain.JobStatusPending, domain.JobStatusRunning, cutoff,
	)
	if err != nil {
		return 0, fmt.Errorf("requeue stale index jobs: %w", err)
	}
