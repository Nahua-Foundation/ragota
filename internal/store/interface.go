...
type UnitStore interface {
	StoreASTUnit(ctx context.Context, u *domain.ASTUnit) error
	BatchStoreASTUnits(ctx context.Context, units []*domain.ASTUnit) error
	GetASTUnits(ctx context.Context, opts domain.QueryOpts) ([]*domain.ASTUnit, error)
	GetASTUnitsByIDs(ctx context.Context, ids []string) ([]*domain.ASTUnit, error)
...
type EdgeStore interface {
	StoreEdge(ctx context.Context, e *domain.Edge) error
	BatchStoreEdges(ctx context.Context, edges []*domain.Edge) error
	GetEdges(ctx context.Context, opts domain.QueryOpts) ([]*domain.Edge, error)
...
type RepoStore interface {
	...
	StoreRepo(ctx context.Context, r *domain.Repo) error
	...
	ListActiveRepos(ctx context.Context) ([]*domain.Repo, error)
	...
	UpdateRepoStatus(ctx context.Context, id string, status domain.Status, lastError string, indexedAt int64) error
...
type JobStore interface {
	...
	EnqueueIndexJob(ctx context.Context, repoID string, force bool) (*domain.IndexJob, error)
	...
	EnqueueCommitJob(ctx context.Context, repoID, payload string) (*domain.IndexJob, error)

	// GetIndexJob returns one job by ID, without its payload.
	GetIndexJob(ctx context.Context, jobID string) (*domain.IndexJob, error)

	// ListIndexJobs returns the repo's jobs, newest first, without their
	// payloads. limit <= 0 selects the backend default.
	ListIndexJobs(ctx context.Context, repoID string, limit int) ([]*domain.IndexJob, error)
	...
	ClaimNextIndexJob(ctx context.Context, workerID string) (*domain.IndexJob, error)
...
type CoverageStore interface {
	...
	// pass behind.
	StoreRepoCoverage(ctx context.Context, c *domain.RepoCoverage) error
	// GetRepoCoverage returns the repo's summary, or ErrNotFound when no
	// coverage-reporting pass has run for it.
	GetRepoCoverage(ctx context.Context, repoID string) (*domain.RepoCoverage, error)
...
type GraphStore interface {
	GetASTUnitByID(ctx context.Context, id string) (*domain.ASTUnit, error)
...
// It handles metadata (files, AST units, edges) and delegates vector store.
...
type VectorStorage interface {
	// Init initializes the vector store.
	Init(ctx context.Context) error
