// Package conform is the conformance suite for store.Storage.
//
...
func Run(t *testing.T, open func(t *testing.T) store.Storage) {
	t.Helper()
...
func seedRepo(t *testing.T, st store.Storage, repoID string) *domain.Repo {
	t.Helper()
...
func testRepoLifecycle(t *testing.T, st store.Storage) {
	ctx := context.Background()
	...
	if got.Name != "conformance" || got.Status != domain.StatusIdle {
		t.Errorf("GetRepo = %+v, want name=conformance status=idle", got)
	}

	// A status write reaches the row and clears the claim fields.
	if err := st.UpdateRepoStatus(ctx, repoID, domain.StatusError, "boom", 42); err != nil {
	...
	if got.Status != domain.StatusError || got.LastError != "boom" || got.IndexedAt != 42 {
	...
	// trace left of it.
	if err := st.UpdateRepoStatus(ctx, repoID+"-missing", domain.StatusIdle, "", 0); !errors.Is(err, store.ErrNotFound) {
		t.Errorf("UpdateRepoStatus on a missing repo = %v, want store.ErrNotFound", err)
	}

	// Re-registering a repo updates its definition without resetting lifecycle
	// state owned by the indexing pipeline.
	if err := st.StoreRepo(ctx, &domain.Repo{
		ID: repoID, Name: "renamed", Source: domain.SourceTypeLocal,
		Path: "/tmp/other", Status: domain.StatusIdle,
	}); err != nil {
	...
	if _, err := st.GetRepo(ctx, repoID); !errors.Is(err, store.ErrNotFound) {
		t.Errorf("GetRepo after delete = %v, want store.ErrNotFound", err)
	}
}

func testRepoClaim(t *testing.T, st store.Storage) {
	ctx := context.Background()
	...
	if got.Status != domain.StatusIndexing {
	...
	if _, err := st.ClaimRepoForIndexing(ctx, repoID+"-missing", "owner-a", 300); !errors.Is(err, store.ErrNotFound) {
		t.Errorf("claim on a missing repo = %v, want store.ErrNotFound", err)
	}
	...
	if err := st.UpdateRepoStatus(ctx, repoID, domain.StatusIdle, "", 1); err != nil {
		t.Fatalf("UpdateRepoStatus: %v", err)
	}
...
func testRepoActive(t *testing.T, st store.Storage) {
	ctx := context.Background()
	...
	if err := st.UpdateRepoStatus(ctx, b, domain.StatusError, "boom", 99); err != nil {
		t.Fatalf("UpdateRepoStatus: %v", err)
	}
	...
	if err := st.StoreRepo(ctx, &domain.Repo{
		ID: b, Name: "conformance", Source: domain.SourceTypeLocal,
		Path: "/tmp/" + b, Status: domain.StatusIdle, Active: true,
	}); err != nil {
		t.Fatalf("StoreRepo (re-register): %v", err)
	}
	if isActive(b) {
	...
	if got.Status != domain.StatusError || got.LastError != "boom" || got.IndexedAt != 99 {
...
func testUnitQueries(t *testing.T, st store.Storage) {
	ctx := context.Background()
	...
	units := []*domain.ASTUnit{
		{RepoID: repoID, FilePath: "a.go", Language: "go", Kind: "function",
			...
			Meta: store.EncodeUnitMeta(&store.UnitMeta{Root: "svc", Summary: "does the first thing"})},
		{RepoID: repoID, FilePath: "a.go", Language: "go", Kind: "function",
	...
	got, err := st.GetASTUnits(ctx, domain.QueryOpts{RepoID: repoID, Limit: 2})
	...
	// late in large files.
	inRange, err := st.GetASTUnits(ctx, domain.QueryOpts{RepoID: repoID, FilePath: "a.go", Line: 15})
	if err != nil || len(inRange) != 1 || inRange[0].Name != "First" {
		t.Errorf("GetASTUnits(line=15) = %+v, %v; want only First", inRange, err)
	}
	if outside, _ := st.GetASTUnits(ctx, domain.QueryOpts{RepoID: repoID, FilePath: "a.go", Line: 25}); len(outside) != 0 {
	...
	if m := store.DecodeUnitMeta(stored.Meta); m.Root != "svc" || m.Summary != "does the first thing" {
	...
	left, err := st.GetASTUnits(ctx, domain.QueryOpts{RepoID: repoID})
...
// cannot say it. Both backends compile the set through store.UnitFilters, so the shape of
// the SQL is asserted there; what belongs here is what the engines return.
func testUnitRepoScope(t *testing.T, st store.Storage) {
	ctx := context.Background()
	...
	unit := func(repoID, path, unitName string) *domain.ASTUnit {
	...
	if err := st.BatchStoreASTUnits(ctx, []*domain.ASTUnit{
		unit(here, "a.go", name),
	...
	repoIDs := func(got []*domain.ASTUnit) []string {
	...
	query := func(t *testing.T, opts domain.QueryOpts) []*domain.ASTUnit {
		t.Helper()
	...
	if got := query(t, domain.QueryOpts{Name: name}); len(got) != 2 {
		t.Errorf("unscoped query = %v, want both repositories", repoIDs(got))
	}
	// One repository in the set answers alone...
	if got := query(t, domain.QueryOpts{Name: name, Repos: []string{here}}); !equalStrings(repoIDs(got), []string{here}) {
		t.Errorf("scoped to one = %v, want only %s", repoIDs(got), here)
	}
	// ...and naming both is the same answer as naming neither.
	if got := query(t, domain.QueryOpts{Name: name, Repos: []string{here, there}}); len(got) != 2 {
	...
	// database.
	if got := query(t, domain.QueryOpts{Name: name, Repos: []string{newRepoID("absent")}}); len(got) != 0 {
	...
	if got := query(t, domain.QueryOpts{RepoID: here, Name: name, Repos: []string{there}}); len(got) != 0 {
		t.Errorf("repo_id outside the set = %v, want nothing", repoIDs(got))
	}
	if got := query(t, domain.QueryOpts{RepoID: here, Name: name, Repos: []string{here, there}}); !equalStrings(repoIDs(got), []string{here}) {
	...
	near := query(t, domain.QueryOpts{NameOrQualified: name, Repos: []string{here}, Limit: 5, Fallback: true})
	if !equalStrings(repoIDs(near), []string{here}) {
		t.Errorf("fallback scoped to one repository = %v, want only %s", repoIDs(near), here)
	}
	far := query(t, domain.QueryOpts{NameOrQualified: name, Repos: []string{there}, Limit: 5, Fallback: true})
...
// front of the answer.
func testSymbolLookup(t *testing.T, st store.Storage) {
	ctx := context.Background()
	repoID := newRepoID("symbols")
	seedRepo(t, st, repoID)

	unit := func(path, name, qualified string) *domain.ASTUnit {
		return &domain.ASTUnit{
			RepoID: repoID, FilePath: path, Language: "go", Kind: "function",
	...
	units := []*domain.ASTUnit{
		...
		// through name.
		unit("src/shippingservice/app.go", "ShippingService", "ShipOrder"),
	...
	paths := func(got []*domain.ASTUnit) []string {
		out := make([]string, 0, len(got))
		for _, u := range got {
	...
	names := func(got []*domain.ASTUnit) []string {
		out := make([]string, 0, len(got))
		for _, u := range got {
	...
	byName := query(t, domain.QueryOpts{Name: "shiporder"})
	byQualified := query(t, domain.QueryOpts{Qualified: "shiporder"})
	either := query(t, domain.QueryOpts{NameOrQualified: "shiporder"})
	...
	// callers rely on.
	if both := query(t, domain.QueryOpts{Name: "ShippingService", Qualified: "ShipOrder"}); len(both) != 1 {
		t.Errorf("Name and Qualified together = %d units, want the 1 matching both", len(both))
	}
	if both := query(t, domain.QueryOpts{Name: "ShipOrder", Qualified: "ShipOrder"}); len(both) != 0 {
	...
	ranked := query(t, domain.QueryOpts{Name: "ShipOrder"})
	...
	if top := query(t, domain.QueryOpts{Name: "ShipOrder", Limit: 1}); len(top) != 1 ||
	...
	if plain := query(t, domain.QueryOpts{Name: "chargecar", Limit: 5}); len(plain) != 0 {
		t.Errorf("a substring matched without Fallback: %v", names(plain))
	}
	near := query(t, domain.QueryOpts{Name: "chargecar", Limit: 5, Fallback: true})
	...
	topped := query(t, domain.QueryOpts{Name: "charge", Limit: 5, Fallback: true})
	...
	if full := query(t, domain.QueryOpts{Name: "charge", Limit: 1, Fallback: true}); len(full) != 1 ||
		full[0].Name != "Charge" {
		t.Errorf("a full page was widened: %v", names(full))
	}
	// The fallback reaches the qualified name through the one-term form too.
	viaQualified := query(t, domain.QueryOpts{NameOrQualified: "chargecardh", Limit: 5, Fallback: true})
	...
	// line.
	checkout := query(t, domain.QueryOpts{NameOrQualified: "placeorder", Limit: 3, Fallback: true})
	...
	all := query(t, domain.QueryOpts{NameOrQualified: "placeorder", Limit: 10, Fallback: true})
	...
	// ranking one badly.
	exactOnly := query(t, domain.QueryOpts{NameOrQualified: "placeorder", Limit: 3})
...
func testFiles(t *testing.T, st store.Storage) {
	ctx := context.Background()
	...
	if _, err := st.GetFile(ctx, repoID, "nope.go"); !errors.Is(err, store.ErrNotFound) {
		t.Errorf("GetFile(missing) = %v, want store.ErrNotFound", err)
	}
...
func testInsertionOrder(t *testing.T, st store.Storage) {
	ctx := context.Background()
	...
	storeEdges := func(repoID string, reversed bool) {
		t.Helper()
		units := make([]*domain.ASTUnit, 0, len(files))
		edges := make([]*domain.Edge, 0, len(files))
		for i := range files {
			f := files[i]
			if reversed {
				f = files[len(files)-1-i]
			}
			units = append(units, &domain.ASTUnit{
		...
		for _, u := range units {
			edges = append(edges, &domain.Edge{
				RepoID: repoID, SrcID: u.ID, Kind: store.EdgeWritesTo, DstName: "db:visits",
	...
	storeEdges(forward, false)
	storeEdges(reverse, true)

	unitPaths := func(repoID string, limit int) []string {
		t.Helper()
		got, err := st.GetASTUnits(ctx, domain.QueryOpts{
	...
	edgePaths := func(repoID string, limit int) []string {
		t.Helper()
		got, err := st.GetEdges(ctx, domain.QueryOpts{
...
func testUnitsAndEdges(t *testing.T, st store.Storage) {
	ctx := context.Background()
	...
	if err := st.BatchStoreASTUnits(ctx, []*domain.ASTUnit{caller, callee}); err != nil {
		t.Fatalf("BatchStoreASTUnits: %v", err)
	}
	...
	byKind, err := st.GetASTUnits(ctx, domain.QueryOpts{RepoID: repoID, Kind: "function"})
	if err != nil || len(byKind) != 2 {
		t.Errorf("GetASTUnits(kind=function) = %d units, %v; want 2, nil", len(byKind), err)
	}
	byFile, err := st.GetASTUnits(ctx, domain.QueryOpts{RepoID: repoID, FilePath: "callee.go"})
	...
	edge := &domain.Edge{
		...
		Kind: store.EdgeCall, DstName: "Callee", FilePath: "caller.go",
		Line: 2, Confidence: 0.9,
	}
	if err := st.BatchStoreEdges(ctx, []*domain.Edge{edge}); err != nil {
		t.Fatalf("BatchStoreEdges: %v", err)
	}
	edges, err := st.GetEdges(ctx, domain.QueryOpts{RepoID: repoID, SrcID: caller.ID})
	...
	if err := st.BatchStoreASTUnits(ctx, []*domain.ASTUnit{kept}); err != nil {
		t.Fatalf("BatchStoreASTUnits (kept): %v", err)
	}
	keptEdge := &domain.Edge{
		RepoID: repoID, SrcID: caller.ID, DstID: kept.ID, DstRepoID: repoID,
		Kind: store.EdgeCall, DstName: "Kept", FilePath: "caller.go", Line: 4, Confidence: 1,
	}
	if err := st.BatchStoreEdges(ctx, []*domain.Edge{keptEdge}); err != nil {
		t.Fatalf("BatchStoreEdges (kept): %v", err)
	}
	if err := st.DeleteASTUnitsByFiles(ctx, repoID, []string{"gone.go"}); err != nil {
		t.Fatalf("DeleteASTUnitsByFiles: %v", err)
	}
	after, err := st.GetEdges(ctx, domain.QueryOpts{RepoID: repoID, SrcID: caller.ID, Kind: store.EdgeCall})
	if err != nil {
...
func testEdgeResolution(t *testing.T, st store.Storage) {
	ctx := context.Background()
	...
	dst := &domain.ASTUnit{
		RepoID: repoID, FilePath: "dst.go", Language: "go", Kind: store.KindHTTPRoute,
		Name: "GET /things", Qualified: "http:GET /things", StartLine: 1, EndLine: 2,
	}
	if err := st.BatchStoreASTUnits(ctx, []*domain.ASTUnit{src, dst}); err != nil {
		t.Fatalf("BatchStoreASTUnits: %v", err)
	}

	edge := &domain.Edge{
		RepoID: repoID, SrcID: src.ID, Kind: store.EdgeHTTPCall,
	...
	stored, err := st.GetEdges(ctx, domain.QueryOpts{RepoID: repoID, Kind: store.EdgeHTTPCall})
	...
	// its work.
	unresolved, err := st.GetEdges(ctx, domain.QueryOpts{RepoID: repoID, Unresolved: true})
	...
	stored, _ = st.GetEdges(ctx, domain.QueryOpts{RepoID: repoID, Kind: store.EdgeHTTPCall})
	...
	stored, _ = st.GetEdges(ctx, domain.QueryOpts{RepoID: repoID, Kind: store.EdgeHTTPCall})
	if got := store.DecodeEdgeMeta(stored[0].Meta); got.Source != "llm" || got.BaseConf != 0.9 {
	...
	// single-edge path.
	batcher, ok := st.(store.EdgeResolutionBatcher)
	if !ok {
		t.Log("backend does not implement EdgeResolutionBatcher; skipping the batch path")
		return
	}
	failures, err := batcher.BatchUpdateEdgeResolutions(ctx, []store.EdgeResolution{
...
func testCoverage(t *testing.T, st store.Storage) {
	ctx := context.Background()
	repoID := newRepoID("coverage")
	seedRepo(t, st, repoID)

	if _, err := st.GetRepoCoverage(ctx, repoID); !errors.Is(err, store.ErrNotFound) {
		t.Errorf("GetRepoCoverage before any pass = %v, want store.ErrNotFound", err)
	}
	...
	first := &domain.RepoCoverage{
		...
		Kinds: map[string]domain.CoverageCounts{
			domain.ContractKindHTTP: {Candidates: 10, Edges: 7},
			domain.ContractKindDB:   {Candidates: 4, Edges: 4},
		},
	}
	...
	if len(got.Kinds) != 2 || got.Kinds[domain.ContractKindHTTP].Edges != 7 {
	...
	if err := st.StoreRepoCoverage(ctx, &domain.RepoCoverage{
		RepoID:    repoID,
		UpdatedAt: 200,
		Kinds:     map[string]domain.CoverageCounts{domain.ContractKindHTTP: {Candidates: 12, Edges: 12}},
	}); err != nil {
	...
	if _, stale := got.Kinds[domain.ContractKindDB]; stale {
	...
	if _, err := st.GetRepoCoverage(ctx, repoID); !errors.Is(err, store.ErrNotFound) {
		t.Errorf("GetRepoCoverage after delete = %v, want store.ErrNotFound", err)
	}
}

func testJobs(t *testing.T, st store.Storage) {
	ctx := context.Background()
	...
	if job.Status != domain.JobStatusPending || job.Kind != domain.JobKindIndex {
