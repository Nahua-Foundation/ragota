...
func TestUpsertRejectsDimensionMismatch(t *testing.T) {
	...
	err := q.Upsert(context.Background(), []*store.VectorPoint{
...
func TestUpsertChecksCollectionOnce(t *testing.T) {
	...
	for range 5 {
		if err := q.Upsert(context.Background(), []*store.VectorPoint{
...
func TestUpsertRecoversFromDroppedCollection(t *testing.T) {
	...
	point := []*store.VectorPoint{{ID: "1", Vector: []float32{1, 2, 3, 4}}}
