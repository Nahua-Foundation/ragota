...
//	RAGOTA_TEST_QDRANT_URL=http://127.0.0.1:6333 go test ./internal/store/qdrant/
...
func livePoint(id, repo, path string, vec []float32) *store.VectorPoint {
...
func TestLiveRoundTrip(t *testing.T) {
	...
	var res []*store.VectorResult
	var err error
	for i := 0; i < 20; i++ {
		res, err = q.Search(ctx, store.VectorSearchOpts{
	...
	other, err := q.Search(ctx, store.VectorSearchOpts{
...
func TestLivePayloadIndexesAreCreated(t *testing.T) {
	...
	if err := q.Upsert(ctx, []*store.VectorPoint{
