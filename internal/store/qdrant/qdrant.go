...
// Qdrant implements store.VectorStorage with Qdrant backend.
// The vector backend satisfies its contract at compile time.
var _ store.VectorStorage = (*Qdrant)(nil)
...
// Init initializes the Qdrant store.
...
func (q *Qdrant) Upsert(ctx context.Context, points []*store.VectorPoint) error {
	if len(points) == 0 {
		return nil
	}
	coll := q.collection()
...
func (q *Qdrant) buildPoints(points []*store.VectorPoint) []map[string]interface{} {
...
func (q *Qdrant) Search(ctx context.Context, opts store.VectorSearchOpts) ([]*store.VectorResult, error) {
	...
	results := make([]*store.VectorResult, len(response.Result))
...
func (q *Qdrant) Stats(ctx context.Context) (*store.VectorStats, error) {
