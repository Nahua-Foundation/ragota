...
type Vector struct {
	...
	sink  atomic.Pointer[WriteSink] // atomic для безопасного concurrent read/write
...
// SetBM25 подключает BM25-индекс к индексатору; если sink == nil —
// лексический индекс не используется. Принимает любой WriteSink.
func (v *Vector) SetBM25(sink WriteSink) { v.sink.Store(&sink) }

// writeSink возвращает текущий поисковый индекс (или nil).
func (v *Vector) writeSink() WriteSink {
	p := v.sink.Load()
	if p == nil {
		return nil
	}
