...
// Indexer implements indexing.Indexer and indexing.Searcher for BM25 keyword search.
type Indexer struct {
	index   bleve.Index
	path    string
	chunker *indexing.WindowChunker
...
func New(cfg *Config) (*Indexer, error) {
	...
	case errors.Is(err, bleve.ErrorIndexPathDoesNotExist):
		...
		if index, err = bleve.New(cfg.Path, mapping); err != nil {
	...
	return &Indexer{
		index:   index,
		path:    cfg.Path,
		chunker: indexing.NewWindowChunker(indexing.ChunkConfig{}),
...
func (i *Indexer) Index(ctx context.Context, req *indexing.IndexRequest) (*indexing.IndexResult, error) {
	...
	for _, file := range req.Files {
		...
		// Chunk the content
		chunks, err := i.chunker.Chunk(ctx, &indexing.ChunkInput{Path: file.Path, Language: file.Language, Content: file.Content})
		if err != nil {
			result.FilesFailed++
			result.Errors = append(result.Errors, fmt.Sprintf("%s: %v", file.Path, err))
			continue
	...
	for _, cf := range chunked {
		syms, ok := symbols.Shared.Take(req.RepoID, cf.file.Path, cf.file.Hash)
		if !ok {
			unlabelled = append(unlabelled, cf)
			continue
		}
		obs.Inc("ragota_bm25_symbols_reused_total", 1)
	...
	for _, cf := range unlabelled {
		...
		case ok:
			obs.Inc("ragota_bm25_symbols_reused_total", 1)
		case symbols.Annotated(cf.file.Language):
			syms = symbolUnits(cf.file.Path, cf.file.Language, cf.file.Content)
			obs.Inc("ragota_bm25_symbols_parsed_total", 1)
		}
		if err := i.indexChunks(batch, req.RepoID, cf, syms); err != nil {
			return nil, err
		}
...
func (i *Indexer) indexChunks(batch *bleve.Batch, repoID string, cf chunkedFile, syms []symbols.Symbol) error {
...
// explicit Compact still does the work (see service.CompactIndexes).
...
func (i *Indexer) Compact(ctx context.Context) error {
	...
	obs.RecordDuration("ragota_bm25_compact_seconds", time.Since(start).Seconds())
	return err
}
...
func (i *Indexer) awaitSteadyState(ctx context.Context) error {
	...
	for {
		...
		case <-deadline.C:
			...
			obs.Inc("ragota_bm25_compact_unsettled_total", 1)
			return nil
...
var ErrIndexDamaged = indexing.ErrIndexDamaged
...
func (i *Indexer) Search(ctx context.Context, q *indexing.SearchQuery) (res *indexing.SearchResult, err error) {
	start := time.Now()
	...
	defer func() {
		if r := recover(); r != nil {
			...
			obs.Inc("ragota_bm25_search_panics_total", 1)
			obs.Inc("ragota_bm25_search_damaged_total", 1)
		}
	}()
	...
	// Convert to hits
	hits := make([]*indexing.Hit, 0, len(searchResult.Hits))
	for _, hit := range searchResult.Hits {
		doc := hit.Fields

		hits = append(hits, &indexing.Hit{
...
func symbolUnits(path, language string, content []byte) []symbols.Symbol {
	if !symbols.Annotated(language) {
		return nil
	}
...
func dominantUnit(syms []symbols.Symbol, startLine, endLine int) (string, string) {
