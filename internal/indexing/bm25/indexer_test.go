...
func TestBM25SearchReturnsLineNumbers(t *testing.T) {
	...
	if _, err := idx.Index(context.Background(), &indexing.IndexRequest{
		RepoID: "r1",
		Files:  []*indexing.FileToIndex{{Path: "calc.go", Language: "go", Content: content}},
	}); err != nil {
		t.Fatalf("Index: %v", err)
	}

	res, err := idx.Search(context.Background(), &indexing.SearchQuery{Query: "Add", Limit: 10})
	if err != nil {
...
func TestBM25QueryWithReservedSyntax(t *testing.T) {
	...
	if _, err := idx.Index(ctx, &indexing.IndexRequest{
		RepoID: "r1",
		Files:  []*indexing.FileToIndex{{Path: "h.go", Language: "go", Content: content}},
	}); err != nil {
		t.Fatalf("Index: %v", err)
	}

	plain, err := idx.Search(ctx, &indexing.SearchQuery{Query: "Content-Type application/json", Limit: 10})
	if err != nil {
	...
	for _, query := range []string{
		...
		res, err := idx.Search(ctx, &indexing.SearchQuery{Query: query, Limit: 10})
		if err != nil {
...
func TestBM25EmptyQueryIsAnError(t *testing.T) {
	...
	for _, query := range []string{"", "   ", "\t\n"} {
		if _, err := idx.Search(context.Background(), &indexing.SearchQuery{Query: query, Limit: 10}); !errors.Is(err, ErrEmptyQuery) {
...
func TestBM25DocumentsCarrySymbolMetadata(t *testing.T) {
	...
	res, err := idx.Search(ctx, &indexing.SearchQuery{Query: "Add", Limit: 10})
	if err != nil {
		t.Fatalf("Search: %v", err)
	}
	if len(res.Hits) == 0 {
...
func TestBM25SearchFilters(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			res, err := idx.Search(ctx, &indexing.SearchQuery{Query: "Add", Limit: 10, Filter: tt.filter})
			if err != nil {
				t.Fatalf("Search: %v", err)
			}
			if len(res.Hits) == 0 {
...
func TestBM25ConfigParamsApplied(t *testing.T) {
	...
	if got := buildMapping().ScoringModel; got != index.BM25Scoring {
		t.Errorf("scoring model = %q, want %q", got, index.BM25Scoring)
	}
}
...
func TestBM25SearchRepoFilter(t *testing.T) {
	...
	if _, err := idx.Index(ctx, &indexing.IndexRequest{
		RepoID: "r1",
		Files:  []*indexing.FileToIndex{{Path: "a.go", Language: "go", Content: []byte("func Add() {}\n")}},
	}); err != nil {
		t.Fatalf("Index r1: %v", err)
	}
	if _, err := idx.Index(ctx, &indexing.IndexRequest{
		RepoID: "r2",
		Files:  []*indexing.FileToIndex{{Path: "b.go", Language: "go", Content: []byte("func Add() {}\n")}},
	}); err != nil {
		t.Fatalf("Index r2: %v", err)
	}

	// Scoped to r1 → only r1 hits.
	res, err := idx.Search(ctx, &indexing.SearchQuery{Query: "Add", Repos: []string{"r1"}, Limit: 10})
	if err != nil {
	...
	// Unscoped → both repos.
	res2, err := idx.Search(ctx, &indexing.SearchQuery{Query: "Add", Limit: 10})
	if err != nil {
...
func TestBM25RemovePath(t *testing.T) {
	...
	if _, err := idx.Index(ctx, &indexing.IndexRequest{RepoID: "r1", Files: files}); err != nil {
		t.Fatalf("Index: %v", err)
	}
...
func TestBM25ReindexRemovesStaleChunks(t *testing.T) {
	...
	if _, err := idx.Index(ctx, &indexing.IndexRequest{
		RepoID: "r1",
		Files:  []*indexing.FileToIndex{{Path: "big.go", Language: "go", Content: []byte(b.String())}},
	}); err != nil {
	...
	if _, err := idx.Index(ctx, &indexing.IndexRequest{
		RepoID: "r1",
		Files:  []*indexing.FileToIndex{{Path: "big.go", Language: "go", Content: []byte("small file now\n")}},
	}); err != nil {
		t.Fatalf("Index small: %v", err)
	}
	res, err := idx.Search(ctx, &indexing.SearchQuery{Query: "UNIQUETOKEN", Limit: 10})
	if err != nil {
		t.Fatalf("Search: %v", err)
	}
	if len(res.Hits) != 0 {
...
func chunkCount(t *testing.T, content []byte) int {
	t.Helper()
	chunks, err := indexing.NewWindowChunker(indexing.ChunkConfig{}).Chunk(context.Background(),
		&indexing.ChunkInput{Path: "x", Content: content})
	if err != nil {
...
func hitPaths(t *testing.T, idx *Indexer, q string) []string {
	t.Helper()
	res, err := idx.Search(context.Background(), &indexing.SearchQuery{Query: q, Limit: 50})
	if err != nil {
...
func TestBM25ReindexWindowDropsOnlyStaleChunks(t *testing.T) {
	...
	files := func(paths ...string) []*indexing.FileToIndex {
		out := make([]*indexing.FileToIndex, 0, len(paths))
		for _, p := range paths {
			out = append(out, &indexing.FileToIndex{Path: p, Content: before[p]})
		}
		return out
	}
	if _, err := idx.Index(ctx, &indexing.IndexRequest{
...
func TestBM25ReindexKeepsSamePathInOtherRepo(t *testing.T) {
	...
	for _, repo := range []string{"r1", "r2"} {
		if _, err := idx.Index(ctx, &indexing.IndexRequest{
			RepoID: repo,
			Files:  []*indexing.FileToIndex{{Path: "shared.go", Content: shared}},
		}); err != nil {
	...
	if _, err := idx.Index(ctx, &indexing.IndexRequest{
		RepoID: "r1",
		Files:  []*indexing.FileToIndex{{Path: "shared.go", Content: filler(10, nil)}},
	}); err != nil {
		t.Fatalf("Index r1 again: %v", err)
	}

	res, err := idx.Search(ctx, &indexing.SearchQuery{Query: "TAILSHARED", Limit: 50})
	if err != nil {
		t.Fatalf("Search: %v", err)
	}
...
func TestBM25ChunkScanPagesThroughEveryChunk(t *testing.T) {
	...
	if _, err := idx.Index(ctx, &indexing.IndexRequest{
		RepoID: "r1",
		Files:  []*indexing.FileToIndex{{Path: "wide.go", Content: big}},
	}); err != nil {
		t.Fatalf("Index big: %v", err)
	}
	...
	if _, err := idx.Index(ctx, &indexing.IndexRequest{
		RepoID: "r1",
		Files:  []*indexing.FileToIndex{{Path: "wide.go", Content: small}},
	}); err != nil {
		t.Fatalf("Index small: %v", err)
	}
...
func TestBM25RemoveMorePathsThanOneQueryNames(t *testing.T) {
	...
	files := make([]*indexing.FileToIndex, 0, nFiles)
	paths := make([]string, 0, nFiles)
	for i := 0; i < nFiles; i++ {
		path := fmt.Sprintf("f%04d.txt", i)
		files = append(files, &indexing.FileToIndex{Path: path, Content: []byte("KEEPTOKEN body\n")})
...
func TestBM25WindowCostsOneChunkScan(t *testing.T) {
	...
	files := make([]*indexing.FileToIndex, 0, 64)
	for i := 0; i < 64; i++ {
...
func TestBM25AnnotatesFromPublishedSymbols(t *testing.T) {
	...
	symbols.Shared.Put("r1", "calc.go", "hash-1", []symbols.Symbol{
	...
	if _, err := idx.Index(ctx, &indexing.IndexRequest{
		RepoID: "r1",
		Files:  []*indexing.FileToIndex{{Path: "calc.go", Language: "go", Hash: "hash-1", Content: content}},
	}); err != nil {
		t.Fatalf("Index: %v", err)
	}
	...
	if got := obs.Value("ragota_bm25_symbols_reused_total"); got != 1 {
		t.Errorf("reused counter = %d, want 1", got)
	}
	if got := obs.Value("ragota_bm25_symbols_parsed_total"); got != 0 {
		t.Errorf("parsed counter = %d, want 0: the file's symbols were already published", got)
	}
}
...
func TestBM25ParsesWhenNoSymbolsPublished(t *testing.T) {
	...
	if _, err := idx.Index(ctx, &indexing.IndexRequest{
		...
		Files:  []*indexing.FileToIndex{{Path: "calc.go", Language: "go", Hash: "hash-2", Content: content}},
	}); err != nil {
		t.Fatalf("Index: %v", err)
	}
	...
	if got := obs.Value("ragota_bm25_symbols_parsed_total"); got != 1 {
...
func TestBM25PublishedAndParsedAnnotationsMatch(t *testing.T) {
	...
	content := []byte("package demo\n\ntype T struct{ N int }\n\n" +
		...
		"func Helper() int {\n\treturn 1\n}\n")
	file := func() *indexing.FileToIndex {
		return &indexing.FileToIndex{Path: "demo.go", Language: "go", Hash: "h", Content: content}
	}
	...
	annotate := func(t *testing.T, publish bool) map[string]string {
		...
		if publish {
			...
			symbols.Shared.Put(repoID, "demo.go", "h", symbols.Project(units))
		}
		if _, err := idx.Index(ctx, &indexing.IndexRequest{RepoID: repoID, Files: []*indexing.FileToIndex{file()}}); err != nil {
			t.Fatalf("Index: %v", err)
		}
		res, err := idx.Search(ctx, &indexing.SearchQuery{Query: "Describe Helper T", Limit: 50})
		if err != nil {
			t.Fatalf("Search: %v", err)
		}
		if len(res.Hits) == 0 {
...
type storageStub struct{ storage.Storage }
...
func (storageStub) BatchStoreASTUnits(context.Context, []*storage.ASTUnit) error  { return nil }
func (storageStub) BatchStoreEdges(context.Context, []*storage.Edge) error        { return nil }
...
func TestBM25RunsConcurrentlyWithASTIndexer(t *testing.T) {
	...
	build := func() []*indexing.FileToIndex {
		files := make([]*indexing.FileToIndex, 0, nFiles)
		for i := 0; i < nFiles; i++ {
	...
	go func() {
		defer wg.Done()
		if _, err := astIdx.Index(ctx, &indexing.IndexRequest{RepoID: "shared", Files: build()}); err != nil {
			t.Errorf("ast Index: %v", err)
		}
	}()
	go func() {
		defer wg.Done()
		if _, err := idx.Index(ctx, &indexing.IndexRequest{RepoID: "shared", Files: build()}); err != nil {
	...
	reused := obs.Value("ragota_bm25_symbols_reused_total")
	parsed := obs.Value("ragota_bm25_symbols_parsed_total")
	if reused+parsed != nFiles {
		t.Errorf("annotated %d files (%d reused, %d parsed), want %d", reused+parsed, reused, parsed, nFiles)
	}

	for i := 0; i < nFiles; i++ {
		name := fmt.Sprintf("Fn%02d", i)
		res, err := idx.Search(ctx, &indexing.SearchQuery{Query: name, Repos: []string{"shared"}, Limit: 5})
		if err != nil {
...
func TestSearchRecoversFromDamagedSegment(t *testing.T) {
	...
	res, err := idx.Search(context.Background(), &indexing.SearchQuery{Query: "anything", Limit: 5})
	if err == nil {
...
func TestErrIndexDamagedIsShared(t *testing.T) {
	if !errors.Is(ErrIndexDamaged, indexing.ErrIndexDamaged) {
		t.Error("bm25.ErrIndexDamaged does not match indexing.ErrIndexDamaged")
	}
	wrapped := fmt.Errorf("%w: %v", ErrIndexDamaged, errors.New("memUvarintReader overflow"))
	if !errors.Is(wrapped, indexing.ErrIndexDamaged) {
		t.Error("a wrapped damaged-index error does not match indexing.ErrIndexDamaged")
	}
}
