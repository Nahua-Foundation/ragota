...
func (p *pyExtractor) recordModel(fc *fileCtx, def *sitter.Node, idx int) {
	...
	p.models[name] = tbl
	fc.addUnit(def, storage.KindDBTable, tbl, contract.DB(tbl), "entity:"+name, fc.units[idx].Doc)
}
...
func (p *pyExtractor) applyDecorators(fc *fileCtx, decorated *sitter.Node, fnIdx int) {
	...
	for i := 0; i < int(decorated.NamedChildCount()); i++ {
		...
		if pyConsumerDecorators[name] {
			...
			fc.addEdge(fnIdx, storage.EdgeConsumes, key, fn.StartLine, contract.ConfHeuristic,
				&storage.EdgeMeta{Topic: tn})
			continue
		}
	}
}
...
func (p *pyExtractor) handleCall(fc *fileCtx, call *sitter.Node) {
	...
	if name != "" {
		fc.addEdge(src, storage.EdgeCall, name, line, contract.ConfHeuristic,
			&storage.EdgeMeta{Args: args, Aliases: aliases, Receiver: object})
	}
}
