...
`
	...
	if findUnit(units, storage.KindHTTPRoute, "POST /api/analytics/events") == nil {
		t.Errorf("missing fastapi route; units: %+v", names(units))
	}
	if findUnit(units, storage.KindHTTPRoute, "POST /legacy") == nil {
	...
	if findEdge(edges, storage.EdgeHandledBy, "ingest_event") == nil {
		t.Errorf("missing handled_by; edges: %+v", edgeNames(edges))
	}

	e := findEdge(edges, storage.EdgeHTTPCall, "http:POST /api/audit/log")
	if e == nil {
		t.Fatalf("missing http_call; edges: %+v", edgeNames(edges))
	}
	if m := storage.DecodeEdgeMeta(e.Meta); m.Fields["user_id"] != "user_id" {
		t.Errorf("http_call fields = %+v", m.Fields)
	}

	w := findEdge(edges, storage.EdgeWritesTo, "db:analytics_events")
	if w == nil {
		t.Fatalf("missing writes_to; edges: %+v", edgeNames(edges))
	}
	if m := storage.DecodeEdgeMeta(w.Meta); m.Fields["user_id"] == "" {
	...
	if findEdge(edges, storage.EdgeProduces, "topic:${ORDERS_TOPIC}") == nil {
		t.Errorf("missing produces with env ref; edges: %+v", edgeNames(edges))
	}
	if findEdge(edges, storage.EdgeCall, "record_event") == nil {
...
`
	...
	if u := findUnit(units, storage.KindDBTable, "analytics_events"); u == nil {
	...
	if u := findUnit(units, storage.KindDBColumn, "user_id"); u == nil {
	...
	if findUnit(units, storage.KindDBColumn, "source") == nil {
		t.Errorf("missing ALTER-added column; units: %+v", names(units))
	}
	// constraint lines must not become columns
	if findUnit(units, storage.KindDBColumn, "primary") != nil {
...
`
	...
	w := findEdge(edges, storage.EdgeWritesTo, "db:orders")
	if w == nil {
		t.Fatalf("missing writes_to; edges: %+v", edgeNames(edges))
	}
	if m := storage.DecodeEdgeMeta(w.Meta); m.Fields["user_id"] == "" {
