		...
		if name != "" {
			...
			if strings.Contains(fc.text(n), `gorm:"`) {
				if tbl := tableName(name); tbl != "" {
					fc.addUnit(n, storage.KindDBTable, tbl, contract.DB(tbl), "entity:"+name, doc)
				}
			}
		}
...
func (g *goExtractor) handleCall(fc *fileCtx, call *sitter.Node) {
	...
	if goKafkaProduceMethods[name] {
		...
		if topic != "" {
			...
			fc.addEdge(src, storage.EdgeProduces, key, line, conf,
				&storage.EdgeMeta{Topic: name, Fields: fields, Aliases: g.aliases.relevant(pos, args, fields)})
			return
		}
		messagingSite = true
	}
	if goKafkaConsumeMethods[name] {
		...
		if topic != "" {
			fc.contractSite(storage.ContractKindMessaging, true)
			key, name := topicEdgeKey(topic)
			fc.addEdge(src, storage.EdgeConsumes, key, line, conf, &storage.EdgeMeta{Topic: name})
			return
		}
		messagingSite = true
	}
	...
	msgCandidates := fc.cov[storage.ContractKindMessaging].Candidates
	...
	if messagingSite && fc.cov[storage.ContractKindMessaging].Candidates == msgCandidates && brokerEvidence(fc, cs) {
		fc.contractSite(storage.ContractKindMessaging, false)
	}
	...
	if !g.memdbAccess(fc, src, name, operand, args, line, pos) &&
		...
		if tbl != "" || isDBReceiver(operand) {
			fc.contractSite(storage.ContractKindDB, tbl != "")
		}
		if tbl != "" {
			...
			fc.addEdge(src, kind, contract.DB(tbl), line, contract.ConfCrossFile,
				&storage.EdgeMeta{Args: args, Aliases: g.aliases.relevant(pos, args, nil)})
		}
	}
	...
	if name != "" && src >= 0 {
		fc.addEdge(src, storage.EdgeCall, name, line, contract.ConfHeuristic,
			&storage.EdgeMeta{Args: args, Aliases: g.aliases.relevant(pos, args, nil), Receiver: operand, RecvType: g.types[lastComponent(operand)]})
	}
}
...
func (g *goExtractor) memdbAccess(fc *fileCtx, src int, name, operand string, args []string, line, pos int) bool {
	...
	fc.contractSite(storage.ContractKindDB, false)
