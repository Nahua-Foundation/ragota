	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, want := range tt.routes {
				u := findUnit(units, storage.KindHTTPRoute, want)
				if u == nil {
			...
			for _, bad := range tt.absent {
				if findUnit(units, storage.KindHTTPRoute, bad) != nil {
...
`
	...
	n := 0
	for _, u := range units {
		if u.Kind == storage.KindHTTPRoute && strings.HasSuffix(u.Name, "/health") {
			n++
		}
	}
	if n != 2 {
	...
	var confs []float32
	for _, e := range edges {
		if e.Kind == storage.EdgeHandledBy && e.DstName == "healthHandler" {
