...
`
	...
	oc := findUnit(units, storage.KindTopicChannel, "orders.created")
	if oc == nil {
	...
	ps := findUnit(units, storage.KindTopicChannel, "payments.settled")
	if ps == nil {
...
`
	...
	if lc := findUnit(units, storage.KindTopicChannel, "legacyChannel"); lc == nil {
...
`
	...
	for _, u := range units {
		if u.Kind == storage.KindTopicChannel {
...
`
	...
	if findUnit(units, storage.KindTopicChannel, "orders.created") == nil {
