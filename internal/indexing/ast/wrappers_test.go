...
`
	...
	e := findEdge(facts.Edges, storage.EdgeHTTPCall, "http:GET /v2/domain-search")
	if e == nil {
...
`
	...
	var through *storage.Edge
	for _, e := range facts.Edges {
		if e.Kind == storage.EdgeHTTPCall && strings.Contains(e.Meta, "wrapper:") {
			through = e
		}
	}
	if through == nil {
...
`
	...
	if _, err := idx.Index(context.Background(), &indexing.IndexRequest{RepoID: "r", Files: files}); err != nil {
...
`
	...
	for _, e := range facts.Edges {
		if e.Kind == storage.EdgeHTTPCall && strings.Contains(e.DstName, "/orders") {
