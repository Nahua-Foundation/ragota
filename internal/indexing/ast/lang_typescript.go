...
func (t *tsExtractor) applyMethodDecorators(fc *fileCtx, n *sitter.Node, idx int, basePath string) {
	...
	for _, d := range tsDecorators(fc, n) {
		...
		case "EventPattern", "MessagePattern", "OnEvent", "Process", "SubscribeMessage":
			...
			fc.addEdge(idx, storage.EdgeConsumes, key, u.StartLine, contract.ConfHigh,
				&storage.EdgeMeta{Topic: name})
		}
	}
}
...
func (t *tsExtractor) handleNew(fc *fileCtx, n *sitter.Node) {
	...
	fc.addEdge(src, storage.EdgeConsumes, key, int(n.StartPoint().Row)+1, contract.ConfHigh,
		&storage.EdgeMeta{Topic: topic})
}
...
func (t *tsExtractor) handleCall(fc *fileCtx, call *sitter.Node) {
	...
	if name == "add" || name == "addBulk" {
		if q, ok := t.queues[lastComponent(object)]; ok {
			key, topic := topicEdgeKey(q)
			fc.addEdge(src, storage.EdgeProduces, key, line, contract.ConfHigh,
				&storage.EdgeMeta{Topic: topic, Args: args, Fields: fields, Aliases: aliases})
	...
	if name != "" {
		fc.addEdge(src, storage.EdgeCall, name, line, contract.ConfHeuristic,
			&storage.EdgeMeta{Args: args, Aliases: aliases, Receiver: object, RecvType: t.types[lastComponent(object)]})
	}
}
