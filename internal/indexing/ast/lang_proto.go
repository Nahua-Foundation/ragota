		...
		func() {
			...
			case strings.HasPrefix(line, "service "):
				...
				if name != "" {
					...
					idx := addUnit(i, storage.KindProtoService, name, q, "")
			...
			case strings.HasPrefix(line, "message "):
				...
				if name != "" {
					...
					idx := addUnit(i, storage.KindProtoMessage, name, qualifiedMsg(full), "")
			...
			case strings.HasPrefix(line, "rpc "):
				...
				idx := addUnit(i, storage.KindRPCMethod, name, q, sig)
				if req != "" {
					fc.addEdge(idx, storage.EdgeRPCRequest, qualifiedMsg(req), i+1, contract.ConfExact, nil)
				}
				if resp != "" {
					fc.addEdge(idx, storage.EdgeRPCResponse, qualifiedMsg(resp), i+1, contract.ConfExact, nil)
				}
			...
			default:
				...
				addUnit(i, storage.KindProtoField, fieldName, q, fieldType)
			}
		}()
...
func addGatewayRoutes(fc *fileCtx, line string, lineNo, rpcIdx int) {
	...
	for _, m := range reGatewayBinding.FindAllStringSubmatch(line, -1) {
		...
		fc.units = append(fc.units, u)
		fc.addEdge(len(fc.units)-1, storage.EdgeHandledBy, rpc.Name, lineNo+1, contract.ConfHigh, nil)
	}
}
