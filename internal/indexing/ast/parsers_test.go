...
func parseOrFail(t *testing.T, lang, path, src string) ([]*storage.ASTUnit, []*storage.Edge) {
	t.Helper()
...
func findUnit(units []*storage.ASTUnit, kind, name string) *storage.ASTUnit {
	for _, u := range units {
...
func findEdge(edges []*storage.Edge, kind, dstName string) *storage.Edge {
	for _, e := range edges {
...
`
	...
	if u := findUnit(units, storage.KindHTTPRoute, "POST /api/v1/orders"); u == nil {
	...
	if findUnit(units, storage.KindHTTPRoute, "GET /health") == nil {
		t.Errorf("missing GET route unit")
	}
	if e := findEdge(edges, storage.EdgeHandledBy, "CreateOrderHandler"); e == nil {
		t.Errorf("missing handled_by edge; edges: %+v", edgeNames(edges))
	}
	if e := findEdge(edges, storage.EdgeCall, "validate"); e == nil {
...
`
	...
	if findEdge(edges, storage.EdgeImplementsRPC, "grpc:OrderService/CreateOrder") == nil {
...
`
	...
	if u := findUnit(units, storage.KindHTTPRoute, "POST /api/billing/charge"); u == nil {
		t.Errorf("missing route unit; units: %+v", names(units))
	}
	if findEdge(edges, storage.EdgeConsumes, "topic:orders.created") == nil {
		t.Errorf("missing consumes edge; edges: %+v", edgeNames(edges))
	}
	e := findEdge(edges, storage.EdgeHTTPCall, "http:POST /api/notify/send")
	if e == nil {
...
`
	...
	if findUnit(units, storage.KindHTTPRoute, "POST /api/notify/send") == nil {
		t.Errorf("missing route unit; units: %+v", names(units))
	}
	if findEdge(edges, storage.EdgeCall, "Deliver") == nil {
		t.Errorf("missing call edge; edges: %+v", edgeNames(edges))
	}
	if findEdge(edges, storage.EdgeHTTPCall, "http:POST /api/audit") == nil {
		t.Errorf("missing http_call edge; edges: %+v", edgeNames(edges))
	}
	if findEdge(edges, storage.EdgeConsumes, "topic:notify.requested") == nil {
		t.Errorf("missing consumes edge; edges: %+v", edgeNames(edges))
	}
}
...
`
	...
	e := findEdge(edges, storage.EdgeHTTPCall, "http:POST /api/v1/orders")
	if e == nil {
	...
	if findUnit(units, storage.KindHTTPRoute, "POST /checkout") == nil {
		t.Errorf("missing route unit; units: %+v", names(units))
	}
	if findEdge(edges, storage.EdgeConsumes, "topic:payments.completed") == nil {
		t.Errorf("missing consumes edge; edges: %+v", edgeNames(edges))
	}
	if findEdge(edges, storage.EdgeProduces, "topic:checkout.started") == nil {
		t.Errorf("missing produces edge; edges: %+v", edgeNames(edges))
	}
	if findEdge(edges, storage.EdgeCall, "createOrder") == nil {
		t.Errorf("missing call edge; edges: %+v", edgeNames(edges))
	}
}
...
`
	...
	if u := findUnit(units, storage.KindRPCMethod, "CreateOrder"); u == nil {
	...
	if u := findUnit(units, storage.KindProtoField, "user_id"); u == nil {
	...
	if findEdge(edges, storage.EdgeRPCRequest, "proto:orders.CreateOrderRequest") == nil {
...
func edgeNames(edges []*storage.Edge) []string {
	var out []string
	for _, e := range edges {
