...
// memStorage is a minimal in-memory storage.Storage implementation covering
...
func (m *memStorage) BatchStoreASTUnits(_ context.Context, units []*storage.ASTUnit) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *memStorage) BatchStoreEdges(_ context.Context, edges []*storage.Edge) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func syntheticGoFiles(n int) []*indexing.FileToIndex {
...
func runASTIndex(t *testing.T, workers, nFiles int) (*indexing.IndexResult, *memStorage) {
	t.Helper()
