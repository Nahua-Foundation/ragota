...
func (r *kafkaProduceRule) apply(fc *fileCtx, cs *callSite, res resolver) (claimed, emitted bool) {
	...
	meta := &storage.EdgeMeta{Topic: name, Fields: cs.Fields, Aliases: cs.Aliases}
	if !r.OmitArgs {
		meta.Args = cs.Args
	}
	fc.addEdge(cs.Src, storage.EdgeProduces, key, cs.Line, r.Conf, meta)
	return true, true
}
...
func (r *kafkaConsumeRule) apply(fc *fileCtx, cs *callSite, res resolver, list listResolver) (claimed, emitted, handled bool) {
	...
	emit := func(topic string) {
		key, name := topicEdgeKey(topic)
		fc.addEdge(cs.Src, storage.EdgeConsumes, key, cs.Line, r.Conf, &storage.EdgeMeta{Topic: name})
		emitted = true
	}
	spec := r.spec()
	if spec.List {
...
func runDetectors(fc *fileCtx, ds *detectorSet, cs *callSite, res resolver, list listResolver) bool {
	...
	for i := range ds.Produce {
		...
		record(storage.ContractKindMessaging, claimed, emitted)
		if emitted {
			return true
		}
	}
	...
	if ds.Broker && !counted[storage.ContractKindMessaging] {
		if claimed, emitted := brokerFallback(fc, cs, res); claimed {
			record(storage.ContractKindMessaging, true, emitted)
			if emitted {
				return true
			}
		}
	...
	for i := range ds.HTTP {
		...
		record(storage.ContractKindHTTP, claimed, emitted)
		if emitted {
			return true
		}
	}
...
func brokerFallback(fc *fileCtx, cs *callSite, res resolver) (claimed, emitted bool) {
	...
	case brokerPublishVerbs[name]:
		kind = storage.EdgeProduces
	case brokerSubscribeVerbs[name]:
		kind = storage.EdgeConsumes
	default:
		return false, false
	}
	...
	for _, expr := range brokerFallbackExprs(cs) {
		...
		fc.addEdge(cs.Src, kind, key, cs.Line, contract.ConfWeak,
			&storage.EdgeMeta{Topic: topic, Args: cs.Args, Fields: cs.Fields, Aliases: cs.Aliases})
		return true, true
	}
	if !named {
