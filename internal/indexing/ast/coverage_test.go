...
`
	...
	if findEdge(edges, storage.EdgeRPCCall, "grpc:OrderService/CreateOrder") == nil {
		t.Fatalf("missing rpc_call edge; edges: %+v", edgeNames(edges))
	}
}
...
`
	...
	if findEdge(edges, storage.EdgeHTTPCall, "http:POST /api/audit/log") == nil {
		t.Errorf("missing got http_call edge; edges: %+v", edgeNames(edges))
	}
	if findEdge(edges, storage.EdgeHTTPCall, "http:POST /api/metrics") == nil {
...
`
	...
	if findEdge(edges, storage.EdgeHTTPCall, "http:GET /api/health") == nil {
...
`
	...
	if findUnit(units, storage.KindHTTPRoute, "POST /api/orders/create") == nil {
	...
	// RestTemplate call.
	if findEdge(edges, storage.EdgeHTTPCall, "http:POST /api/billing/charge") == nil {
		t.Errorf("missing http_call edge; edges: %+v", edgeNames(edges))
	}
	// Plain call edge fallback.
	if findEdge(edges, storage.EdgeCall, "process") == nil {
...
`
	...
	if findEdge(edges, storage.EdgeHTTPCall, "http:POST /api/notify/send") == nil {
		t.Errorf("missing http_call edge; edges: %+v", edgeNames(edges))
	}
}
