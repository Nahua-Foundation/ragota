...
`
	units, edges := parseOrFail(t, "typescript", "user.service.ts", src)

	if u := findUnit(units, storage.KindHTTPRoute, "GET /api/users"); u != nil {
		t.Errorf("client call registered a phantom route: %+v", names(units))
	}
	if findEdge(edges, storage.EdgeHTTPCall, "http:GET /api/users") == nil {
...
`
	...
	for _, want := range []string{"GET /orders", "POST /orders", "PUT /orders/:id"} {
		if findUnit(units, storage.KindHTTPRoute, want) == nil {
			t.Errorf("missing route %q; units: %+v", want, names(units))
		}
	}
	if findEdge(edges, storage.EdgeHandledBy, "createOrder") == nil {
...
`
	...
	f := findUnit(units, storage.KindProtoField, "user_id")
	if f == nil {
	...
	m := findUnit(units, storage.KindRPCMethod, "CreateOrder")
	if m == nil {
...
`
	...
	if findUnit(units, storage.KindHTTPRoute, "GET /v1/orders/{order_id}") == nil {
		t.Errorf("missing gateway GET route; units: %+v", names(units))
	}
	if findUnit(units, storage.KindHTTPRoute, "POST /v1/orders") == nil {
		t.Errorf("missing gateway POST route; units: %+v", names(units))
	}
	if findEdge(edges, storage.EdgeHandledBy, "GetOrder") == nil {
	...
	if findUnit(units, storage.KindProtoField, "id") == nil {
...
`)
	e := findEdge(edges, storage.EdgeRPCRequest, "proto:p.Req")
	if e == nil {
...
`)
	e = findEdge(edges, storage.EdgeWritesTo, "db:orders")
	if e == nil {
...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			e := findEdge(edges, storage.EdgeHTTPCall, tt.want)
			if e == nil {
...
`
	...
	if findUnit(units, storage.KindHTTPRoute, "GET /api/orders") == nil {
...
`
	...
	if findUnit(units, storage.KindHTTPRoute, "GET /api/v1/orders") == nil {
		t.Errorf("missing APIRouter-prefixed route; units: %+v", names(units))
	}
	if findUnit(units, storage.KindHTTPRoute, "POST /api/legacy") == nil {
...
`
	...
	if findUnit(units, storage.KindHTTPRoute, "GET /orders/:id") == nil {
		t.Errorf("missing @Get route; units: %+v", names(units))
	}
	if findUnit(units, storage.KindHTTPRoute, "POST /orders") == nil {
		t.Errorf("missing @Post route; units: %+v", names(units))
	}
	if findEdge(edges, storage.EdgeConsumes, "topic:order.created") == nil {
...
`
	...
	e := findEdge(edges, storage.EdgeProduces, "topic:orders.created")
	if e == nil {
...
`
	...
	if findEdge(edges, storage.EdgeProduces, "topic:orders.created") == nil {
...
`
	...
	for _, want := range []string{"topic:orders.created", "topic:orders.shipped"} {
		if findEdge(edges, storage.EdgeConsumes, want) == nil {
...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, edges := parseOrFail(t, tt.lang, tt.file, tt.src)
			e := findEdge(edges, storage.EdgeRPCCall, "grpc:OrderService/CreateOrder")
			if e == nil {
...
`
	...
	if findEdge(edges, storage.EdgeHTTPCall, "http:GET /api/things") == nil {
...
`
	...
	e := findEdge(edges, storage.EdgeConsumes, "topic:${orders.topic}")
	if e == nil {
		t.Fatalf("topic ref must drop the default so config resolution can match; edges: %+v", edgeNames(edges))
	}
	if got := storage.DecodeEdgeMeta(e.Meta).Topic; got != "orders.created" {
...
func TestSwaggerBasePath(t *testing.T) {
	...
	if findUnit(units, storage.KindHTTPRoute, "POST /api/v2/orders") == nil {
...
`
	...
	if findUnit(units, storage.KindDBTable, "orders") == nil {
		t.Errorf("gorm-tagged struct must publish its table; units: %+v", names(units))
	}
	if findEdge(edges, storage.EdgeWritesTo, "db:orders") == nil {
		t.Errorf("missing writes_to edge; edges: %+v", edgeNames(edges))
	}
	if findEdge(edges, storage.EdgeReadsFrom, "db:orders") == nil {
...
`
	_, edges := parseOrFail(t, "typescript", "orders.service.ts", src)

	if findEdge(edges, storage.EdgeReadsFrom, "db:order") == nil {
		t.Errorf("prisma findMany must read; edges: %+v", edgeNames(edges))
	}
	if findEdge(edges, storage.EdgeWritesTo, "db:order_item") == nil {
...
`
	...
	if findEdge(edges, storage.EdgeConsumes, "topic:process_order") == nil {
		t.Errorf("missing celery task consumes edge; edges: %+v", edgeNames(edges))
	}
	if findEdge(edges, storage.EdgeProduces, "topic:process_order") == nil {
...
`
	...
	if findEdge(edges, storage.EdgeConsumes, "topic:orders.created") == nil {
...
`
	...
	if findEdge(edges, storage.EdgeProduces, "topic:orders") == nil {
		t.Errorf("missing bullmq produces edge; edges: %+v", edgeNames(edges))
	}
	if findEdge(edges, storage.EdgeConsumes, "topic:orders") == nil {
...
`
	...
	e := findEdge(edges, storage.EdgeHTTPCall, "http:POST /api/billing/charge")
	if e == nil {
		t.Fatalf("missing http_call; edges: %+v", edgeNames(edges))
	}
	if m := storage.DecodeEdgeMeta(e.Meta); m.Host != "billing" || m.Method != "POST" {
		t.Errorf("meta = %+v", m)
	}
}
...
`
	...
	if findEdge(edges, storage.EdgeHTTPCall, "http:GET /api/orders") == nil {
...
`
	...
	if e := findEdge(edges, storage.EdgeProduces, "topic:orders.created"); e == nil {
		t.Errorf("a broker publish with a destination literal must still be detected; edges: %v", edgeNames(edges))
	}
	for _, e := range edges {
		if e.Kind == storage.EdgeProduces && e.DstName != "topic:orders.created" {
