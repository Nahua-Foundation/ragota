...
func TestRagotaYAMLOverridesEverything(t *testing.T) {
	root := t.TempDir()
	write(t, root, ".ragota.yaml", "services:\n  - name: api\n    root: backend\n  - name: ui\n    root: frontend\n")
	write(t, root, "docker-compose.yml", "services:\n  other:\n    build: ./other\n")

	svcs, err := Detect(root, "repo")
	if err != nil {
		t.Fatal(err)
	}
	if len(svcs) != 2 {
...
func TestComposeSkipsImageOnlyServices(t *testing.T) {
	root := t.TempDir()
	write(t, root, "docker-compose.yml",
		"services:\n  app:\n    build: ./app\n  db:\n    image: postgres:16\n")
	write(t, root, "app/Dockerfile", "FROM scratch\n")
...
func TestCmdConvention(t *testing.T) {
	root := t.TempDir()
	write(t, root, "cmd/worker/main.go", "package main\n")
...
func TestNestedServicesGetDistinctNames(t *testing.T) {
	root := t.TempDir()
	write(t, root, "services/orders/api/pom.xml", "<project/>\n")
	write(t, root, "services/users/api/pom.xml", "<project/>\n")
	write(t, root, "services/orders/worker/pom.xml", "<project/>\n")
...
func TestDeepManifestIsDetected(t *testing.T) {
	root := t.TempDir()
	write(t, root, "apps/backend/services/orders/api/pom.xml", "<project/>\n")
...
func TestMergeHintsNoHints(t *testing.T) {
	...
	out := MergeHints(detected, nil)
	if len(out) != 1 || out[0] != detected[0] {
		t.Fatalf("MergeHints(detected, nil) = %+v, want detected unchanged", out)
	}
}
