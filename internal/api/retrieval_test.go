...
func hitsFixture(n int) []*indexing.Hit {
	out := make([]*indexing.Hit, 0, n)
	for i := 0; i < n; i++ {
		out = append(out, &indexing.Hit{
			RepoID:   "r1",
...
func TestSearchHitWireShape(t *testing.T) {
	svc := &fakeService{searchRes: &indexing.SearchResult{
...
func degradedResult() *indexing.SearchResult {
...
func contextFixture(n int) *service.ContextResult {
	res := &service.ContextResult{Query: "handler", Mode: "keyword"}
	for _, h := range hitsFixture(n) {
		res.Items = append(res.Items, &service.ContextItem{
			Hit:     h,
			Unit:    &storage.ASTUnit{ID: "u1", RepoID: "r1", FilePath: h.FilePath, Name: "Handle", Kind: "function"},
			Service: "orders",
			Related: []*graph.RelatedUnit{{
				Unit:      &storage.ASTUnit{ID: "u2", RepoID: "r1", FilePath: "src/router.go", Name: "Route"},
				Via:       "call",
				Direction: "in",
				Distance:  1,
			}},
