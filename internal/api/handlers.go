...
// Service is the narrow interface of *service.Service that the API needs.
type Service interface {
	AddRepo(ctx context.Context, src repos.SourceType, req *repos.AddRequest) (*repos.Repo, error)
	ListRepos(ctx context.Context) ([]*repos.Repo, error)
	GetRepo(ctx context.Context, id string) (*repos.Repo, error)
	DeleteRepo(ctx context.Context, id string) error
	StartIndex(ctx context.Context, id string, force bool) (*service.IndexAck, error)
	ResetRepo(ctx context.Context, id string) (*repos.Repo, error)
	Ready(ctx context.Context) error
	Search(ctx context.Context, q *indexing.SearchQuery, mode string) (*indexing.SearchResult, error)
	Symbols(ctx context.Context, opts storage.QueryOpts) ([]*storage.ASTUnit, error)
	Definition(ctx context.Context, repoID, filePath string, line int) (*storage.ASTUnit, error)
	References(ctx context.Context, repoID, filePath string, line, limit int) ([]*storage.Edge, error)
	Stats(ctx context.Context) (map[string]*indexing.IndexerStats, error)
	...
	BuildContext(ctx context.Context, query string, repos []string, mode string, limit, hops int, intent string) (*service.ContextResult, error)
	SyncRepoAsync(ctx context.Context, repoID string, force bool) error
	FindRepoByHints(ctx context.Context, hints []string) (*repos.Repo, error)
	IngestRuntimeServiceGraph(ctx context.Context, edges []service.RuntimeServiceEdge) (*service.RuntimeIngestResult, error)
	ApplyCommits(ctx context.Context, repoID string, commits []service.CommitEvent) (*service.CommitAck, error)
	RepoCoverage(ctx context.Context, repoID string) (*service.CoverageReport, error)
	RepoJobs(ctx context.Context, repoID string, limit int) ([]*storage.IndexJob, error)
	RepoJob(ctx context.Context, repoID, jobID string) (*storage.IndexJob, error)
...
func decode[T any](r *http.Request) (*T, error) {
	...
	if err := json.NewDecoder(r.Body).Decode(&v); err != nil {
		...
		return nil, fmt.Errorf("%w: invalid request body", service.ErrBadRequest)
	}
	return &v, nil
}
...
func writeError(w http.ResponseWriter, err error) {
	...
	case errors.Is(err, service.ErrRepoBusy):
	...
	case errors.Is(err, indexing.ErrIndexDamaged):
...
func parseSnippetMode(mode string) (string, error) {
	...
	default:
		return "", fmt.Errorf("%w: unknown snippet mode %q (want %q, %q or %q)",
			service.ErrBadRequest, mode, SnippetChunk, SnippetLine, SnippetNone)
	}
}
...
func checkMaxBytes(n int) error {
	if n < 0 {
		return fmt.Errorf("%w: max_bytes must not be negative", service.ErrBadRequest)
	}
	return nil
}
...
func queryLimit(raw string) (int, error) {
	...
	if err != nil || n <= 0 {
		return 0, fmt.Errorf("%w: limit must be a positive integer", service.ErrBadRequest)
	}
	return n, nil
}
...
func toSymbol(u *storage.ASTUnit) ASTSymbol {
	return ASTSymbol{
...
func (s *Server) HandleAddRepo(w http.ResponseWriter, r *http.Request) {
	...
	if req.Name == "" {
		writeError(w, fmt.Errorf("%w: name is required", service.ErrBadRequest))
		return
	}
	if req.Source == "local" && req.Path == "" {
		writeError(w, fmt.Errorf("%w: path is required for local source", service.ErrBadRequest))
		return
	}
	if req.Source == "git" && req.URL == "" {
		writeError(w, fmt.Errorf("%w: url is required for git source", service.ErrBadRequest))
		return
	}
	repo, err := s.svc.AddRepo(r.Context(), repos.SourceType(req.Source), &repos.AddRequest{
...
func (s *Server) HandleIndex(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
		writeError(w, fmt.Errorf("%w: repo id is required", service.ErrBadRequest))
		return
	}
	...
	if r.ContentLength != 0 {
		if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
			...
			writeError(w, fmt.Errorf("%w: invalid request body", service.ErrBadRequest))
			return
		}
	}
...
func (s *Server) HandleRepoCommits(w http.ResponseWriter, r *http.Request) {
	...
	if len(req.Commits) == 0 {
		writeError(w, fmt.Errorf("%w: commits are required", service.ErrBadRequest))
		return
	}
	commits := make([]service.CommitEvent, len(req.Commits))
	for i, c := range req.Commits {
		files := make([]service.CommitFile, len(c.Files))
		for j, f := range c.Files {
			files[j] = service.CommitFile{Path: f.Path, OldPath: f.OldPath, Status: f.Status, Content: f.Content}
		}
		commits[i] = service.CommitEvent{SHA: c.SHA, Parents: c.Parents, Files: files}
	}
...
func (s *Server) HandleRepoJob(w http.ResponseWriter, r *http.Request) {
	...
	if id == "" || jobID == "" {
		writeError(w, fmt.Errorf("%w: repo id and job id are required", service.ErrBadRequest))
		return
	}
...
func (s *Server) HandleSearch(w http.ResponseWriter, r *http.Request) {
	...
	if req.Query == "" {
		writeError(w, fmt.Errorf("%w: query is required", service.ErrBadRequest))
		return
	}
	...
	q := &indexing.SearchQuery{Query: req.Query, Repos: req.Repos, Limit: req.Limit, Filter: req.Filter, Intent: req.Intent}
...
// service.References — it used to bound each of the endpoint's two lookups
...
func (s *Server) HandleGraphNeighbors(w http.ResponseWriter, r *http.Request) {
	...
	if req.UnitID == "" {
		writeError(w, fmt.Errorf("%w: unit_id is required", service.ErrBadRequest))
		return
	}
...
func (s *Server) HandleGraphPath(w http.ResponseWriter, r *http.Request) {
	...
	if req.FromUnitID == "" || req.ToUnitID == "" {
		writeError(w, fmt.Errorf("%w: from_unit_id and to_unit_id are required", service.ErrBadRequest))
		return
	}
...
func (s *Server) HandleGraphTrace(w http.ResponseWriter, r *http.Request) {
	...
	if req.Symbol == "" || req.Param == "" {
		writeError(w, fmt.Errorf("%w: symbol and param are required", service.ErrBadRequest))
		return
	}
...
func (s *Server) HandleGitWebhook(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		writeError(w, fmt.Errorf("%w: cannot read request body", service.ErrBadRequest))
		return
	}
	...
	hints := payload.hints()
	if len(hints) == 0 {
		writeError(w, fmt.Errorf("%w: no repository identifiers in payload", service.ErrBadRequest))
		return
	}
