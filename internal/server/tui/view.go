...
func (f frame) activity() cell {
	...
	for _, r := range f.snap.Repos {
		switch r.Phase {
		case progress.PhaseIndexing:
			indexing = append(indexing, r.Name)
		case progress.PhaseQueued:
			queued++
		}
		if r.Watched {
			watched++
...
func layout(width int, repos []progress.RepoState) cols {
	longest := 0
	for _, r := range repos {
...
func (f frame) repoSection(repos []progress.RepoState, rows int) []string {
	if rows < 2 {
		return nil
	}
...
func (f frame) repoRow(c cols, r progress.RepoState) string {
	watch := cell{}
	if r.Watched {
...
func (f frame) statusCell(r progress.RepoState, w int) cell {
	switch r.Phase {
	case progress.PhaseIndexing:
		if r.Total <= 0 {
...
func (f frame) logSection(entries []progress.Entry, rows int) []string {
	if rows < 2 {
		return nil
	}
	head := "RECENT"
...
func (f frame) logRow(e progress.Entry, names map[string]string) string {
	var b strings.Builder
	rest := f.width
...
// view needs.
func (f frame) entries() []progress.Entry {
	if !f.problems {
		return f.snap.Log
	}
	out := make([]progress.Entry, 0, len(f.snap.Log))
...
func orderRepos(in []progress.RepoState) []progress.RepoState {
	out := append([]progress.RepoState(nil), in...)
...
func phaseRank(p progress.Phase) int {
	switch p {
	case progress.PhaseIndexing:
		return 0
	case progress.PhaseError:
...
func phaseStyle(p progress.Phase) lipgloss.Style {
	switch p {
	case progress.PhaseIndexing:
		return runStyle
	case progress.PhaseError:
...
func more(hidden []progress.RepoState) string {
	counts := map[progress.Phase]int{}
	for _, r := range hidden {
		counts[r.Phase]++
	}
	var parts []string
	for _, p := range []progress.Phase{progress.PhaseIndexing, progress.PhaseError, progress.PhaseQueued, progress.PhaseIdle} {
		if n := counts[p]; n > 0 {
