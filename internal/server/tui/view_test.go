...
func sampleFrame() frame {
	return frame{
		snap: progress.Snapshot{
			...
			Repos: []progress.RepoState{
				{ID: "a", Name: "alpha", Path: "/home/dev/projects/alpha", Phase: progress.PhaseIndexing,
					Files: 412, Total: 1000, Watched: true, StartedAt: testNow.Add(-20 * time.Second)},
				{ID: "b", Name: "beta", Path: "/home/dev/projects/beta", Phase: progress.PhaseIdle,
					Indexed: 1204, Watched: true, StartedAt: testNow.Add(-5 * time.Minute), FinishedAt: testNow.Add(-2 * time.Minute)},
				{ID: "c", Name: "gamma", Path: "/home/dev/projects/gamma", Phase: progress.PhaseError,
					Failed: 3, Err: "walk: permission denied", StartedAt: testNow.Add(-9 * time.Minute), FinishedAt: testNow.Add(-8 * time.Minute)},
				{ID: "d", Name: "delta", Path: "/home/dev/projects/delta", Phase: progress.PhaseQueued},
				{ID: "e", Name: "epsilon", Path: "/srv/other/epsilon", Phase: progress.PhaseIdle},
				{ID: "f", Name: "zeta", Path: "/home/dev/projects/zeta", Phase: progress.PhaseIdle,
...
func TestRenderFitsTheTerminal(t *testing.T) {
	base := sampleFrame()
	base.snap.Repos = append(base.snap.Repos, progress.RepoState{
		ID:    "long",
	...
	base.snap.Log = append(base.snap.Log, progress.Entry{
...
func TestRenderKeepsTheActiveRepositoriesOnScreen(t *testing.T) {
	...
	f.height = 14
	for i := 0; i < 40; i++ {
		f.snap.Repos = append(f.snap.Repos, progress.RepoState{
			ID: string(rune('A' + i%26)), Name: "idle-repo", Phase: progress.PhaseIdle,
		})
	}
	out := render(f)
...
func TestRenderEmptySnapshot(t *testing.T) {
	f := frame{snap: (*progress.Bus)(nil).Snapshot(), width: 80, height: 24, now: testNow}
	out := render(f)
	...
	if got := render(frame{snap: progress.Snapshot{}, width: 0, height: 24}); !strings.Contains(got, "ragota") {
