// Package bootstrap wires the process: it builds the store, indexers, services
// and transports from a config.Config, and is the only place that knows how the
...
import (
	...
	"github.com/Nahua-Foundation/ragota/internal/index/bm25"
	"github.com/Nahua-Foundation/ragota/internal/index/vector"
	...
	"github.com/Nahua-Foundation/ragota/internal/store/postgres"
	"github.com/Nahua-Foundation/ragota/internal/store/qdrant"
...
func Build(ctx context.Context, cfg *config.Config) (_ *app.Service, retErr error) {
	...
	var indexers = make(map[index.IndexType]index.Indexer)
	...
	default:
		...
		// units and edges.
		return nil, fmt.Errorf("no relational storage backend configured (store.sqlite or store.postgres required)")
	}
	...
	if cfg.Indexes.AST != nil && cfg.Indexes.AST.Enabled {
		...
		indexers[index.IndexTypeAST] = idx
	}
	...
	if cfg.Indexes.Vector != nil && cfg.Indexes.Vector.Enabled {
		...
		indexers[index.IndexTypeVector] = idx
	}
	...
	if cfg.Indexes.BM25 != nil && cfg.Indexes.BM25.Enabled {
		...
		indexers[index.IndexTypeBM25] = idx
	}
	...
	if cfg.LSP != nil && cfg.LSP.Enabled {
		...
		if astIdx, ok := indexers[index.IndexTypeAST]; ok {
			indexers[index.IndexTypeAST] = lsp.Chain(astIdx, refiner)
		} else {
			indexers[index.IndexTypeCustom] = refiner
		}
	}
	...
	// Initialize sources
	sources := make(map[domain.SourceType]repos.RepoSource)

	if cfg.Repos.Sources.Local != nil && cfg.Repos.Sources.Local.Enabled {
		src := initLocalSource(cfg)
		sources[domain.SourceTypeLocal] = src
	}
	...
	if cfg.Repos.Sources.Git != nil && cfg.Repos.Sources.Git.Enabled {
		...
		sources[domain.SourceTypeGit] = src
	}

	// Build search service from indexers that implement Searcher
	searchers := make(map[index.IndexType]index.Searcher)
	for _, idx := range indexers {
		if srch, ok := idx.(index.Searcher); ok {
	...
	svc := app.New(cfg, stor, indexers, sources, searchSvc)
	if callRefiner != nil {
	...
	if cfg.Models.Assistant != nil {
		...
		// same assistant generator (see internal/app/recon.go).
...
// initSQLite initializes SQLite store.
...
// initQdrant initializes Qdrant vector store. store.qdrant.mode selects
...
func initQdrant(cfg *config.Config, ctx context.Context) (*qdrant.Qdrant, error) {
	...
	if mode == "cloud" && cfg.Storage.Qdrant.APIKey == "" {
		return nil, fmt.Errorf("store.qdrant.api_key is required when store.qdrant.mode is cloud")
	}
...
func initASTIndexer(cfg *config.Config, stor store.Storage) index.Indexer {
	...
	parserSkippedLangs.Add(float64(len(skipped)))
...
func initVectorIndexer(cfg *config.Config, stor store.Storage, embedders map[string]llm.Embedder) (index.Indexer, error) {
...
func initBM25Indexer(cfg *config.Config) (index.Indexer, error) {
