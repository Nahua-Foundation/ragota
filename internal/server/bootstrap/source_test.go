...
// a fully wired Service, which internal/servertest builds by calling bootstrap.Build
...
func TestApplySourceAddsToTheLocalPaths(t *testing.T) {
	...
	abs, err := bootstrap.ApplySource(cfg, dir)
	if err != nil {
...
func TestApplySourceKeepsConfiguredPaths(t *testing.T) {
	...
	if _, err := bootstrap.ApplySource(cfg, dir); err != nil {
		t.Fatalf("ApplySource() error = %v", err)
	}
...
func buildService(t *testing.T, root string) *app.Service {
	t.Helper()
...
func TestDiscoverAndRegisterIsIdempotent(t *testing.T) {
	...
	first, err := bootstrap.DiscoverAndRegister(ctx, svc, root, nil)
	if err != nil {
	...
	second, err := bootstrap.DiscoverAndRegister(ctx, svc, root, nil)
	if err != nil {
...
func TestDiscoverAndRegisterWithoutABusDoesNotPanic(t *testing.T) {
	...
	svc := buildService(t, root)
	got, err := bootstrap.DiscoverAndRegister(context.Background(), svc, root, nil)
	if err != nil {
		t.Fatalf("DiscoverAndRegister() error = %v", err)
	}
...
func TestActivateOnlyNarrowsTheWorkingSet(t *testing.T) {
	...
	found, err := bootstrap.DiscoverAndRegister(ctx, svc, root, nil)
	if err != nil {
		t.Fatalf("DiscoverAndRegister() error = %v", err)
	}
	if len(found) < 2 {
	...
	if err := bootstrap.ActivateOnly(ctx, svc, found[:1]); err != nil {
...
func ids(list []*domain.Repo) []string {
