...
func TestRepoBusyIsConflictWithRetryAfter(t *testing.T) {
	svc := &fakeService{
		repos:    map[string]*domain.Repo{"r1": {ID: "r1"}},
		indexErr: fmt.Errorf("%w: r1", app.ErrRepoBusy),
	}
	srv := newTestServer(t, svc, nil)
...
func TestInvalidPathCode(t *testing.T) {
	svc := &fakeService{
		repos:      map[string]*domain.Repo{"r1": {ID: "r1"}},
		commitsErr: fmt.Errorf("%w: ../etc/passwd", app.ErrInvalidPath),
	}
	srv := newTestServer(t, svc, nil)
...
func TestCommitGapCode(t *testing.T) {
	no := false
	svc := &fakeService{
		repos:     map[string]*domain.Repo{"r1": {ID: "r1", LastCommit: "sha-old"}},
		commitsOK: &no,
	}
	srv := newTestServer(t, svc, nil)
...
func TestCommitsAcceptLargeBody(t *testing.T) {
	svc := &fakeService{repos: map[string]*domain.Repo{"r1": {ID: "r1"}}}
	srv := newTestServer(t, svc, nil)
...
func TestIndexAckReportsQueueState(t *testing.T) {
	svc := &fakeService{
		...
		indexAck: &app.IndexAck{
			...
			JobStatus: domain.JobStatusPending, Force: true,
		},
	}
	srv := newTestServer(t, svc, nil)
	...
	if ack["status"] != "queued" || ack["job_id"] != "42" || ack["job_status"] != domain.JobStatusPending {
...
func TestSyncStateReportsInFlightBatch(t *testing.T) {
	svc := &fakeService{repos: map[string]*domain.Repo{"r1": {
		ID: "r1", Status: domain.StatusIndexing, LastCommit: "sha-1",
...
func TestCommitAckReportsQueueState(t *testing.T) {
	svc := &fakeService{
		repos:     map[string]*domain.Repo{"r1": {ID: "r1", LastCommit: "sha-1"}},
		commitAck: &app.CommitAck{Accepted: true, Status: "queued", Queued: true, JobID: "42", Target: "sha-2", Before: "sha-1"},
	}
	srv := newTestServer(t, svc, nil)
...
func TestRepoJobsEndpoint(t *testing.T) {
	svc := &fakeService{
		...
		jobs: []*domain.IndexJob{
			{ID: "2", RepoID: "r1", Kind: domain.JobKindCommits, Status: domain.JobStatusRunning,
				CreatedAt: 10, ClaimedAt: 11, HeartbeatAt: 12, ClaimedBy: "worker-a",
				Payload: `[{"sha":"sha-1"}]`},
			{ID: "1", RepoID: "r1", Kind: domain.JobKindIndex, Force: true,
				Status: domain.JobStatusError, Error: "boom", CreatedAt: 5},
		},
	}
	srv := newTestServer(t, svc, nil)
	...
	running := body.Jobs[0]
	for field, want := range map[string]any{
		"id": "2", "repo_id": "r1", "kind": domain.JobKindCommits,
		"status": domain.JobStatusRunning, "claimed_by": "worker-a",
...
func TestRepoJobEndpoint(t *testing.T) {
	svc := &fakeService{
		repos: map[string]*domain.Repo{"r1": {ID: "r1"}, "r2": {ID: "r2"}},
		jobs: []*domain.IndexJob{
			{ID: "7", RepoID: "r1", Kind: domain.JobKindIndex, Status: domain.JobStatusPending, CreatedAt: 3},
			{ID: "8", RepoID: "r2", Kind: domain.JobKindIndex, Status: domain.JobStatusPending, CreatedAt: 4},
		},
	}
	srv := newTestServer(t, svc, nil)
	...
	if job["id"] != "7" || job["status"] != domain.JobStatusPending {
...
func TestResetRepoEndpoint(t *testing.T) {
	svc := &fakeService{repos: map[string]*domain.Repo{"r1": {ID: "r1", Status: domain.StatusIndexing}}}
	srv := newTestServer(t, svc, nil)
	...
	if repo["status"] != string(domain.StatusIdle) {
...
func TestGraphPathNoPathIs200(t *testing.T) {
	svc := &fakeService{pathErr: store.ErrNotFound}
	srv := newTestServer(t, svc, nil)
...
func TestWebhookFailsClosedWithoutSecret(t *testing.T) {
	...
	svc := &fakeService{repos: map[string]*domain.Repo{"r1": {ID: "r1", Name: "gateway"}}}
	srv := newTestServer(t, svc, nil)
