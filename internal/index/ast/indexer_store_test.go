...
func indexFiles(t *testing.T, st *memStorage, files []*index.FileToIndex) *index.IndexResult {
	t.Helper()
	idx := New(&Config{Storage: st, Workers: 4})
	idx.RegisterParser(NewTreeSitterParser("go"))
	res, err := idx.Index(context.Background(), &index.IndexRequest{RepoID: "r", Files: files})
	if err != nil {
...
func TestParseStagePublishesSymbols(t *testing.T) {
	...
	idx.RegisterParser(NewTreeSitterParser("go"))
	if _, err := idx.Index(context.Background(), &index.IndexRequest{RepoID: "r-publish", Files: files}); err != nil {
		t.Fatalf("Index: %v", err)
	}

	for _, f := range files {
		syms, ok := index.SharedSymbols.Take("r-publish", f.Path, f.Hash)
		if !ok {
...
func TestParseStageDoesNotPublishUnannotatedLanguages(t *testing.T) {
	...
	if _, err := idx.Index(context.Background(), &index.IndexRequest{
		RepoID: "r-yaml", Files: []*index.FileToIndex{file},
	}); err != nil {
		t.Fatalf("Index: %v", err)
	}

	if _, ok := index.SharedSymbols.Take("r-yaml", file.Path, file.Hash); ok {
