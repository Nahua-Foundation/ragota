	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, want := range tt.writes {
				if findEdge(edges, store.EdgeWritesTo, want) == nil {
					t.Errorf("writes_to %q missing from %v", want, edgeNamesOfKind(edges, store.EdgeWritesTo))
				}
			}
			for _, want := range tt.reads {
				if findEdge(edges, store.EdgeReadsFrom, want) == nil {
					t.Errorf("reads_from %q missing from %v", want, edgeNamesOfKind(edges, store.EdgeReadsFrom))
				}
			}
			for _, not := range tt.absent {
				for _, kind := range []string{store.EdgeWritesTo, store.EdgeReadsFrom} {
...
`
	files := []*index.FileToIndex{
		...
		{Path: "internal/store/inmem/store.go", Language: "go", Content: []byte(other)},
	}
	...
	for _, e := range mem.edges {
		if e.Kind != store.EdgeReadsFrom && e.Kind != store.EdgeWritesTo {
			continue
		}
		got[e.Kind+" "+e.DstName] = e.FilePath
		if e.FilePath == "internal/store/inmem/store.go" {
	...
	want := domain.CoverageCounts{Candidates: 4, Edges: 3}
	if got := result.ContractCoverage()[domain.ContractKindDB]; got != want {
...
`
	...
	if got := facts.Coverage[domain.ContractKindDB]; got != want {
		t.Errorf("db coverage = %+v, want %+v", got, want)
	}
}
