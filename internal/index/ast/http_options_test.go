...
func TestHTTPOptionsObjectTarget(t *testing.T) {
  ...
  return await this.helpers.httpRequest({
    method: 'POST',
    url: '/b/my-bucket/o',
    baseURL: 'https://store.googleapis.com/upload/storage/v1',
    json: true,
  });
}
`,
			key:  "http:POST /upload/storage/v1/b/my-bucket/o",
			host: "store.googleapis.com",
		},
		{
...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			e := findEdge(edges, store.EdgeHTTPCall, tt.key)
			if e == nil {
				t.Fatalf("missing http_call %q; edges: %+v", tt.key, edgeNames(edges))
			}
			if meta := store.DecodeEdgeMeta(e.Meta); meta.Host != tt.host {
