...
func (c *csharpExtractor) handleMethod(fc *fileCtx, n *sitter.Node, scope, basePath string) {
	...
	if c.rpcSvc != "" && csharpIsRPCImpl(fc, n, sig) {
		fc.addEdge(idx, store.EdgeImplementsRPC, grpcKey(c.rpcSvc, name), line, contract.ConfCrossFile, nil)
	}
	if c.eventTopic != "" && csharpEventHandlerMethods[name] {
		key, topic := topicEdgeKey(c.eventTopic)
		fc.addEdge(idx, store.EdgeConsumes, key, line, contract.ConfCrossFile, &store.EdgeMeta{Topic: topic})
...
func (c *csharpExtractor) handleObjectCreation(fc *fileCtx, n *sitter.Node) {
	...
	fc.addEdge(src, store.EdgeHTTPCall, routeKey(method, path), line, conf,
		&store.EdgeMeta{Method: method, Path: path, Host: host, Args: args,
...
func (c *csharpExtractor) handleInvocation(fc *fileCtx, n *sitter.Node) {
	...
	if svc != "" && name != "" {
		...
		if method != "" {
			fc.addEdge(src, store.EdgeRPCCall, grpcKey(svc, capitalizeFirst(method)), line, contract.ConfHigh,
	...
	if eventPublishVerbs[name] && len(args) > 0 &&
		...
		if topic := eventTopicName(c.argType(args[0])); topic != "" {
			...
			fc.addEdge(src, store.EdgeProduces, key, line, contract.ConfCrossFile,
				&store.EdgeMeta{Topic: t, Args: args, Aliases: aliases})
			fc.contractSite(domain.ContractKindMessaging, true)
			return
		}
	}
	...
	if kind, topic, handler, ok := genericTopicContract(name, csharpTypeArgs(fc, fn)); ok && brokerHandle(fc, cs) {
		key, t := topicEdgeKey(topic)
		meta := &store.EdgeMeta{Topic: t, Args: args, Aliases: aliases, Receiver: object}
		if handler != "" {
	...
	if name != "" {
		if svc := grpcStubService(c.types[lastComponent(object)], fc.hasGRPC() || len(c.clients) > 0); svc != "" {
			method := strings.TrimSuffix(name, "Async")
			if method != "" {
				fc.addEdge(src, store.EdgeRPCCall, grpcKey(svc, capitalizeFirst(method)), line, contract.ConfCrossFile,
					&store.EdgeMeta{Args: args, Aliases: aliases})
				fc.contractSite(domain.ContractKindRPC, true)
				return
			}
	...
	fc.addEdge(src, store.EdgeCall, name, line, contract.ConfHeuristic,
		&store.EdgeMeta{Args: args, Aliases: aliases, Receiver: object, RecvType: c.types[lastComponent(object)]})
}
...
func (c *csharpExtractor) addEFTable(fc *fileCtx, n *sitter.Node, entity string) {
	...
	fc.addUnit(n, store.KindDBTable, tbl, contract.DB(tbl), "entity:"+entity,
