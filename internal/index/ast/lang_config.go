...
func (p *configParser) Parse(filePath, content string) ([]*domain.ASTUnit, []*domain.Edge, error) {
...
func openAPIRoutes(doc any) []*domain.ASTUnit {
...
func asyncAPIChannels(doc any) []*domain.ASTUnit {
	root := asStringMap(doc)
	if root == nil {
		return nil
	}
	...
	units := make([]*domain.ASTUnit, 0, len(infos))
