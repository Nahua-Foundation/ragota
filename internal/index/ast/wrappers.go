...
func newWrapper(lang string, u *domain.ASTUnit, site httpSite) (wrapper, bool) {
...
func linkWrapperCalls(edges []*domain.Edge, byName map[string]wrapper) (int, []*domain.Edge) {
	if len(byName) == 0 {
		return 0, edges
	}
	type siteKey struct {
	...
	for _, e := range edges {
		...
		edges = append(edges, &domain.Edge{
			SrcID:      e.SrcID,
			Kind:       store.EdgeHTTPCall,
			DstName:    routeKey(method, route),
			Line:       e.Line,
			Confidence: contract.ConfWeak,
			Meta: store.EncodeEdgeMeta(&store.EdgeMeta{
