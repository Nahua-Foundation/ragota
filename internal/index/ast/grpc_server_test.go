...
`,
		...
		},
		{
			...
			src: `namespace cartapp.services;
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, want := range tt.want {
				if findEdge(facts.Edges, store.EdgeImplementsRPC, want) == nil {
					t.Errorf("implements_rpc %q missing from %v", want, edgeNamesOfKind(facts.Edges, store.EdgeImplementsRPC))
				}
			}
			for _, not := range tt.not {
				if findEdge(facts.Edges, store.EdgeImplementsRPC, not) != nil {
...
func edgeNamesOfKind(edges []*domain.Edge, kind string) []string {
	var out []string
	for _, e := range edges {
		if e.Kind == kind {
