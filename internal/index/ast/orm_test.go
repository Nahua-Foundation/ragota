...
`,
		...
		},
		{
			...
			lang: "typescript", file: "order.app.ts",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, want := range tt.writes {
				if findEdge(facts.Edges, store.EdgeWritesTo, want) == nil {
					t.Errorf("writes_to %q missing from %v", want, edgeNamesOfKind(facts.Edges, store.EdgeWritesTo))
				}
			}
			for _, want := range tt.reads {
				if findEdge(facts.Edges, store.EdgeReadsFrom, want) == nil {
					t.Errorf("reads_from %q missing from %v", want, edgeNamesOfKind(facts.Edges, store.EdgeReadsFrom))
				}
			}
...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			facts := parseFactsOrFail(t, tt.lang, tt.file, tt.src)
			var got []string
			byKey := map[string]*domain.ASTUnit{}
			for _, u := range facts.Units {
				if u.Kind != store.KindDBTable {
					continue
				}
				byKey[u.Qualified] = u
