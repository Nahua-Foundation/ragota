	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, want := range tt.keys {
				if findEdge(edges, store.EdgeHTTPCall, want) == nil {
					t.Errorf("http_call %q missing from %v", want, edgeNamesOfKind(edges, store.EdgeHTTPCall))
				}
			}
			for _, not := range tt.absent {
				if findEdge(edges, store.EdgeHTTPCall, not) != nil {
