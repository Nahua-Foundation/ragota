...
func (p *sqlParser) Parse(filePath, content string) ([]*domain.ASTUnit, []*domain.Edge, error) {
	...
	for i, raw := range lines {
		...
		if m := reCreateTable.FindStringSubmatch(line); m != nil {
			...
			tableStart = i
			addUnit(i, store.KindDBTable, currentTable, "db:"+currentTable, "")
			continue
		}
		if m := reAlterAdd.FindStringSubmatch(line); m != nil {
			table := sqlTableName(m[1])
			col := strings.ToLower(m[2])
			addUnit(i, store.KindDBColumn, col, "db:"+table+"."+col, m[3])
			continue
		}
		...
		if strings.HasPrefix(line, ")") {
			...
			for _, u := range units {
				if u.Kind == store.KindDBTable && u.Name == currentTable && u.StartLine == tableStart+1 {
					u.EndLine = i + 1
				}
			}
			currentTable = ""
		...
		addUnit(i, store.KindDBColumn, strings.ToLower(name), "db:"+currentTable+"."+strings.ToLower(name), colType)
	}
...
func sqlEdgesFromArgs(fc *fileCtx, src int, line int, args []string) bool {
	...
	defer func() {
		if candidate || emitted {
			fc.contractSite(domain.ContractKindDB, emitted)
		}
	}()
	for _, arg := range args {
		...
		if m := reInsert.FindStringSubmatch(lit); m != nil {
			...
			fc.addEdge(src, store.EdgeWritesTo, contract.DB(table), line, contract.ConfHigh,
				&store.EdgeMeta{Args: args, Fields: fields})
			emitted = true
			continue
		}
		...
		if m := reDelete.FindStringSubmatch(lit); m != nil {
			table := sqlTableName(m[1])
			fc.addEdge(src, store.EdgeWritesTo, contract.DB(table), line, contract.ConfHigh, &store.EdgeMeta{Args: args})
			emitted = true
			continue
		}
		if strings.Contains(upper, "SELECT") {
			for _, m := range reSelect.FindAllStringSubmatch(lit, -1) {
				table := sqlTableName(m[1])
				fc.addEdge(src, store.EdgeReadsFrom, contract.DB(table), line, contract.ConfHigh, &store.EdgeMeta{Args: args})
				emitted = true
			}
		}
	}
