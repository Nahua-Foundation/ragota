...
func newTestCtx() *fileCtx {
	...
	fc.units = append(fc.units, &domain.ASTUnit{Kind: "function", Name: "caller"})
	return fc
}
...
func TestHTTPClientRuleMatch(t *testing.T) {
	...
	e := fc.edges[0]
	if e.Kind != store.EdgeHTTPCall || e.DstName != "http:POST /api/orders" {
...
func TestKafkaProduceRuleTopicArg(t *testing.T) {
	...
	e := fc.edges[0]
	if e.Kind != store.EdgeProduces || e.DstName != "topic:orders.created" || e.Confidence != contract.ConfHigh {
		t.Errorf("edge = %+v", e)
	}
	if m := store.DecodeEdgeMeta(e.Meta); m.Topic != "orders.created" || len(m.Args) != 2 {
		t.Errorf("meta = %+v", m)
	}
}
...
func TestKafkaProduceRuleTopicFromFieldOmitArgs(t *testing.T) {
	...
	m := store.DecodeEdgeMeta(fc.edges[0].Meta)
...
func TestNewFrameworkAsDataRow(t *testing.T) {
	...
	e := fc.edges[0]
	if e.Kind != store.EdgeHTTPCall || e.DstName != "http:GET /api/users/1" {
		t.Errorf("edge = %s -> %s", e.Kind, e.DstName)
	}
	if m := store.DecodeEdgeMeta(e.Meta); m.Host != "users" {
		t.Errorf("meta = %+v", m)
	}
}
...
func TestJavaPutDeleteNeedsHTTPClientReceiver(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if findEdge(edges, store.EdgeHTTPCall, tt.want) == nil {
