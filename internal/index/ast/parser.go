...
// units are persisted.
func (p *TreeSitterParser) Parse(filePath, content string) ([]*domain.ASTUnit, []*domain.Edge, error) {
...
// The list comes from config, the same single source repos.DetectLanguage
// uses, so a parser registered here is always reachable from a detected file.
