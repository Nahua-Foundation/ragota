	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, want := range tt.produces {
				if findEdge(facts.Edges, store.EdgeProduces, want) == nil {
					t.Errorf("produces %q missing from %v", want, edgeNamesOfKind(facts.Edges, store.EdgeProduces))
				}
			}
			for _, want := range tt.consumes {
				if findEdge(facts.Edges, store.EdgeConsumes, want) == nil {
					t.Errorf("consumes %q missing from %v", want, edgeNamesOfKind(facts.Edges, store.EdgeConsumes))
				}
			}
			for _, not := range tt.absent {
				for _, kind := range []string{store.EdgeProduces, store.EdgeConsumes} {
...
`
	...
	if findEdge(pub.Edges, store.EdgeProduces, key) == nil {
		t.Errorf("produces %q missing from %v", key, edgeNamesOfKind(pub.Edges, store.EdgeProduces))
	}
	// The abstract base names no destination — the routing key is the runtime
	// type — so it must not become a topic of its own.
	if findEdge(pub.Edges, store.EdgeProduces, topicKey("IntegrationEvent")) != nil {
	...
	if findEdge(sub.Edges, store.EdgeConsumes, key) == nil {
		t.Errorf("consumes %q missing from %v", key, edgeNamesOfKind(sub.Edges, store.EdgeConsumes))
	}
}
...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, want := range tt.produces {
				e := findEdge(facts.Edges, store.EdgeProduces, want)
				if e == nil {
					t.Fatalf("produces %q missing from %v", want, edgeNamesOfKind(facts.Edges, store.EdgeProduces))
				}
				if e.Confidence >= 0.7 {
...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			facts := parseFactsOrFail(t, tt.lang, tt.file, tt.src)
			got := facts.Coverage[domain.ContractKindMessaging]
			want := domain.CoverageCounts{Candidates: tt.candidates, Edges: tt.edges}
			if got != want {
...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, want := range tt.consumes {
				e := findEdge(facts.Edges, store.EdgeConsumes, want)
				if e == nil {
					t.Fatalf("consumes %q missing from %v", want, edgeNamesOfKind(facts.Edges, store.EdgeConsumes))
				}
				if got := store.DecodeEdgeMeta(e.Meta).Fields["handler"]; got != tt.handler {
...
func TestGenericTopicContract(t *testing.T) {
	tests := []struct {
		...
		{name: "subscription", call: "AddSubscription", typeArgs: []string{"OrderPaidIntegrationEvent", "OrderPaidIntegrationEventHandler"},
			kind: store.EdgeConsumes, topic: "OrderPaidIntegrationEvent", handler: "OrderPaidIntegrationEventHandler"},
		{name: "message second", call: "Register", typeArgs: []string{"CatalogViewModel", "ProductCountChangedMessage"},
			kind: store.EdgeConsumes, topic: "ProductCountChangedMessage"},
		{name: "publish", call: "PublishAsync", typeArgs: []string{"eShop.Events.OrderShippedEvent"},
			kind: store.EdgeProduces, topic: "OrderShippedEvent"},
...
`
	...
	for _, not := range []string{topicKey("HttpRequestMessage"), topicKey("OutboundKeepAliveMessage")} {
		if findEdge(facts.Edges, store.EdgeProduces, not) != nil {
			t.Errorf("produces %q was emitted and should not be", not)
		}
	}
	if findEdge(facts.Edges, store.EdgeProduces, topicKey("CancelOrderCommand")) == nil {
		t.Errorf("a mediator send is still a dispatch: %v", edgeNamesOfKind(facts.Edges, store.EdgeProduces))
	}
}
...
`
	...
	if findEdge(facts.Edges, store.EdgeConsumes, topicKey("process_order")) == nil {
		t.Errorf("celery task consumer missing from %v", edgeNamesOfKind(facts.Edges, store.EdgeConsumes))
	}
	...
	for _, not := range []string{topicKey("produce_asset_events"), topicKey("consume_asset_event")} {
		if findEdge(facts.Edges, store.EdgeConsumes, not) != nil {
...
`
	...
	if findEdge(facts.Edges, store.EdgeProduces, topicKey("process_order")) == nil {
		t.Errorf("celery dispatch missing from %v", edgeNamesOfKind(facts.Edges, store.EdgeProduces))
	}
	...
	if got := facts.Coverage[domain.ContractKindMessaging]; got != (domain.CoverageCounts{Candidates: 1, Edges: 1}) {
