...
type fileCtx struct {
	...
	// contracts, per domain.ContractKind*.
	cov map[string]domain.CoverageCounts
...
func newCoverage() map[string]domain.CoverageCounts {
	cov := make(map[string]domain.CoverageCounts, len(domain.ContractKinds))
	for _, kind := range domain.ContractKinds {
		cov[kind] = domain.CoverageCounts{}
	}
	return cov
}
...
// directly.
func (fc *fileCtx) addEdge(srcIdx int, kind, dstName string, line int, conf float32, meta *store.EdgeMeta) {
	if srcIdx < 0 {
		return
	}
	if kind == store.EdgeHTTPCall && meta != nil {
	...
	fc.edges = append(fc.edges, &domain.Edge{
		SrcID:      srcMark(srcIdx),
		Kind:       kind,
		DstName:    dstName,
		Line:       line,
		Confidence: conf,
		Meta:       store.EncodeEdgeMeta(meta),
	})
}
...
func edgeNeedsBaseConf(kind string) bool {
	switch kind {
	case store.EdgeCall, store.EdgeHandledBy, store.EdgeRPCCall,
		store.EdgeImplementsRPC, store.EdgeHTTPCall,
...
func (fc *fileCtx) addRoute(n *sitter.Node, method, path, handler string, line int, conf float32, detectedBy string) int {
	idx := fc.addUnit(n, store.KindHTTPRoute, method+" "+path, routeKey(method, path), "path:"+path, "")
	if detectedBy != "" {
		fc.units[idx].Meta = store.EncodeUnitMeta(&store.UnitMeta{DetectedBy: detectedBy})
	}
	if handler != "" {
		fc.addEdge(idx, store.EdgeHandledBy, lastComponent(handler), line, conf, nil)
	}
	return idx
}
...
var callableKinds = []string{"function", "method", store.KindHTTPRoute}
