...
`
	...
	e := findEdge(edges, store.EdgeCall, "g")
	if e == nil {
...
`
	...
	if got := store.DecodeEdgeMeta(e.Meta).Aliases["x"]; got != "body.UserID" {
...
`
	...
	if got := store.DecodeEdgeMeta(e.Meta).Aliases["x"]; got != "user_id" {
...
`
	...
	if got := store.DecodeEdgeMeta(e.Meta).Aliases["x"]; got != "userId" {
...
`
	...
	if got := store.DecodeEdgeMeta(e.Meta).Aliases["id"]; got != "req.UserID" {
		t.Errorf("g: Aliases[id] = %q, want req.UserID", got)
	}
	e = findEdge(edges, store.EdgeCall, "h")
	if e == nil {
		t.Fatalf("missing call edge to h; edges: %+v", edgeNames(edges))
	}
	if got := store.DecodeEdgeMeta(e.Meta).Aliases["id"]; got != "order.ID" {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for call, want := range map[string]string{"g": "req.userId", "h": "order.oid"} {
				e := findEdge(edges, store.EdgeCall, call)
				if e == nil {
					t.Fatalf("missing call edge to %s; edges: %+v", call, edgeNames(edges))
				}
				got := store.DecodeEdgeMeta(e.Meta).Aliases["id"]
				if tt.lang == "python" {
...
`
	...
	if got := store.DecodeEdgeMeta(e.Meta).Aliases["x"]; got != "extractUserID(req)" {
...
`
	...
	if got, ok := store.DecodeEdgeMeta(e.Meta).Aliases["topic"]; ok {
...
`
	...
	if got := store.DecodeEdgeMeta(e.Meta).Aliases["x"]; got != "extract_user_id(req)" {
