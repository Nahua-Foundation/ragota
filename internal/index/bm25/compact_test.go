...
func corpusFiles(n int) []*index.FileToIndex {
...
func buildInWindows(t *testing.T, files []*index.FileToIndex, window int) *Indexer {
	t.Helper()
...
func buildInWindowsAt(t *testing.T, dir string, files []*index.FileToIndex, window int) *Indexer {
	t.Helper()
	...
	for start := 0; start < len(files); start += window {
		...
		batch := make([]*index.FileToIndex, 0, end-start)
		for _, f := range files[start:end] {
			cp := *f
			batch = append(batch, &cp)
		}
		if _, err := idx.Index(context.Background(), &index.IndexRequest{RepoID: "r1", Files: batch}); err != nil {
...
func scoredHits(t *testing.T, idx *Indexer, query string, limit int) map[string]float32 {
	t.Helper()
	res, err := idx.Search(context.Background(), &index.SearchQuery{Query: query, Limit: limit})
	if err != nil {
...
func corpusStats(t *testing.T, idx *Indexer, field string) (cardinality int, docs uint64) {
	...
	bm25Reader, ok := reader.(bleveIndexAPI.BM25Reader)
	if !ok {
...
func TestSegmentLayoutStillMovesTheCorpusStatistics(t *testing.T) {
	...
	compacted := func(files []*index.FileToIndex) (int, uint64) {
...
// loader asks for one compaction when it is done (see app.CompactIndexes).
...
func TestIndexerIsACompactor(t *testing.T) {
	...
	defer idx.Close()
	if _, ok := any(idx).(index.Compactor); !ok {
		t.Fatal("*Indexer no longer implements index.Compactor")
	}
}
...
func TestCompactedReindexMatchesAFreshBuild(t *testing.T) {
	...
	for start := 0; start < len(files); start += 64 {
		batch := make([]*index.FileToIndex, 0, 64)
		for _, f := range files[start:min(start+64, len(files))] {
			cp := *f
			batch = append(batch, &cp)
		}
		if _, err := reopened.Index(context.Background(), &index.IndexRequest{RepoID: "r1", Files: batch}); err != nil {
