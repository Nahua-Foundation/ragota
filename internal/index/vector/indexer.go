...
// Indexer implements index.Indexer and index.Searcher for vector search.
type Indexer struct {
	embedder         llm.Embedder
	vecStore         store.VectorStorage
	chunkCfg         index.ChunkConfig
	cards            bool
	maxChars         int
	workers          int
	excludes         []string
...
func (i *Indexer) embedFileChunks(ctx context.Context, chunks []*index.Chunk) ([][]float32, error) {
	max := i.embedMaxChars()
	var lastErr error
	...
	for language, files := range byLanguage {
		...
		for _, file := range files {
			...
			if len(chunks) == 0 {
				...
				var err error
				chunks, err = index.ForFile(i.chunkCfg, language).Chunk(ctx, &index.ChunkInput{
					Path:     file.Path,
					Language: language,
...
func (i *Indexer) storeFile(ctx context.Context, req *index.IndexRequest, job *fileJob,
	embeddings [][]float32, result *index.IndexResult, mu *sync.Mutex, fail func(string, string, error)) {
...
func (i *Indexer) Search(ctx context.Context, q *index.SearchQuery) (*index.SearchResult, error) {
	if len(q.Vector) > 0 {
...
func (i *Indexer) searchWithVector(ctx context.Context, q *index.SearchQuery, vector []float32) (*index.SearchResult, error) {
	start := time.Now()

	filters := index.ParseFilters(q.Filter)

	opts := store.VectorSearchOpts{
		Query:     vector,
	...
	hits := make([]*index.Hit, 0, len(results))
...
func buildCards(path, language string, content []byte, annotations map[string]string) []*index.Chunk {
	...
	chunks := make([]*index.Chunk, 0, len(units))
	for _, u := range units {
		...
		if note := annotations[index.AnnotationKey(path, u.StartLine)]; note != "" {
			b.WriteString(note)
			b.WriteString("\n")
		}
