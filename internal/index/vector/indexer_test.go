...
// memVecStore is a minimal in-memory store.VectorStorage. The indexer
...
func (m *memVecStore) Upsert(ctx context.Context, points []*store.VectorPoint) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *memVecStore) Search(ctx context.Context, opts store.VectorSearchOpts) ([]*store.VectorResult, error) {
	results := make([]*store.VectorResult, 0, len(m.points))
...
func (m *memVecStore) Stats(ctx context.Context) (*store.VectorStats, error) {
	return &store.VectorStats{Documents: int64(len(m.points))}, nil
}
...
func indexOne(t *testing.T, idx *Indexer, path, language, content string) *index.IndexResult {
	t.Helper()
...
func TestCardsModeBuildsSymbolCards(t *testing.T) {
	...
	bySymbol := map[string]*store.VectorPoint{}
	...
	for _, sym := range []string{"Add", "Sub"} {
		p, ok := bySymbol[sym]
		if !ok {
			t.Fatalf("no card point for symbol %s; points: %d", sym, len(vs.points))
		}
		if p.Kind == "" {
...
func TestCardsModeFallsBackToWindows(t *testing.T) {
	...
	idx := New(&Config{
		Embedder: fakeEmbedder{},
		Storage:  vs,
		Chunking: index.ChunkConfig{Method: "window", WindowLines: 5, Overlap: 1},
		Cards:    true,
	})
	...
	if len(vs.points) < 2 {
		t.Fatalf("expected multiple window chunks, got %d", len(vs.points))
	}
	for _, p := range vs.points {
		if p.Symbol != "" {
...
func TestVectorSearchFilters(t *testing.T) {
	...
	idx := New(&Config{Embedder: fakeEmbedder{}, Storage: vs})
...
func TestVectorSearchOverFetchesWhenFiltering(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		...
		vs.points = append(vs.points, &store.VectorPoint{
			ID:       string(rune('a' + i)),
			RepoID:   "r1",
...
func manyFiles(n int) []*index.FileToIndex {
	files := make([]*index.FileToIndex, n)
	for i := range files {
		files[i] = &index.FileToIndex{
...
func (g *gatedStore) Upsert(ctx context.Context, points []*store.VectorPoint) error {
	<-g.gate
...
func TestStoreDoesNotBlockEmbedding(t *testing.T) {
	...
	vs := &gatedStore{gate: make(chan struct{})}
	...
	idx := New(&Config{
		...
		Chunking: index.ChunkConfig{Method: "window", WindowLines: 2, Overlap: 0},
	})

	big := make([]*index.FileToIndex, files)
	for i := range big {
		var b strings.Builder
		for line := 0; line < 2*packTargetTexts; line++ {
			fmt.Fprintf(&b, "// file %d line %d\n", i, line)
		}
		big[i] = &index.FileToIndex{
			Path: fmt.Sprintf("pkg/f%d.go", i), Language: "go", Content: []byte(b.String()),
		}
	}

	done := make(chan *index.IndexResult, 1)
	go func() {
		res, err := idx.Index(context.Background(), &index.IndexRequest{
...
func TestIndexExclude(t *testing.T) {
	...
	res, err := idx.Index(context.Background(), &index.IndexRequest{
		RepoID: "repo1", RepoPath: t.TempDir(), Files: []*index.FileToIndex{
...
// a card without it cannot be retrieved by a question that names the app.
...
func TestBuildCardsIncludesAnnotation(t *testing.T) {
	...
	ann := map[string]string{index.AnnotationKey("alloc.go", 3): note}
