...
func TestDistributedIndexing(t *testing.T) {
	...
	buildInstance := func() *app.Service {
		t.Helper()
		svc, err := bootstrap.Build(ctx, distributedConfig(dbPath))
		if err != nil {
	...
	repo, err := svc1.AddRepo(ctx, domain.SourceTypeLocal, &domain.AddRequest{
		Name: "microservices",
		Path: servertest.TestdataPath(t, "microservices"),
	})
	if err != nil {
	...
	syms, err := svc2.Symbols(ctx, domain.QueryOpts{RepoID: repo.ID, Name: "publishOrderCreated", Limit: 5})
	if err != nil {
