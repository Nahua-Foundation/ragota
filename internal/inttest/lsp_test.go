...
func lspHostRoot(t *testing.T) string {
	...
	return filepath.Dir(servertest.TestdataPath(t, ""))
}
...
func TestLSPServers(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.lang, func(t *testing.T) {
			...
			rootPath := servertest.TestdataPath(t, tc.root)
			if err := client.Initialize(ctx, mapper.ToURI(rootPath), tc.init); err != nil {
				t.Fatalf("initialize: %v", err)
			}

			filePath := servertest.TestdataPath(t, tc.file)
...
func TestLSPRefinement(t *testing.T) {
	...
	repoPath := servertest.TestdataPath(t, "microservices")
	...
	res, err := refiner.Index(ctx, &index.IndexRequest{
		RepoID:   repoID,
		RepoPath: repoPath,
	...
	restored, err := st.GetASTUnits(ctx, domain.QueryOpts{
	...
	edges, err := st.GetEdges(ctx, domain.QueryOpts{RepoID: repoID})
	if err != nil {
	...
	for _, list := range units {
		for _, u := range list {
			...
			e := &domain.Edge{
				...
				Kind: store.EdgeCall, DstName: target[0].Name,
