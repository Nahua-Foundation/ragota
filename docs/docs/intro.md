...
1. [Getting started](./getting-started.md) — the whole local setup on one
   page: install, the Qwen embedder and reranker, index your projects,
   connect an agent.
2. [Running a server](./server.md) — a config file, API keys, PostgreSQL,
   GPU-grade models, the LSP pass.
