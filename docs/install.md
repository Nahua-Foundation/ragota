...
configure, start the services the configuration names, run, connect. Almost
everything here is optional — the [minimal run](#the-minimal-run) needs no
config file, no Docker and no network, and it is the configuration the
...
(`checksums.txt` sits next to the archives), unpack, and put `ragota` on your
PATH:
...
The archive holds the binary and nothing else, because the binary is the
...
run. Building from source instead:
...
Every repository under `--source` is discovered, registered and indexed;
`--watch` keeps the index in step with saves; `--interactive` is a terminal
dashboard of the progress. Storage is SQLite under `~/.ragota`, retrieval is
AST + BM25. This already answers questions — you can jump straight to
[connecting an agent](#connect-an-agent) and return for the optional muscle
later.
...
1 the config is invalid, 2 something configured is unreachable. Run it after
each block you enable below.

First decision worth making: auth. Two keys separate what an agent may do
from what an operator may:
...
The agent gets the `read:` key and cannot be talked into a DELETE; details in
[Configuration](./configuration.md#auth-give-agents-the-read-key).

## The optional muscle, service by service

Each block is off by default, changes the [measured line](./quality.md) when
enabled, and is probed by `--check-config` once configured.
...
vector index it reorders (the measured table lives in `config.example.yaml`;
its conclusion: `top_n: 25` and a small model, not a big one).

On a laptop, llama.cpp serves the 0.6B in one line — the port matters
...
Docker. This is the one piece that still wants a clone of the repository —
the containers build from `deploy/lsp/` (`make lsp-up`) — and the `lsp:`
block needs the `host_root`/`mount_root` mapping explained in
`config.example.yaml`.

### The whole thing as one stack

If you cloned the repository, the full estate is two files: app, PostgreSQL,
Qdrant and the LSP servers in one, Ollama and a reranker in the other, wired
together by `deploy/config.docker.yaml`:
...
llama.cpp or vLLM service as above and set the vector `model:` to the pulled
tag.

## Run

```bash
ragota --config config.yaml --source ~/projects --watch --interactive run
```

`--source` also defines the run's **working set**: repositories from earlier
runs stay indexed but go dormant for unscoped retrieval. `ragota repos
list|activate|deactivate` adjusts it offline; [Getting
started](./getting-started.md#the-working-set) explains the semantics.

## Connect an agent

```bash
claude mcp add ragota -e RAGOTA_URL=http://localhost:8080 -e RAGOTA_MCP_KEY=$RAGOTA_MCP_KEY -- ragota mcp
ragota mcp -check      # proves the whole path: server, API version, key, scope
```

Then install the skills into the workspace the agent analyzes code in — they
teach it when the index beats its own grep, and they are versioned with the
binary:

```bash
cd ~/that-workspace && ragota skills install     # or: ragota skills install ~/.claude/skills
```

The ten tools, the environment reference and the design notes:
