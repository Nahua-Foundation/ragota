...
func (c *chunkedIntCoder) Write(w io.Writer) (int, error) {
	...
	// empty chunks. A skipped slot kept the end-offset the first pass left in
	// it, and the second lengths-to-offsets pass then treated that offset as a
	// length. Any term whose postings had an empty chunk before its last
	// non-empty chunk was written with an offset table that overruns its data;
	// reading such postings decodes foreign bytes — the "memUvarintReader
	// overflow" errors and index-out-of-range panics BM25 search hit, plus
	// silently wrong results for every other affected term.
	//
	// The fix processes chunks by their lengths and prefix-sums exactly once,
	// afterwards, so empty chunks keep length zero.
